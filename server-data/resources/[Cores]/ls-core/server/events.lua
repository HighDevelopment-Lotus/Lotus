local ImgurClientID = '' -- create account on imgur

AddEventHandler('playerDropped', function(reason) 
	local src = source
	TriggerClientEvent('LSCore:Client:OnPlayerUnload', src)
	TriggerClientEvent('LSCore:Player:UpdatePlayerPosition', src)
	TriggerEvent("ls-logs:server:SendLog", "joinleave", "Dropped", "red", "**".. GetPlayerName(src) .. "** ("..GetPlayerIdentifiers(src)[1]..")  left.. The reason is: " .. reason)
...
AddEventHandler('playerConnecting', function(playerName, setKickReason, deferrals)
	...
	TriggerEvent("ls-logs:server:SendLog", "joinleave", "Queue", "orange", "**"..name .. "** ("..json.encode(GetPlayerIdentifiers(src))..") in queue..")
...
AddEventHandler('LSCore:UpdatePlayer', function(data)
	...
	if Player ~= nil then
		...
		TriggerClientEvent("ls-ui:client:update:needs", source, NewHunger, NewThirst)
...
AddEventHandler('LSCore:Server:SetMetaData', function(Meta, Data)
	...
	if Player ~= nil then 
		...
		TriggerClientEvent("ls-ui:client:update:needs", source, Player.PlayerData.metadata["hunger"], Player.PlayerData.metadata["thirst"])
	end
end)
...
AddEventHandler('LSCore:ToggleDuty', function(Source)
	...
	if not Player.PlayerData.job.onduty then
		...
		if Player.PlayerData.job.name == 'police' then
			TriggerEvent("ls-police:server:UpdateCurrentCops")
		end
	else
