...
CloseInput = function() {
    ...
    $.post('http://ls-ui/CloseMenuSecond', JSON.stringify({clear: false}))
}

$(document).on('click', '.input-button-cancel', function(e) {
    e.preventDefault();
    $.post('http://ls-ui/ClickSound', JSON.stringify({}))
    $.post('http://ls-ui/TriggerInput', JSON.stringify({Input: false}))
    CloseInput();
});
...
$(document).on('click', '.input-button-accept', function(e) {
    ...
    if (CurrentInputType === 'Number') {
        ...
        if (NumberVal > 0) {
            $.post('http://ls-ui/TriggerInput', JSON.stringify({Input: NumberVal}))
            CloseInput();
        }
    } else {
        var TextVal = $('#remove-text').val();
        $.post('http://ls-ui/TriggerInput', JSON.stringify({Input: TextVal}))
        CloseInput();
    }
});
