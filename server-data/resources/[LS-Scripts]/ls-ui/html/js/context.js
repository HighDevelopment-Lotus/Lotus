...
$(document).on('click', '.context-item', function(e) {
    ...
    if (CanClick) {
        ...
        if (MenuData['SecondMenu'] != null && MenuData['SecondMenu'] != undefined) {
            $.post('http://ls-ui/TriggerContextMenu', JSON.stringify({ShitData: MenuData['Data']}))
            CanClick = false;
...
$(document).on('click', '.context-sub-item', function(e) {
    ...
    if (CanClick) {
        ...
        $.post('http://ls-ui/TriggerContextMenu', JSON.stringify({ShitData: MenuData['Event']}))
    }
});
...
$(document).on('click', '.return', function(e) {
    ...
    if (CanClick) {
        ...
        if (TotalMenuData['ReturnEvent'] != null && TotalMenuData['ReturnEvent'] != undefined) {
            $.post('http://ls-ui/TriggerContextMenu', JSON.stringify({ShitData: TotalMenuData['ReturnEvent']}))
        }
        GoBackToMainMenu()
    }
});
...
CloseContextMenu = function(DoCloseEvent) {
    if (DoCloseEvent) {
        ...
        if (TotalMenuData != null && TotalMenuData != undefined && TotalMenuData['CloseEvent'] != null && TotalMenuData['CloseEvent'] != undefined) {
            $.post('http://ls-ui/TriggerContextMenu', JSON.stringify({ShitData: TotalMenuData['CloseEvent']}))
        }
    }
    ...
    $.post('http://ls-ui/CloseContext', JSON.stringify({}))
}
...
GoToSubMenu = function(MenuData) {
    ...
    $.each(MenuData, function (key, value) {
        var AddMenuItem = `<div class="context-menu-item context-menu-hover context-sub-item" id="menu-item${key}">${value['Title']}</div>`
...
SetupContextMenu = function(Data) {
    ...
    $.each(Data['MainMenuItems'], function (key, value) {
        var AddMenuItem = `<div class="context-menu-item context-menu-hover context-item" id="menu-item${key}"><div class="context-menu-title">${value['Title']}</div><div class="context-menu-desc">${value['Desc']}</div></div>`
        $('.context-menu-items').append(AddMenuItem);
        $(`#menu-item${key}`).data('MenuData', value);  
    });
