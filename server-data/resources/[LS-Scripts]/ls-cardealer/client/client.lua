...
AddEventHandler('LSCore:Client:OnPlayerLoaded', function()
    Citizen.SetTimeout(1250, function()  
        ...
        LSCore.Functions.TriggerCallback("ls-cardealer:server:get:config", function(config)
            Config = config
        end)
        LoggedIn = true
    end)
                ...
                if Vehicle ~= 0 and Distance < 3.5 then
                    ...
                    if not ShowedInfo then
                        ...
                        if VehicleData ~= nil then
                            LSCore.Functions.TriggerCallback('ls-cardealer:server:get:vehicle:stock', function(ReturnValue) 
...
RegisterNetEvent('ls-cardealer:client:open:select:menu')
AddEventHandler('ls-cardealer:client:open:select:menu', function()
    local MenuItems = {}
    for k, v in pairs(Config.CardealerMenu) do
        local TempData = {}
        TempData['Title'] = v['Label']
        TempData['Desc'] = 'Selecteer Categorie'
        TempData['Data'] = {['Event'] = 'ls-cardealer:client:open:vehicle:menu', ['Type'] = 'Client', ['Category'] = v['Category']}
...
RegisterNetEvent('ls-cardealer:client:open:vehicle:menu')
AddEventHandler('ls-cardealer:client:open:vehicle:menu', function(data)
    Citizen.SetTimeout(500, function()
        local MenuItems = {}
        LSCore.Functions.TriggerCallback('ls-cardealer:server:get:cardealer:vehicles', function(ReturnValue) 
            for k, v in pairs(ReturnValue) do
                ...
                TempData['Data'] = {['Event'] = 'ls-cardealer:client:open:place:menu', ['Type'] = 'Client', ['Price'] = v['Price'], ['Vehicle'] = v['Vehicle']}
                table.insert(MenuItems, TempData)
            end
            if (#MenuItems > 0) then
...
RegisterNetEvent('ls-cardealer:client:open:place:menu')
AddEventHandler('ls-cardealer:client:open:place:menu', function(data)
    Citizen.SetTimeout(500, function()
        local MenuItems = {}
        for k, v in pairs(Config.CardealerSpots) do
            ...
            TempData['Data'] = {['Event'] = 'ls-cardealer:server:set:vehicle', ['Type'] = 'Server', ['Location'] = k, ['Vehicle'] = data['Vehicle']}
            table.insert(MenuItems, TempData)
        end
        if (#MenuItems > 0) then
...
RegisterNetEvent('ls-cardealer:client:test:closest:vehicle')
AddEventHandler('ls-cardealer:client:test:closest:vehicle', function(Niks, Entity)
...
RegisterNetEvent('ls-cardealer:client:sell:clossest:vehicle')
AddEventHandler('ls-cardealer:client:sell:clossest:vehicle', function(Niks, Entity)
    ...
    if IsModelASellVehicle(Entity['Entity']) then
        if Player ~= -1 and Distance < 2.5 then
            LSCore.Functions.Progressbar("lockpick-door", "Voertuig Verkopen..", 5500, false, false, {
                ...
                TriggerServerEvent('ls-cardealer:server:sell:closest', GetPlayerServerId(Player), GetEntityModel(Entity['Entity']))
...
RegisterNetEvent('ls-cardealer:client:spawn:vehicle')
AddEventHandler('ls-cardealer:client:spawn:vehicle', function(Model, Plate)
...
RegisterNetEvent('ls-cardealer:client:sync:data')
AddEventHandler('ls-cardealer:client:sync:data', function(ConfigData)
