        ...
        if PlayerData.job.name == "police" then
            ...
            if (IsPedInAnyVehicle( ped ) ) then 
                ...
                if (IsPoliceVehicle(vehicle)) then
                    ...
                    LSCore.Functions.Notify( "Je moet in een politie voertuig zitten!", "error")
                end 
            else 
                LSCore.Functions.Notify( "Je moet in een voertuig zitten!", "error")
            end 
        end
    end)
end)
...
AddEventHandler( 'wk:changeRadarLimit', function( speed ) 
    ...
    LSCore.Functions.Notify("Max snelheid gezet op: " .. speed .. "km/u")
end )
                ...
                if ( radarInfo.fwdXmit ) then  
                    ...
                    if ( DoesEntityExist( fwdVeh ) and IsEntityAVehicle( fwdVeh ) ) then 
                        ...
                        if plateChecked ~= fwdPlate then
                            LSCore.Functions.TriggerCallback('police:IsPlateFlagged', function(result)
                                if result then
                                    ...
                                    radarInfo.fwdFastLocked = true 
                                    SendNUIMessage( { lockfwdfast = true } )
                                    LSCore.Functions.Notify("Gemarkeerd voertuig gevonden!", "error")
                                end
        ...
        if LSCore ~= nil then 
            ...
            if ( not IsDisabledControlPressed( 0, Keys["LEFTSHIFT"] ) and IsDisabledControlJustPressed( 0, Keys["7"] ) ) then 
                if LSCore.Functions.GetPlayerData().job.name == "police" and LSCore.Functions.GetPlayerData().job.onduty then
                    ...
                    ResetFrontAntenna()
                    ResetRearAntenna()
                    LSCore.Functions.Notify("Politie ANPR gereset")
                end
            end 
            ...
            if ( IsDisabledControlJustPressed( 0, Keys["8"] ) ) then 
                if LSCore.Functions.GetPlayerData().job.name == "police" and LSCore.Functions.GetPlayerData().job.onduty then
                    ...
                    if (radarInfo.plateLocked) then
                        ...
                        LSCore.Functions.Notify("Kenteken locked: " .. radarInfo.fwdPlate)
                    else
                        SendNUIMessage( { unlockPlate = true } )
                        LSCore.Functions.Notify("Kenteken unlocked")
                    end
                end
            end
