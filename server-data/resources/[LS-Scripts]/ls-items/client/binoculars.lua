        ...
        if camera then
            ...
            if not ( IsPedSittingInAnyVehicle( lPed ) ) then
                Citizen.CreateThread(function()
                    ...
                    TriggerEvent("ls-police:client:set:evidence", true)
                end)
            end
            ...
            SetNightvision(false)
            SetSeethrough(false)
            TriggerEvent("ls-police:client:set:evidence", false)
        end
    end
end)
...
-- Activate binoculars
RegisterNetEvent('ls-items:binoculars:toggle')
AddEventHandler('ls-items:binoculars:toggle', function()
...
RegisterNetEvent('ls-items:camera:toggle')
AddEventHandler('ls-items:camera:toggle', function()
    camera = not camera
    if not camera then
        ClearPedTasks(GetPlayerPed(-1))
    end
