...
RegisterNetEvent('ls-radialmenu:client:flip:vehicle')
AddEventHandler('ls-radialmenu:client:flip:vehicle', function()
...
RegisterNetEvent("ls-radialmenu:client:send:panic:button")
AddEventHandler("ls-radialmenu:client:send:panic:button",function()
  LSCore.Functions.TriggerCallback('LSCore:HasItem', function(HasItem)
      if HasItem then
          ...
          TriggerServerEvent('ls-police:server:send:alert:panic:button', GetEntityCoords(GetPlayerPed(-1)), StreetLabel, Info)
      else
...
RegisterNetEvent("ls-radialmenu:client:send:down")
AddEventHandler("ls-radialmenu:client:send:down",function(Type)
    local Player = LSCore.Functions.GetPlayerData()
    local Info = {['Firstname'] = Player.charinfo.firstname, ['Lastname'] = Player.charinfo.lastname, ['Callsign'] = Player.metadata['callsign']}
    local StreetLabel = LSCore.Functions.GetStreetLabel()
    local Priority = 2
    ...
    TriggerServerEvent('ls-police:server:send:alert:officer:down', GetEntityCoords(GetPlayerPed(-1)), StreetLabel, Info, Priority)
end)

RegisterNetEvent("ls-radialmenu:client:open:door")
AddEventHandler("ls-radialmenu:client:open:door",function(DoorNumber)
...
RegisterNetEvent("ls-radialmenu:client:enter:radio")
AddEventHandler("ls-radialmenu:client:enter:radio",function(RadioNumber)
    LSCore.Functions.TriggerCallback('LSCore:HasItem', function(HasItem)
        if HasItem then
            exports['ls-radio']:SetRadioState(true)
            exports['ls-radio']:JoinRadio(RadioNumber, 1)
        else
