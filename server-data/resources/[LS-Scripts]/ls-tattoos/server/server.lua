...
LSCore.Functions.CreateCallback('ls-tattoos:GetPlayerTattoos', function(source, cb)
	local src = source
...
LSCore.Functions.CreateCallback('ls-tattoos:GetPlayerTattoosMC', function(source, cb, citizenid)
	local src = source
	local xPlayer = LSCore.Functions.GetPlayer(src)
...
LSCore.Functions.CreateCallback('ls-tattoos:PurchaseTattoo', function(source, cb, tattooList, price, tattoo, tattooName)
	local src = source
	local xPlayer = LSCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ls-tattoos:RemoveTattoo')
AddEventHandler('ls-tattoos:RemoveTattoo', function (tattooList)
	local src = source
	local xPlayer = LSCore.Functions.GetPlayer(src)
	LSCore.Functions.ExecuteSql(true, "UPDATE `players` SET `tattoos` = '"..json.encode(tattooList).."' WHERE citizenid = '"..xPlayer.PlayerData.citizenid.."'", function(result)
	end)
