local LSCore, Stashes, TempStashes, Gloveboxes, Trunks = exports['ls-core']:GetCoreObject(), {}, {}, {}, {}
...
RegisterServerEvent('ls-inventory-new:server:open:inventory:other')
AddEventHandler('ls-inventory-new:server:open:inventory:other', function(OtherData, Type, Slots, MaxWeight, ExtraData)
	local src = source
	...
	if Type == 'Glovebox' then
		if not OpenInventories['G'..OtherData] then
			...
			TriggerClientEvent('ls-inventory-new:client:open:inventory:other', src, TotalData)
			Citizen.SetTimeout(5, function()
				OpenInventories['G'..OtherData] = true
			end)
		else
			TriggerClientEvent('ls-inventory-new:client:open:empty:other', source)
		end
	...
	elseif Type == 'OtherPlayer' then
		if not OpenInventories['OT'..OtherData] then
			...
			if OtherPlayer ~= nil then
				...
				TriggerClientEvent('ls-inventory-new:client:set:inventory:state', tonumber(OtherData), false)
				TriggerClientEvent('ls-inventory-new:client:open:inventory:other', src, TotalData)
				Citizen.SetTimeout(5, function()
	...
	elseif Type == 'Drop' then
		if not OpenInventories['D'..OtherData['SubType']] then
			TriggerClientEvent('ls-inventory-new:client:open:inventory:other', src, OtherData)
			Citizen.SetTimeout(5, function()
...
RegisterServerEvent('ls-inventory-new:server:check:other')
AddEventHandler('ls-inventory-new:server:check:other', function(Type, Name)
	if Type == 'Glovebox' then
	...
	elseif Type == 'OtherPlayer' then
		...
		TriggerClientEvent('ls-inventory-new:client:set:inventory:state', tonumber(Name), true)
	end
end)

RegisterServerEvent('ls-inventory-new:server:disable:other')
AddEventHandler('ls-inventory-new:server:disable:other', function(PlayerId, State)
	local OtherPlayer = LSCore.Functions.GetPlayer(tonumber(PlayerId))
	if OtherPlayer ~= nil then
		TriggerClientEvent('ls-inventory-new:client:set:inventory:state', tonumber(PlayerId), State)
	end
end)

RegisterServerEvent('ls-inventory-new:server:steal:money')
AddEventHandler('ls-inventory-new:server:steal:money', function(TargetId)
	...
	if TargetPlayer ~= nil then
		...
		if StealCash > 0 then
			if SourcePlayer.PlayerData.job.name == 'police' and SourcePlayer.PlayerData.job.onduty then
				...
				TriggerClientEvent('ls-inventory-new:client:update:player', source)
			else
...
RegisterServerEvent('ls-inventory-new:server:set:player:items')
AddEventHandler('ls-inventory-new:server:set:player:items', function(PlayerItems)
	local Player = LSCore.Functions.GetPlayer(source)
	Player.Functions.SetItemData(PlayerItems)
end)

RegisterServerEvent('ls-inventory-new:server:done:crafting')
AddEventHandler('ls-inventory-new:server:done:crafting', function(ItemName, Amount, ToSlot, Info, Cost)
	local Player = LSCore.Functions.GetPlayer(source)
...
RegisterServerEvent('ls-inventory-new:server:done:combinding')
AddEventHandler('ls-inventory-new:server:done:combinding', function(FromSlot, FromItem, ToSlot, ToItem, Reward)
	local Player = LSCore.Functions.GetPlayer(source)
...
RegisterServerEvent('ls-inventory-new:server:use:item')
AddEventHandler('ls-inventory-new:server:use:item', function(Slot)
	local Player = LSCore.Functions.GetPlayer(source)
	local ItemData = Player.Functions.GetItemBySlot(Slot)
	if ItemData ~= nil and ItemData.amount > 0 then
		if LSCore.Functions.CanUseItem(ItemData.name) then
			TriggerClientEvent('ls-inventory-new:client:item:box', source, 'Used', Shared.ItemList[ItemData.name], false)
			LSCore.Functions.UseItem(source, ItemData)
		elseif ItemData.type == 'weapon' then
			if ItemData.info.quality ~= nil then
				if ItemData.info.quality > 0 then
					TriggerClientEvent('ls-inventory-new:client:item:box', source, 'Used', Shared.ItemList[ItemData.name], false)
					TriggerClientEvent('ls-inventory-new:client:use:weapon', source, ItemData)
				else
...
RegisterServerEvent('ls-inventory-new:server:add:new:drop:core')
AddEventHandler('ls-inventory-new:server:add:new:drop:core', function(Source, ItemName, Amount, Info)
	local source = Source
	...
	TriggerClientEvent('ls-inventory-new:client:update:drops', -1, Config.Drops[RandomId], RandomId)
end)

LSCore.Functions.CreateCallback('ls-inventory:server:is:player:crafting', function(source, cb, target)
...
LSCore.Functions.CreateCallback('ls-inventory:server:do:item:data', function(source, cb, data)
    local src = source
	...
	if ToInventory == '.my-inventory-blocks' and FromInventory == '.other-inventory-blocks' then
		if Type == 'Store' then
			if Player.PlayerData.money['cash'] >= (OtherInventoryItems[FromSlot].price * Amount) then
				...
				if Player.Functions.AddItem(OtherInventoryItems[FromSlot].name, Amount, ToSlot, OtherInventoryItems[FromSlot].info, false, 'Inventory') then
					Player.Functions.RemoveMoney('cash', (OtherInventoryItems[FromSlot].price * Amount)) 
					if SubType == 'WeaponDealer' then
						TriggerClientEvent('ls-inventory-new:client:force:close', src)
						TriggerEvent('ls-randomdealer:server:set:weapons', 'Minus', OtherInventoryItems[FromSlot].name, 1)
					elseif SubType == 'DealerDealer' then
						TriggerClientEvent('ls-inventory-new:client:force:close', src)
						TriggerEvent('ls-dealers:server:update:dealer:items', OtherInventoryItems[FromSlot].slot, Amount, ExtraData)
					end
					cb(true)
				else
					cb(false)
			...
			if Config.Drops[SubType] ~= nil and Config.Drops[SubType]['Items'] ~= nil then
				if Config.Drops[SubType]['Items'][FromSlot] ~= nil then
					if Player.Functions.AddItem(Config.Drops[SubType]['Items'][FromSlot].name, Amount, ToSlot, Config.Drops[SubType]['Items'][FromSlot].info, false, 'Inventory') then
						TriggerEvent("ls-logs:server:SendLog", "drop", "Item Removed", 'green', "(citizenid: *"..Player.PlayerData.citizenid.."* | name: "..Player.PlayerData.name.." | id: *(" ..src..")* Heeft **"..Amount..'x '..Config.Drops[SubType]['Items'][FromSlot].name.. "** opgepakt van drop: "..SubType)
						...
						TriggerClientEvent('ls-inventory-new:client:update:drops', -1, Config.Drops[SubType], SubType)
						cb(true)
					else
						cb(false)
					end
		...
		elseif Type == 'Trunk' then
			local DBItems = Trunks[SubType] ~= nil and Trunks[SubType] or TriggerClientEvent('ls-inventory-new:client:force:close', src)
			if DBItems ~= nil and DBItems[FromSlot] ~= nil then
				if Player.Functions.AddItem(DBItems[FromSlot].name, Amount, ToSlot, DBItems[FromSlot].info, false, 'Inventory') then
					TriggerEvent("ls-logs:server:SendLog", "trunk", "Item Removed", 'green', "(citizenid: *"..Player.PlayerData.citizenid.."* | name: "..Player.PlayerData.name.." | id: *(" ..src..")* Heeft **"..Amount..'x '..DBItems[FromSlot].name.. "** gepakt uit de kofferbak van het kenteken: "..SubType)
		...
		elseif Type == 'Glovebox' then
			local DBItems = Gloveboxes[SubType] ~= nil and Gloveboxes[SubType] or TriggerClientEvent('ls-inventory-new:client:force:close', src)
			if DBItems ~= nil and DBItems[FromSlot] ~= nil then
				if Player.Functions.AddItem(DBItems[FromSlot].name, Amount, ToSlot, DBItems[FromSlot].info, false, 'Inventory') then
					TriggerEvent("ls-logs:server:SendLog", "glovebox", "Item Removed", 'green', "(citizenid: *"..Player.PlayerData.citizenid.."* | name: "..Player.PlayerData.name.." | id: *(" ..src..")* Heeft **"..Amount..'x '..DBItems[FromSlot].name.. "** gepakt uit de glovebox van het kenteken: "..SubType)
					if DBItems[FromSlot].amount - Amount == 0 then
						DBItems[FromSlot] = nil
					else
						DBItems[FromSlot].amount = DBItems[FromSlot].amount - Amount
		...
		elseif Type == 'Stash' then
			local DBItems = Stashes[SubType] ~= nil and Stashes[SubType] or TriggerClientEvent('ls-inventory-new:client:force:close', src)
			if DBItems ~= nil and DBItems[FromSlot] ~= nil then
				if Player.Functions.AddItem(DBItems[FromSlot].name, Amount, ToSlot, DBItems[FromSlot].info, false, 'Inventory') then
					TriggerEvent("ls-logs:server:SendLog", "stash", "Item Removed", 'green', "(citizenid: *"..Player.PlayerData.citizenid.."* | name: "..Player.PlayerData.name.." | id: *(" ..src..")* Heeft **"..Amount..'x '..DBItems[FromSlot].name.. "** gepakt uit de stash met het id: "..SubType)
					if DBItems[FromSlot].amount - Amount == 0 then
						DBItems[FromSlot] = nil
					else
						DBItems[FromSlot].amount = DBItems[FromSlot].amount - Amount
		...
		elseif Type == 'TempStashes' then
			local DBItems = TempStashes[SubType] ~= nil and TempStashes[SubType] or TriggerClientEvent('ls-inventory-new:client:force:close', src)
			if DBItems ~= nil and DBItems[FromSlot] ~= nil then
				if Player.Functions.AddItem(DBItems[FromSlot].name, Amount, ToSlot, DBItems[FromSlot].info, false, 'Inventory') then
					TriggerEvent("ls-logs:server:SendLog", "stash", "Item Removed", 'green', "(citizenid: *"..Player.PlayerData.citizenid.."* | name: "..Player.PlayerData.name.." | id: *(" ..src..")* Heeft **"..Amount..'x '..DBItems[FromSlot].name.. "** gepakt uit de stash met het id: "..SubType)
					if DBItems[FromSlot].amount - Amount == 0 then
		...
		elseif Type == 'OtherPlayer' then
			if Player.Functions.AddItem(OtherPlayer.PlayerData.inventory[FromSlot].name, Amount, ToSlot, OtherPlayer.PlayerData.inventory[FromSlot].info, false, 'Inventory') then
				...
				TriggerEvent("ls-logs:server:SendLog", "robbing", "Item Removed", 'green', "(citizenid: *"..Player.PlayerData.citizenid.."* | name: "..Player.PlayerData.name.." | id: *(" ..src..")* Heeft **"..Amount..'x '..OtherPlayer.PlayerData.inventory[FromSlot].name.. "** gepakt uit de zakken van: ("..OtherPlayer.PlayerData.citizenid..') '..OtherPlayer.PlayerData.name)
				...
				if ItemDataFrom['type'] == 'weapon' then
					TriggerClientEvent('ls-inventory-new:client:reset:weapon', OtherPlayer.PlayerData.source)
				end
				TriggerClientEvent('ls-inventory-new:client:update:player', OtherPlayer.PlayerData.source)
				cb(true)
			else
				cb(false)
			end
		elseif Type == 'Crafting' then
			if HasCraftingItems(src, OtherInventoryItems[FromSlot]['Cost'], Amount) then
				...
				Player.Functions.SetMetaData("craftingrep", Player.PlayerData.metadata["craftingrep"] + (OtherInventoryItems[FromSlot]['AddingPoints'] * Amount))
				TriggerClientEvent('ls-inventory-new:client:craft', src, OtherInventoryItems[FromSlot]['ItemName'], Amount, ToSlot, OtherInventoryItems[FromSlot]['Info'], OtherInventoryItems[FromSlot]['Cost'])
				cb('Crafting')
			else
				cb(false)
			end
	...
	elseif ToInventory == '.other-inventory-blocks' and FromInventory == '.my-inventory-blocks' then
			...
			if Config.Drops[SubType] ~= nil and Config.Drops[SubType]['Items'] ~= nil then
				...
				if Player.PlayerData.inventory[FromSlot] ~= nil then
					if WeightCheck(MaxOtherWeight, TotalWeight, Amount, Player.PlayerData.inventory[FromSlot].weight) then
						...
						TriggerEvent("ls-logs:server:SendLog", "drop", "Dropped", 'red', "(citizenid: *"..Player.PlayerData.citizenid.."* | name: "..Player.PlayerData.name.." | id: *(" ..src..")* Heeft **"..Amount..'x '..Player.PlayerData.inventory[FromSlot].name.. "** op de grond gegooid met id: "..SubType)
		...
		elseif Type == 'Trunk' then
			...
			if Player.PlayerData.inventory[FromSlot] ~= nil then
				if WeightCheck(MaxOtherWeight, TotalWeight, Amount, Player.PlayerData.inventory[FromSlot].weight) then
					Player.Functions.RemoveItem(Player.PlayerData.inventory[FromSlot].name, Amount, FromSlot, false)
					TriggerEvent("ls-logs:server:SendLog", "trunk", "Trunk Item Added", 'red', "(citizenid: *"..Player.PlayerData.citizenid.."* | name: "..Player.PlayerData.name.." | id: *(" ..src..")* Heeft **"..Amount..'x '..Player.PlayerData.inventory[FromSlot].name.. "** in de kofferbak gegooid met het kenteken: "..SubType)
		...
		elseif Type == 'Glovebox' then
			...
			if Player.PlayerData.inventory[FromSlot] ~= nil then
				if WeightCheck(MaxOtherWeight, TotalWeight, Amount, Player.PlayerData.inventory[FromSlot].weight) then
					Player.Functions.RemoveItem(Player.PlayerData.inventory[FromSlot].name, Amount, FromSlot, false)
					TriggerEvent("ls-logs:server:SendLog", "glovebox", "Glovebox Item Added", 'red', "(citizenid: *"..Player.PlayerData.citizenid.."* | name: "..Player.PlayerData.name.." | id: *(" ..src..")* Heeft **"..Amount..'x '..Player.PlayerData.inventory[FromSlot].name.. "** in het dashboard gegooid met het kenteken: "..SubType)
					if DBItems[ToSlot] == nil then
						DBItems[ToSlot] = {
							name = Player.PlayerData.inventory[FromSlot].name,
							slot = ToSlot,
			...
			if Player.PlayerData.inventory[FromSlot] ~= nil then
				if WeightCheck(MaxOtherWeight, TotalWeight, Amount, Player.PlayerData.inventory[FromSlot].weight) then
					if ExtraData ~= Player.PlayerData.inventory[FromSlot].name then
						Player.Functions.RemoveItem(Player.PlayerData.inventory[FromSlot].name, Amount, FromSlot, false)
						TriggerEvent("ls-logs:server:SendLog", "stash", "Stash Item Added", 'red', "(citizenid: *"..Player.PlayerData.citizenid.."* | name: "..Player.PlayerData.name.." | id: *(" ..src..")* Heeft **"..Amount..'x '..Player.PlayerData.inventory[FromSlot].name.. "** in de stash gegooid met id: "..SubType)
						if DBItems[ToSlot] == nil then
							DBItems[ToSlot] = {
								name = Player.PlayerData.inventory[FromSlot].name,
								slot = ToSlot,
		...
		elseif Type == 'OtherPlayer' then
			if OtherPlayer.Functions.AddItem(Player.PlayerData.inventory[FromSlot].name, Amount, ToSlot, Player.PlayerData.inventory[FromSlot].info, false, 'Inventory') then
				Player.Functions.RemoveItem(Player.PlayerData.inventory[FromSlot].name, Amount, FromSlot, false)
				TriggerEvent("ls-logs:server:SendLog", "robbing", "Player Item Added", 'red', "(citizenid: *"..Player.PlayerData.citizenid.."* | name: "..Player.PlayerData.name.." | id: *(" ..src..")* Heeft **"..Amount..'x '..Player.PlayerData.inventory[FromSlot].name.. "** in de zakken gegooid van ("..OtherPlayer.PlayerData.citizenid..') '..OtherPlayer.PlayerData.name)
				TriggerClientEvent('ls-inventory-new:client:update:player', OtherPlayer.PlayerData.source)
				cb(true)
			else
				cb(false)
			...
			if vehicleresult ~= nil and vehicleresult[1] ~= nil then
				LSCore.Functions.ExecuteSql(false, "SELECT * FROM `player_inventory-vehicle` WHERE `plate` = '"..Name.."'", function(result)
					if result ~= nil and result[1] ~= nil then
						...
						LSCore.Functions.ExecuteSql(false, "INSERT INTO `player_inventory-vehicle` (`plate`, `trunkitems`) VALUES ('"..Name.."', '"..json.encode(ItemsJson).."')")
					end
				end)
			end
		end)
			...
			if vehicleresult ~= nil and vehicleresult[1] ~= nil then
				LSCore.Functions.ExecuteSql(false, "SELECT * FROM `player_inventory-vehicle` WHERE `plate` = '"..Name.."'", function(result)
					if result ~= nil and result[1] ~= nil then
						...
						LSCore.Functions.ExecuteSql(false, "INSERT INTO `player_inventory-vehicle` (`plate`, `gloveboxitems`) VALUES ('"..Name.."', '"..json.encode(ItemsJson).."')")
					end
				end)
			end
		end)
	elseif Type == 'Stash' then
		LSCore.Functions.ExecuteSql(false, "SELECT * FROM `player_inventory-stash` WHERE `stash` = '"..Name.."'", function(result)
			if result ~= nil and result[1] ~= nil then
				...
				LSCore.Functions.ExecuteSql(false, "INSERT INTO `player_inventory-stash` (`stash`, `items`) VALUES ('"..Name.."', '"..json.encode(ItemsJson).."')")
			end
		end)
	end
end
