...
$('document').ready(function() {
    ...
    Progressbar.Progress = function(data) {
        ...
        $(".progress-container").fadeIn('fast', function() {
            $("#progress-bar").stop().css({"width": 0, "background-color": "#396991"}).animate({
              width: '100%'
            }, {
              ...
              complete: function() {
                $(".progress-container").fadeOut('fast', function() {
                    ...
                    $.post('http://ls-progressbar/FinishAction', JSON.stringify({
                        })
                    );
                })
              }
    ...
    Progressbar.ProgressCancel = function() {
        ...
        cancelledTimer = setTimeout(function () {
            $(".progress-container").fadeOut('fast', function() {
                $("#progress-bar").css("width", 0);
                $.post('http://ls-progressbar/CancelAction', JSON.stringify({
                    })
                );
            });
        }, 1000);
