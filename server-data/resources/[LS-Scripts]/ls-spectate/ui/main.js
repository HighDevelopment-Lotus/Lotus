$(function(){
	...
	$('.users').on('click', '.spec', function(){
		...
		if (target == player) {
			...
			$('.spectate').fadeOut();
			$.post('http://ls-spectate/select', JSON.stringify({id: target}));
		}
	...
	// 	} else {
	// 	    $('.spectate').fadeOut();
	// 		$.post('http://ls-spectate/kick', JSON.stringify({id: target, reason: reason}));
	// 	}
	// });

	$('.header').on('click', '#close', function(){
		$('.spectate').fadeOut();
		$.post('http://ls-spectate/quit');
	});
