local LSCore, LoggedIn, CurrentPlate = exports['ls-core']:GetCoreObject(), false, nil
local Clicked, IsRobbing, LastVehicle = false, false, nil

RegisterNetEvent('LSCore:Client:OnPlayerLoaded')
AddEventHandler('LSCore:Client:OnPlayerLoaded', function()
    Citizen.SetTimeout(1250, function()
        LSCore.Functions.TriggerCallback("ls-vehiclekeys:server:get:config", function(ConfigData)
           Config = ConfigData
        end)
        LoggedIn = true
    end)
    ...
    while true do
        ...
        if LoggedIn then
            if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                ...
                if not HasCurrentKey and GetPedInVehicleSeat(Vehicle, -1) == GetPlayerPed(-1) then
                    SetVehicleEngineOn(Vehicle, false, false, true)
                end
            else
                CurrentPlate = nil
            ...
            if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
                ...
                if not IsPauseMenuActive() then
                    if IsDisabledControlJustPressed(0, 14) then
                        if GetPedInVehicleSeat(Vehicle, -1) == PlayerPedId() then
                            TriggerEvent('ls-vehiclekeys:client:toggle:engine', false)
                        end
                    end
                    if IsDisabledControlJustPressed(0, 15) then
                        if GetPedInVehicleSeat(Vehicle, -1) == PlayerPedId() then
                            TriggerEvent('ls-vehiclekeys:client:toggle:engine', true)
                        end
                    end
                end
            else
    ...
    while true do
        ...
        if LoggedIn then
            if IsControlJustReleased(1, 182) then
                TriggerEvent('ls-vehiclekeys:client:toggle:locks')
            end
        else
            Citizen.Wait(450)
        end
...
RegisterNetEvent('ls-vehiclekeys:client:sync:keys')
AddEventHandler('ls-vehiclekeys:client:sync:keys', function(Plate, KeyData)
...
RegisterNetEvent('ls-vehiclekeys:client:give:key')
AddEventHandler('ls-vehiclekeys:client:give:key', function()
...
RegisterNetEvent('ls-vehiclekeys:client:toggle:engine')
AddEventHandler('ls-vehiclekeys:client:toggle:engine', function(Type)
    local Vehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
...
RegisterNetEvent('ls-vehiclekeys:client:toggle:locks')
AddEventHandler('ls-vehiclekeys:client:toggle:locks', function()
    local TargetEntity, EntityType, TargetCoords = exports['ls-ui']:GetEntityPlayerIsLookingAt(1.0, 0.2, 2)
    if EntityType == 2 then
        local Distance = #(GetEntityCoords(GetPlayerPed(-1)) - TargetCoords)
        if Distance < 0.8 then
            ...
            if HasVehicleKey(Plate) then
                if VehicleLocks == 1 then
                    ...
                    TriggerEvent('ls-vehicleley:client:blink:lights', TargetEntity)
                    TriggerServerEvent("ls-sound:server:play:distance", 4.5, "car-lock", 0.2)
                else
                    SetVehicleDoorsLocked(TargetEntity, 1)
                    ClearPedTasks(GetPlayerPed(-1))
                    TriggerEvent('ls-vehicleley:client:blink:lights', TargetEntity)
                    TriggerServerEvent("ls-sound:server:play:distance", 4.5, "car-unlock", 0.2)
                end
...
AddEventHandler('ls-items:client:use:lockpick', function(IsAdvanced)
    ...
    if VehDistance <= 4.5 then
        if not HasVehicleKey(Plate) then
            if IsPedInAnyVehicle(GetPlayerPed(-1)) and GetPedInVehicleSeat(Vehicle, -1) == PlayerPedId() then
                exports['ls-assets']:RequestAnimationDict("anim@amb@clubhouse@tutorial@bkr_tut_ig3@")
                ...
                TaskPlayAnim(GetPlayerPed(-1), 'anim@amb@clubhouse@tutorial@bkr_tut_ig3@', 'machinic_loop_mechandplayer' ,3.0, 3.0, -1, 16, 0, false, false, false)
                    exports['ls-ui']:StartSkillTest(3, 'Normal', function(Outcome)
                        if Outcome then
                            ...
                            TriggerEvent('ls-vehicleley:client:blink:lights', Vehicle)
                            TriggerServerEvent("ls-sound:server:play:distance", 5, "car-unlock", 0.2)
                            StopAnimTask(GetPlayerPed(-1), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
                        else
                            local RemoveChance = LSCore.Functions.GetMiniGameSkill('Lockpick')
                            if IsAdvanced then
...
RegisterNetEvent('ls-vehicleley:client:blink:lights')
AddEventHandler('ls-vehicleley:client:blink:lights', function(Vehicle)
...
function SetVehicleKeys(Plate, Bool, CitizenId)
    ...
    TriggerServerEvent('ls-vehiclekeys:server:set:keys', Plate, Bool, Cid)
end
