...
LSCore.Commands.Add("admin", "Open het admin menu", {}, false, function(source, args)
    TriggerClientEvent('ls-admin:client:open:admin:menu', source)
end, 'admin')
...
LSCore.Commands.Add("s", "Bericht naar alle staff sturen", {{name="bericht", help="Bericht die je wilt sturen"}}, true, function(source, args)
    ...
    TriggerClientEvent('ls-admin:client:staffchat:message', -1, GetPlayerName(source), Message)
end, "admin")

LSCore.Commands.Add("openstash", "Open een stash van iemand", {{name="stashid", help="Stash ID"}}, true, function(source, args)
    local Message = table.concat(args, " ")
    if Message ~= nil then
        TriggerClientEvent('ls-admin:client:openstash', source, Message)
    end
end, "admin")
...
LSCore.Commands.Add("report", "Stuur een report naar admins (alleen wanneer nodig, MAAK HIER GEEN MISBRUIK VAN)", {{name="bericht", help="Bericht die je wilt sturen"}}, true, function(source, args)
    ...
    TriggerClientEvent('ls-admin:client:send:report', -1, GetPlayerName(source), source, Message)
    TriggerClientEvent('chatMessage', source, "REPORT VERSTUURD", "normal", Message)
    TriggerEvent("ls-logs:server:SendLog", "report", "Report", "green", "**"..GetPlayerName(source).."** (CitizenID: "..Player.PlayerData.citizenid.." | ID: "..source..") **Report:** " ..Message, false)
end)
...
LSCore.Commands.Add("reportr", "Reply op een report", {}, false, function(source, args)
    ...
    if OtherPlayer ~= nil then
        ...
        TriggerEvent("ls-logs:server:SendLog", "report", "Report Reply", "red", "**"..GetPlayerName(source).."** reageerde op: **"..OtherPlayer.PlayerData.name.. " **(ID: "..OtherPlayer.PlayerData.source..") **Bericht:** " ..Message, false)
...
RegisterServerEvent('ls-admin:server:slay:player')
AddEventHandler('ls-admin:server:slay:player', function(TargetPlayer)
    if LSCore.Functions.HasPermission(source, Config.AdminPerms['KillPlayer']) then
        TriggerClientEvent("ls-admin:client:slay:player", TargetPlayer)
    end
end)

RegisterServerEvent('ls-admin:server:kick:chosen:player')
AddEventHandler('ls-admin:server:kick:chosen:player', function(TargetPlayer, Reason)
    if LSCore.Functions.HasPermission(source, Config.AdminPerms['KickPlayer']) then
        DropPlayer(TargetPlayer, "\n🛑 Je bent gekicked uit de server!\n🛑 Reden: "..Reason.."\n\n")
    end
end)

RegisterServerEvent('ls-admin:server:bring:chosen:player')
AddEventHandler('ls-admin:server:bring:chosen:player', function(TargetPlayer, Coords)
    if LSCore.Functions.HasPermission(source, Config.AdminPerms['BringPlayer']) then
        TriggerClientEvent('ls-admin:client:bring:chosen:player', TargetPlayer, Coords)
    end
end)

RegisterServerEvent('ls-admin:server:opem:skin:menu')
...
RegisterServerEvent('ls-admin:server:send:report')
AddEventHandler('ls-admin:server:send:report', function(Name, TargetSrc, Message)
...
RegisterServerEvent('ls-admin:server:staffchat:message')
AddEventHandler('ls-admin:server:staffchat:message', function(Name, Message)
    if LSCore.Functions.HasPermission(source, "admin") then
        if LSCore.Functions.IsOptin(source) then
