...
local LSCore, LoggedIn, AdminMenuOpen = exports['ls-core']:GetCoreObject(), false, false
IsInNoclip = false
        ...
        if LoggedIn and AdminMenuOpen then
            ...
            elseif Menu.IsMenuOpened('AdminOptions') then 
                ...
                if Menu.MenuButton('Revive Spelers (Distance)', 'AdminOptions') then
                    ...
                    for k, v in pairs(DistancePlayers) do
                        ...
                        TriggerServerEvent('ls-hospital:server:revive:player', PlayerServerId)
                    end
                end
                if Menu.MenuButton('Kill Spelers (Distance)', 'AdminOptions') then
                    ...
                    for k, v in pairs(DistancePlayers) do
                        local PlayerServerId = GetPlayerServerId(v)
                        TriggerServerEvent('ls-admin:server:slay:player', PlayerServerId)
                    end
                end
                Menu.Display()
            ...
            elseif Menu.IsMenuOpened('PlayerOption') then
                if Menu.MenuButton('Revive Speler', 'PlayerOptions') then
                    TriggerServerEvent('ls-hospital:server:revive:player', CurrentPlayer['ServerId'])
                end
                if Menu.MenuButton('Dood Speler', 'PlayerOptions') then
                    TriggerServerEvent('ls-admin:server:slay:player', CurrentPlayer['ServerId'])
                end
                --if Menu.MenuButton('Spectate Speler', 'PlayerOptions') then
                --    --
                --end
                if Menu.MenuButton('Open Inventaris', 'PlayerOptions') then
                    TriggerEvent("ls-inventory-new:client:show:button:steal", CurrentPlayer['ServerId'])
                    TriggerServerEvent('ls-inventory-new:server:open:inventory:other', CurrentPlayer['ServerId'], 'OtherPlayer', false, false)
                    Menu.CloseMenu()
                end
                if Menu.MenuButton('Geef Kleding Menu', 'PlayerOptions') then
                    TriggerServerEvent('ls-admin:server:opem:skin:menu', CurrentPlayer['ServerId'])
                    Menu.CloseMenu()
                end
                Menu.Display()
                ...
                if Menu.MenuButton('Kick Speler', 'AdminOption') then
                    ...
                    LSCore.Functions.OpenInput(Data, function(ReturnData)
                        if ReturnData ~= false then
                            TriggerServerEvent('ls-admin:server:kick:chosen:player', CurrentPlayer['ServerId'], ReturnData)
                            Menu.CloseMenu()
                        end
                    end)
                end
            ...
            elseif Menu.IsMenuOpened('TeleportOpties') then
                if Menu.MenuButton('Breng Speler', 'TeleportOpties') then
                    TriggerServerEvent('ls-admin:server:bring:chosen:player', CurrentPlayer['ServerId'], GetEntityCoords(GetPlayerPed(-1)))
                end
                if Menu.MenuButton('Ga Naar Speler', 'TeleportOpties') then
                    local TargetCoords = GetEntityCoords(CurrentPlayer['Ped'])
                    SetEntityCoords(GetPlayerPed(-1), TargetCoords)
                end
                Menu.Display()
                ...
                if Menu.MenuButton('~b~Spawn Item', 'ItemManagment', false, false) then
                    if CurrentItemName ~= 'None' then
                        if CurrentItemAmount > 0 then
                            TriggerServerEvent('ls-admin:server:give:item', CurrentItemName, CurrentItemAmount)
                        else
...
RegisterNetEvent('ls-admin:client:open:admin:menu')
AddEventHandler('ls-admin:client:open:admin:menu', function()
    SetupMenus()
    AdminMenuOpen = true
    Menu.OpenMenu('AdminMenu')
end)

RegisterNetEvent('ls-admin:client:slay:player')
AddEventHandler('ls-admin:client:slay:player', function()
    SetEntityHealth(GetPlayerPed(-1), 0.0)
end)

RegisterNetEvent('ls-admin:client:bring:chosen:player')
AddEventHandler('ls-admin:client:bring:chosen:player', function(Coords)
    SetEntityCoords(GetPlayerPed(-1), Coords)
end)

RegisterNetEvent('ls-admin:client:send:report')
AddEventHandler('ls-admin:client:send:report', function(Name, Source, Message)
    TriggerServerEvent('ls-admin:server:send:report', Name, Source, Message)
end)

RegisterNetEvent('ls-admin:client:staffchat:message')
AddEventHandler('ls-admin:client:staffchat:message', function(Name, Message)
    TriggerServerEvent('ls-admin:server:staffchat:message', Name, Message)
end)

RegisterNetEvent('ls-admin:client:openstash')
AddEventHandler('ls-admin:client:openstash', function(StashID)
    if exports['ls-inventory-new']:CanOpenInventory() then
        TriggerServerEvent('ls-inventory-new:server:open:inventory:other', StashID, 'Stash', 200, 5000000)
    end
end)
...
function SetupMenus()
    ...
    Menu.SetSubTitle("AdminMenu", "Lotus Admin Menu")
    
    Menu.SetMenuMaxOptionCountOnScreen("AdminMenu", 20)

    Menu.CreateSubMenu('AdminOptions', 'AdminMenu')
    Menu.SetSubTitle("AdminOptions", "Lotus Admin Menu")
