...
$(document).on('click', '.multichar-arrow-right', function(e) {
    if (CanClick) {
        CanClick = false;
        $.post('http://ls-multichar/ClickSound', JSON.stringify({}))
        $('.multichar-container').slideUp(450, function() {
            CurrentCharNumber = CurrentCharNumber + 1
            $.post('http://ls-multichar/GetChar', JSON.stringify({CharNumber: CurrentCharNumber}), function(CharData){
    ...
    if (CanClick) {
        ...
        if (CreatingChar) {
            ...
            if (CharName != '' && CharLast != '' && CharBirth != '') {
                $.post('http://ls-multichar/CreateChar', JSON.stringify({Slot: CurrentCharNumber, FirstName: CharName, LastName: CharLast, BirthDate: CharBirth, Gender: ResultGender}))
                CloseCharScreen()
            } else {
                $.post('http://ls-multichar/ErrorSound', JSON.stringify({}))
            }
        } else {
            $.post('http://ls-multichar/ChooseChar', JSON.stringify({CitizenId: CurrentCitizenId}))
            CloseCharScreen()
        }
    }
});
...
$(document).on('click', '.multichar-delete-button', function(e) {
    $.post('http://ls-multichar/ClickSound', JSON.stringify({}))
    $.post('http://ls-multichar/DeleteCharacter', JSON.stringify({CitizenId: CurrentCitizenId}))
    CloseCharScreen()
});

SetupFirstChar = function() {
    $.post('http://ls-multichar/GetChar', JSON.stringify({CharNumber: 1}), function(CharData){
        SetCharData(CharData)
        $('.multichar-container').slideDown(500)
    });
}
...
SetCharData = function(Data) {
    if (Data['Active']) {
        ...
        $('.multichar-date').html('Datum: <br><span class="small-font">'+Data['Date']+'</span>');
        $('.multichar-work').html('Werk: <br><span class="small-font">'+Data['JobName']+'</span>');
        $('.multichar-bsn').html('BSN: <br><span class="small-font">'+Data['CitizenId']+'</span>');
...
CloseCharScreen = function() {
    $.post('http://ls-multichar/CloseUi', JSON.stringify({}))
    $('.multichar-container').slideUp(450, function() {
        $('.multichar-arrow-left').hide(0);
        $('.multichar-arrow-right').show(0);
        $('.multichar-play-button').html('Bevestigen');
