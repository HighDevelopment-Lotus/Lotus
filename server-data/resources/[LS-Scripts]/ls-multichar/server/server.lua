...
LSCore.Functions.CreateCallback("ls-multichar:server:get:char:data", function(source, cb)
    local src = source
...
RegisterServerEvent('ls-multichar:server:select:char')
AddEventHandler('ls-multichar:server:select:char', function(CitizenId)
    local src = source
    if LSCore.Player.Login(src, false, CitizenId) then
        ...
        TriggerClientEvent('ls-spawn:client:choose:spawn', src, Player.PlayerData)
        TriggerEvent("ls-logs:server:SendLog", "joinleave", "Loaded", "green", "**".. GetPlayerName(src) .. "** ("..CitizenId.." | "..src..") loaded..")
	end
end)

RegisterServerEvent('ls-multichar:server:create:new:char')
AddEventHandler('ls-multichar:server:create:new:char', function(NewCharData)
    local src = source
    ...
    if LSCore.Player.Login(src, true, false, newData) then
        ...
        TriggerClientEvent('ls-spawn:client:choose:appartment', src)
        TriggerEvent("ls-logs:server:SendLog", "joinleave", "Character Creation", "green", "**".. GetPlayerName(src) .. "** ("..src..") Created their character..")
	end
end)

RegisterServerEvent('ls-multichar:server:delete:char')
AddEventHandler('ls-multichar:server:delete:char', function(CitizenId)
