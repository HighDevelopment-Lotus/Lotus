local LSCore, LoggedIn, ActivePeds, NearDealer = exports['ls-core']:GetCoreObject(), false, {}, false

RegisterNetEvent('LSCore:Client:OnPlayerLoaded')
AddEventHandler('LSCore:Client:OnPlayerLoaded', function()
    Citizen.SetTimeout(1250, function()
	    LSCore.Functions.TriggerCallback("ls-randomdealer:server:get:config", function(config)
            Config = config
        end)
       	LoggedIn = true
    end)
...
RegisterNetEvent('ls-randomdealer:client:talk')
AddEventHandler('ls-randomdealer:client:talk', function()
...
RegisterNetEvent('ls-randomdealer:client:sync:data')
AddEventHandler('ls-randomdealer:client:sync:data', function(Data)
	Config.CurrentDealerData = Data
end)

RegisterNetEvent('ls-randomdealer:client:set:dealer:data')
AddEventHandler('ls-randomdealer:client:set:dealer:data', function(Data, Type)
	Config.CurrentDealerData = Data
	if Type == 'Set' then
		exports['ls-assets']:RequestModelHash('a_m_m_eastsa_01')
