...
LSCore.Functions.CreateCallback("ls-houseplants:server:get:config", function(source, cb)
    cb(Config)
end)
...
Citizen.CreateThread(function()
    LSCore.Functions.ExecuteSql(false, "SELECT * FROM `player_house_plants`", function(result)
        for k, v in pairs(result) do
            ...
            TriggerClientEvent('ls-houseplants:client:sync:plant:data', -1, v.houseid, Config.Plants[v.houseid])
        end
    end)
end)

RegisterServerEvent('ls-houseplants:server:destroy:plant')
AddEventHandler('ls-houseplants:server:destroy:plant', function(HouseId, RemoveId, IsDead)
    local src = source
    ...
    if HouseId ~= nil then
        LSCore.Functions.ExecuteSql(false, "SELECT * FROM `player_house_plants` WHERE `houseid` = '"..HouseId.."'", function(result)
            ...
            Citizen.SetTimeout(500, function()
                ...
                if IsDead then
                    Player.Functions.AddItem('wet-tak', math.random(1, 3), false, false, true)
                end
            end)
        end)
    end
...
RegisterServerEvent('ls-houseplants:server:add:plant')
AddEventHandler('ls-houseplants:server:add:plant', function(HouseId, NewPlant)
    if HouseId ~= nil then
        LSCore.Functions.ExecuteSql(false, "SELECT * FROM `player_house_plants` WHERE `houseid` = '"..HouseId.."'", function(result)
            if result[1] ~= nil then
                ...
                LSCore.Functions.ExecuteSql(false, "INSERT INTO `player_house_plants` (`houseid`, `plants`) VALUES ('"..HouseId.."', '"..json.encode(NewPlant).."')")
...
RegisterServerEvent('ls-houseplants:server:feed:plant')
AddEventHandler('ls-houseplants:server:feed:plant', function(HouseId, PlantId)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
...
RegisterServerEvent('ls-houseplants:server:harvest:plant')
AddEventHandler('ls-houseplants:server:harvest:plant', function(HouseId, PlantId, Amount)
   local src = source
   local Player = LSCore.Functions.GetPlayer(src)
   ...
   if Config.Plants[HouseId] ~= nil and Config.Plants[HouseId][PlantId] ~= nil then
        if PlasticBag.amount >= RandomWeedAmount then
            ...
            TriggerEvent('ls-houseplants:server:destroy:plant', HouseId, PlantId, false)
        else
...
function RefreshPlants(HouseId)
    LSCore.Functions.ExecuteSql(false, "SELECT * FROM `player_house_plants` WHERE `houseid` = '"..HouseId.."'", function(result)
        ...
        TriggerClientEvent('ls-houseplants:client:sync:plant:data', -1, HouseId, Config.Plants[HouseId])
        TriggerClientEvent('ls-houseplants:client:sync:plants', -1, HouseId)
    end)
end