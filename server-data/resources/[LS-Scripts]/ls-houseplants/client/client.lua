...
AddEventHandler('LSCore:Client:OnPlayerLoaded', function()
    Citizen.SetTimeout(1250, function()
        LSCore.Functions.TriggerCallback('ls-houseplants:server:get:config', function(ConfigData)
          Config = ConfigData
        end)
        LoggedIn = true
    end)
...
RegisterNetEvent('ls-houseplants:client:sync:plant:data')
AddEventHandler('ls-houseplants:client:sync:plant:data', function(HouseId, ConfigData)
    Config.Plants[HouseId] = ConfigData
end)

RegisterNetEvent('ls-houseplants:client:sync:plants')
AddEventHandler('ls-houseplants:client:sync:plants', function(HouseId)
    if CurrentHouse ~= nil then
...
RegisterNetEvent('ls-houseplants:client:feed:plants')
AddEventHandler('ls-houseplants:client:feed:plants', function()
    if CurrentHouse ~= nil then
...
RegisterNetEvent('ls-houseplants:client:use:sheer')
AddEventHandler('ls-houseplants:client:use:sheer', function()
    if CurrentHouse ~= nil then
        if ClosestPlant ~= nil then
            DestroyDeadPlant(ClosestPlant)
        else
...
RegisterNetEvent('ls-houseplants:client:plant')
AddEventHandler('ls-houseplants:client:plant', function(PlantName, PlantType, ItemName)
    if CurrentHouse ~= nil then
        if ClosestPlant == nil then
        ...
        LSCore.Functions.TriggerCallback('LSCore:HasItem', function(HasItem)
            if HasItem then
                LSCore.Functions.Progressbar("pick-weed", "Plukken..", 10000, false, true, {
                    ...
                    TriggerServerEvent('ls-houseplants:server:harvest:plant', CurrentHouse, PlantId, math.random(1, 5))
                    StopAnimTask(GetPlayerPed(-1), "amb@world_human_gardener_plant@male@base", "base", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
                end, function()
                    StopAnimTask(GetPlayerPed(-1), "amb@world_human_gardener_plant@male@base", "base", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
                    LSCore.Functions.Notify("Geannuleerd..", "error")
...
function DestroyDeadPlant(PlantId)
    if Config.Plants[CurrentHouse][PlantId]['Health'] <= 0 then
        LSCore.Functions.Progressbar("destroy-weed", "Vernietigen..", 10000, false, true, {
            ...
            StopAnimTask(GetPlayerPed(-1), "amb@world_human_gardener_plant@male@base", "base", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
            TriggerServerEvent('ls-houseplants:server:destroy:plant', CurrentHouse, PlantId, true)
        end, function()
            StopAnimTask(GetPlayerPed(-1), "amb@world_human_gardener_plant@male@base", "base", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
            LSCore.Functions.Notify("Geannuleerd..", "error")
        end)
...
function FeedSelectedPlant(PlantId)
    ...
    LSCore.Functions.Progressbar("feed-weed", "Voeden..", 10000, false, true, {
        ...
        TriggerServerEvent('ls-houseplants:server:feed:plant', CurrentHouse, PlantId)
...
function AddPlant(PlantName, PlantType)
    ...
    if CurrentHouse ~= nil then
        if Config.Plants[CurrentHouse] ~= nil then
            ...
            TriggerServerEvent('ls-houseplants:server:add:plant', CurrentHouse, Config.Plants[CurrentHouse])
        else
