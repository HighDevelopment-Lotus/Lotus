...
LSCore.Commands.Add("addammo", "Geef kogels voor het wapen wat je vast hebt", {{name="Hoeveelheid", help="hoeveel kogels?"}}, true, function(source, args)
    if args[1] ~= nil then
      TriggerClientEvent('ls-weapons:client:set:ammo', source, args[1])
    end
end, "admin")
...
LSCore.Functions.CreateUseableItem("pistol_ammo", function(source, item)
	...
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent('ls-weapons:client:reload:ammo', source, 'AMMO_PISTOL', 'pistol_ammo')
    end
end)
...
LSCore.Functions.CreateUseableItem("rifle_ammo", function(source, item)
	local Player = LSCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent('ls-weapons:client:reload:ammo', source, 'AMMO_RIFLE', 'rifle_ammo')
    end
end)
...
LSCore.Functions.CreateUseableItem("smg_ammo", function(source, item)
	local Player = LSCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent('ls-weapons:client:reload:ammo', source, 'AMMO_SMG', 'smg_ammo')
    end
end)
...
LSCore.Functions.CreateUseableItem("shotgun_ammo", function(source, item)
	local Player = LSCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent('ls-weapons:client:reload:ammo', source, 'AMMO_SHOTGUN', 'shotgun_ammo')
    end
end)
...
LSCore.Functions.CreateUseableItem("taser_ammo", function(source, item)
	local Player = LSCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent('ls-weapons:client:reload:ammo', source, 'AMMO_TAZER', 'taser_ammo')
    end
end)
...
LSCore.Functions.CreateUseableItem("paintball_ammo", function(source, item)
	local Player = LSCore.Functions.GetPlayer(source)
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent('ls-weapons:client:reload:ammo', source, 'AMMO_PAINTBALL', 'paintball_ammo')
    end
end)

RegisterServerEvent('ls-weapons:server:update:quality')
AddEventHandler('ls-weapons:server:update:quality', function(data)
            ...
            if not IsWeaponBlocked(WeaponData['IdName']) then
                if WeaponSlot.info.quality ~= nil then
                    if WeaponSlot.info.quality - DecreaseAmount > 0 then
                        ...
                        TriggerClientEvent('ls-inventory-new:client:reset:weapon', source)
...
RegisterServerEvent("ls-weapons:server:update:weapon:ammo")
AddEventHandler('ls-weapons:server:update:weapon:ammo', function(CurrentWeaponData, Amount)
    local Player = LSCore.Functions.GetPlayer(source)
