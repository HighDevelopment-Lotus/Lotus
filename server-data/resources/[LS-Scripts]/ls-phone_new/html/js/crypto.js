...
function SetupCryptoData(Crypto) {
    ...
    $(".crypto-action-page-wallet").html("Portefeuille: "+CryptoData.Portfolio+" Bitcoin('s)");
...
$(document).on('click', '#buy-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (LS.Phone.Data.PlayerData.money.bank >= Price) {
            $.post('http://ls-phone_new/BuyCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    LS.Phone.Data.PlayerData.money.bank = parseInt(LS.Phone.Data.PlayerData.money.bank) - parseInt(Price);
                    LS.Phone.Notifications.Add("fas fa-university", "LSBank", "Er is &euro; "+Price+",- afgeschreven!", "#badc58", 2500);
                } else {
                    LS.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Je hebt niet genoeg geld..", "#badc58", 1500);
                }
            });
        } else {
            LS.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Je hebt niet genoeg geld..", "#badc58", 1500);
        ...
        LS.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Vul alles in!", "#badc58", 1500);
    }
});
...
$(document).on('click', '#sell-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('http://ls-phone_new/SellCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    LS.Phone.Data.PlayerData.money.bank = parseInt(LS.Phone.Data.PlayerData.money.bank) + parseInt(Price);
                    LS.Phone.Notifications.Add("fas fa-university", "LSBank", "Er is &euro; "+Price+",- bijgeschreven!", "#badc58", 2500);
                } else {
                    LS.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Je hebt niet genoeg Bitcoins..", "#badc58", 1500);
                }
            });
        } else {
            LS.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Je hebt niet genoeg Bitcoins..", "#badc58", 1500);
    ...
    if ((Coins !== "") && (WalletId !== "")) {
        if (CryptoData.Portfolio >= Coins) {
            if (WalletId !== CryptoData.WalletId) {
                $.post('http://ls-phone_new/TransferCrypto', JSON.stringify({
                    Coins: Coins,
                    WalletId: WalletId,
                }), function(CryptoData){
                    if (CryptoData == "notenough") {
                        ...
                        LS.Phone.Notifications.Add("fas fa-university", "Crypto", "Dit Wallet-ID bestaat niet!", "#badc58", 2500);
                    } else {
                        UpdateCryptoData(CryptoData)
                        CloseCryptoPage()
                        LS.Phone.Notifications.Add("fas fa-university", "Crypto", "Je hebt "+Coins+",- overgemaakt naar "+WalletId+"!", "#badc58", 2500);
                    }
                });
            } else {
                LS.Phone.Notifications.Add("fas fa-university", "Crypto", "Je kan niet naar jezelf overmaken..", "#badc58", 2500);
            }
        } else {
            LS.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Je hebt niet genoeg Bitcoins..", "#badc58", 1500);
        }
