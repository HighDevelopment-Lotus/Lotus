...
$(document).on('click', '.whatsapp-chat', function(e){
    ...
    LS.Phone.Functions.SetupChatMessages(ChatData);

    $.post('http://ls-phone_new/ClearAlerts', JSON.stringify({
...
LS.Phone.Functions.GetLastMessage = function(messages) {
...
LS.Phone.Functions.LoadWhatsappChats = function(chats) {
    $(".whatsapp-chats").html("");
    $.each(chats, function(i, chat){
        ...
        var LastMessage = LS.Phone.Functions.GetLastMessage(chat.messages);
...
LS.Phone.Functions.ReloadWhatsappAlerts = function(chats) {
    $.each(chats, function(i, chat){
        if (chat.Unread > 0 && chat.Unread !== undefined && chat.Unread !== null) {
            $(".unread-chat-id-"+i).html(chat.Unread);
            $(".unread-chat-id-"+i).css({"display":"block"});
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    var SanitizeChat = Message.replace("<", "&lt;").replace(">", "&gt;").replace("(", "&lpar;").replace(")", "&rpar;").replace(/'/g, "&apos;").replace(/"/g, '&quot;').replace(":", "&colon;").replace(";", "&semi;").replace("$", "&dollar;").replace("[", "&dollar;").replace("]", "&dollar;");

    if (Message !== null && Message !== undefined && Message !== "") {
        $.post('http://ls-phone_new/SendMessage', JSON.stringify({
            ChatNumber: OpenedChatData.number,
            ChatDate: GetCurrentDateKey(),
            ChatMessage: SanitizeChat,
            ChatTime: FormatMessageTime(),
            ChatType: "message",
        }));
        $("#whatsapp-openedchat-message").val("");
    } else {
        LS.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "Je kan geen leeg bericht sturen!", "#25D366", 1750);
    }
});
...
// LS.Phone.Functions.SetupChatMessages = function(cData, NewChatData) {
//     if (cData) {
//         OpenedChatData.number = cData.number;

//         if (OpenedChatPicture == null) {
//             $.post('http://ls-phone_new/GetProfilePicture', JSON.stringify({
...
//                 var Sender = "me";
//                 if (message.sender !== LS.Phone.Data.PlayerData.citizenid) { Sender = "other"; }
//                 var MessageElement
    ...
    if (cData) {
        ...
        $.each(cData.messages, function(i, chat){
            ...
            $.each(cData.messages[i].messages, function(index, message){
                ...
                var MessageElement
                if (message.type == "message") {
                    if (htmlMatch.test(message.message)) {
                    ...
                    var messageFormatted = message.message.replace("<", "&lt;").replace(">", "&gt;").replace("(", "&lpar;").replace(")", "&rpar;").replace(/'/g, "&apos;").replace(/"/g, '&quot;').replace(":", "&colon;").replace(";", "&semi;").replace("$", "&dollar;").replace("[", "&dollar;").replace("]", "&dollar;");
                    MessageElement = '<div class="whatsapp-openedchat-message whatsapp-openedchat-message-'+Sender+'">'+messageFormatted+'<div class="whatsapp-openedchat-message-time">'+message.time+'</div></div><div class="clearfix"></div>'
                } else if (message.type == "location") {
...
$(document).on('click', '.whatsapp-shared-location', function(e){
    ...
    $.post('http://ls-phone_new/SharedLocation', JSON.stringify({
