...
$(document).on('click', '.racing-race', function(e){
    ...
    var IsRacer = IsInRace(LS.Phone.Data.PlayerData.citizenid, RaceData.RaceData.Racers)
    ...
    if (!RaceData.RaceData.Started || IsRacer) {
        ...
        LS.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "De race is al gestart..", "#1DA1F2");
    }
});
...
function SetupRaces(Races) {
    ...
    if (Races.length > 0) {
        ...
        $.each(Races, function(i, race){
            ...
            var InRace = IsInRace(LS.Phone.Data.PlayerData.citizenid, race.RaceData.Racers);
            var Creator = IsCreator(LS.Phone.Data.PlayerData.citizenid, race);
...
$(document).on('click', '#join-race', function(e){
    ...
    $.post('http://ls-phone_new/IsInRace', JSON.stringify({}), function(IsInRace){
        if (!IsInRace) {
            $.post('http://ls-phone_new/RaceDistanceCheck', JSON.stringify({
                RaceId: Data.RaceId,
                Joined: true,
            }), function(InDistance){
                if (InDistance) {
                    $.post('http://ls-phone_new/IsBusyCheck', JSON.stringify({
                        check: "editor"
                    }), function(IsBusy){
                        if (!IsBusy) {
                            $.post('http://ls-phone_new/JoinRace', JSON.stringify({
                                RaceData: Data,
                            }));
                            ...
                            LS.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Je zit in een editor..", "#1DA1F2");
                        }
                    });
                }
            })
        } else {
            LS.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Je zit al in een race..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#quit-race', function(e){
    ...
    $.post('http://ls-phone_new/LeaveRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '#start-race', function(e){
    ...
    $.post('http://ls-phone_new/StartRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '.dropdown .dropdown-menu li', function(e) {
    $.post('http://ls-phone_new/GetTrackData', JSON.stringify({
        RaceId: $(this).attr('id')
    }), function(TrackData){
...
$(document).on('click', '#setup-race', function(e){
    ...
    $.post('http://ls-phone_new/GetRaces', JSON.stringify({}), function(Races){
...
$(document).on('click', '#create-race', function(e){
    e.preventDefault();
    $.post('http://ls-phone_new/IsAuthorizedToCreateRaces', JSON.stringify({}), function(data){
            ...
            if (!data.IsBusy) {
                $.post('http://ls-phone_new/IsBusyCheck', JSON.stringify({
                    ...
                    if (!InRace) {
                        ...
                        LS.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Je zit in een race..", "#1DA1F2");
                    }
                });
            } else {
                LS.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Je bent al een Track aan het maken..", "#1DA1F2");
            }
        } else {
            LS.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Je hebt geen rechten om Race Track's te maken..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#racing-create-accept', function(e){
    ...
    if (TrackName !== "" && TrackName !== undefined && TrackName !== null) {
        $.post('http://ls-phone_new/IsAuthorizedToCreateRaces', JSON.stringify({
            TrackName: TrackName
        }), function(data){
            if (data.IsAuthorized) {
                if (data.IsNameAvailable) {
                    $.post('http://ls-phone_new/StartTrackEditor', JSON.stringify({
                        TrackName: TrackName
                    }));
                    ...
                    LS.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Deze naam is niet beschikbaar..", "#1DA1F2");
                }
            } else {
                LS.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Je hebt geen rechten om Race Track's te maken..", "#1DA1F2");
            }
        ...
        LS.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Je moet een Tracknaam invullen..", "#1DA1F2");
    }
});
...
$(document).on('click', '#setup-race-accept', function(e){
    ...
    $.post('http://ls-phone_new/HasCreatedRace', JSON.stringify({}), function(HasCreatedRace){
        if (!HasCreatedRace) {
            $.post('http://ls-phone_new/RaceDistanceCheck', JSON.stringify({
                RaceId: track,
                Joined: false,
                ...
                if (InDistance) {
                    if (track !== undefined || track !== null) {
                        if (laps !== "") {
                            $.post('http://ls-phone_new/CanRaceSetup', JSON.stringify({}), function(CanSetup){
                                if (CanSetup) {
                                    $.post('http://ls-phone_new/SetupRace', JSON.stringify({
                                        RaceId: track,
                                        AmountOfLaps: laps,
                                    }))
                                    $(".racing-overview").animate({
                                    ...
                                    LS.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Er kunnen momenteel geen races gemaakt worden..", "#1DA1F2");
                                }
                            });
                        } else {
                            LS.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Vul een hoeveelheid rondes in..", "#1DA1F2");
                        }
                    } else {
                        LS.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Je hebt geen Track gekozen..", "#1DA1F2");
                    }
                }
            })
        } else {
            LS.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Je hebt al een race actief..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#leaderboards-race', function(e){
    ...
    $.post('http://ls-phone_new/GetRacingLeaderboards', JSON.stringify({}), function(Races){
        if (Races !== null) {
