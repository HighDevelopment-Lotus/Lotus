...
$(document).on('click', '.phone-app-footer-button', function(e){
    ...
    if (PressedFooterTab !== CurrentFooterTab) {
        ...
        if (PressedFooterTab == "recent") {
            $.post('http://ls-phone_new/ClearRecentAlerts');
...
LS.Phone.Functions.SetupRecentCalls = function(recentcalls) {
...
$(document).on('click', '.phone-recent-call', function(e){
    ...
    //console.log(LS.Phone.Data.AnonymousCall)

    if (RecentData.name == "Anoniem") {
        LS.Phone.Notifications.Add("fas fa-phone", "Telefoon", "Je kan een anoniem nummer niet terug bellen!")
    } else {
...
LS.Phone.Functions.LoadContacts = function(myContacts) {
...
$(document).on('click', '#new-chat-phone', function(e){
    ...
    if (ContactData.number !== LS.Phone.Data.PlayerData.charinfo.phone) {
        ...
        setTimeout(function(){
            ...
            LS.Phone.Functions.ToggleApp("phone", "none");
            LS.Phone.Functions.ToggleApp("whatsapp", "block");
            LS.Phone.Data.currentApplication = "whatsapp";
        
            $.post('http://ls-phone_new/GetWhatsappChat', JSON.stringify({phone: ContactData.number}), function(chat){
                LS.Phone.Functions.SetupChatMessages(chat, {
        ...
        LS.Phone.Notifications.Add("fa fa-phone-alt", "Telefoon", "Je kunt jezelf niet appen, sad fuck..", "default", 3500);
    }
});
...
$(document).on('click', '#edit-contact', function(e){
    ...
    LS.Phone.Animations.TopSlideDown(".phone-edit-contact", 200, 0);
});
...
$(document).on('click', '#edit-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('http://ls-phone_new/EditContact', JSON.stringify({
            ...
            LS.Phone.Functions.LoadContacts(PhoneContacts);
        });
        LS.Phone.Animations.TopSlideUp(".phone-edit-contact", 250, -100);
        setTimeout(function(){
            $(".phone-edit-contact-number").val("");
            $(".phone-edit-contact-name").val("");
        }, 250)
    } else {
        LS.Phone.Notifications.Add("fas fa-exclamation-circle", "Contact Bewerken", "Vul alle velden in!");
    }
});
...
$(document).on('click', '#edit-contact-delete', function(e){
    ...
    $.post('http://ls-phone_new/DeleteContact', JSON.stringify({
        CurrentContactName: ContactName,
        CurrentContactNumber: ContactNumber,
        CurrentContactIban: ContactIban,
    }), function(PhoneContacts){
...
$(document).on('click', '#phone-plus-icon', function(e){
    ...
    LS.Phone.Animations.TopSlideDown(".phone-add-contact", 200, 0);
});
...
$(document).on('click', '#add-contact-save', function(e){
    ...
    if (ContactName != "" && ContactNumber != "") {
        $.post('http://ls-phone_new/AddNewContact', JSON.stringify({
        ...
        LS.Phone.Animations.TopSlideUp(".phone-add-contact", 250, -100);
        setTimeout(function(){
        ...
        if (SelectedSuggestion !== null) {
            $.post('http://ls-phone_new/RemoveSuggestion', JSON.stringify({
        ...
        LS.Phone.Notifications.Add("fas fa-exclamation-circle", "Contact Toevoegen", "Vul alle velden in!");
    }
});
...
CancelOutgoingCall = function() {
    if (LS.Phone.Data.currentApplication == "phone-call") {
        LS.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
...
$(document).on('click', '#outgoing-cancel', function(e){
    ...
    $.post('http://ls-phone_new/CancelOutgoingCall');
});

$(document).on('click', '#incoming-deny', function(e){
    e.preventDefault();

    $.post('http://ls-phone_new/DenyIncomingCall');
});

$(document).on('click', '#ongoing-cancel', function(e){
    e.preventDefault();
    
    $.post('http://ls-phone_new/CancelOngoingCall');
});

IncomingCallAlert = function(CallData, Canceled, AnonymousCall) {
    if (!Canceled) {
        if (!LS.Phone.Data.CallActive) {
            LS.Phone.Animations.TopSlideUp('.phone-application-container', 400, -160);
            LS.Phone.Animations.TopSlideUp('.'+LS.Phone.Data.currentApplication+"-app", 400, -160);
            setTimeout(function(){
                ...
                $(".phone-app").css({"display":"none"});
                LS.Phone.Functions.HeaderTextColor("white", 400);
                $("."+LS.Phone.Data.currentApplication+"-app").css({"display":"none"});
...
LS.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        CallData = cData;
...
$(document).on('click', '.phone-currentcall-container', function(e){
    ...
    LS.Phone.Functions.HeaderTextColor("white", 500);
    LS.Phone.Animations.TopSlideDown('.phone-application-container', 500, 0);
    LS.Phone.Animations.TopSlideDown('.phone-call-app', 500, 0);
    LS.Phone.Functions.ToggleApp("phone-call", "block");
...
$(document).on('click', '#incoming-answer', function(e){
    ...
    $.post('http://ls-phone_new/AnswerCall');
});

LS.Phone.Functions.AnswerCall = function(CallData) {
    $(".phone-call-incoming").css({"display":"none"});
    $(".phone-call-outgoing").css({"display":"none"});
    $(".phone-call-ongoing").css({"display":"block"});
...
LS.Phone.Functions.SetupSuggestedContacts = function(Suggested) {
