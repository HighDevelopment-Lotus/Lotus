...
$(document).on('click', '.twitter-header-tab', function(e){
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab === "twitter-mentions") {
            $.post('http://ls-phone_new/ClearMentions');
        }
...
$(document).on('click', '.twitter-new-tweet', function(e){
    ...
    LS.Phone.Animations.TopSlideDown(".twitter-new-tweet-tab", 450, 0);
});

LS.Phone.Notifications.LoadTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        $(".twitter-home-tab").html("");
        $.each(Tweets, function(i, Tweet){
            var TwtMessage = LS.Phone.Functions.FormatTwitterMessage(Tweet.message);
            var today = new Date();
...
LS.Phone.Notifications.LoadMentionedTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets.length > 0) {
...
LS.Phone.Functions.FormatTwitterMessage = function(TwitterMessage) {

    if (htmlMatch.test(TwitterMessage)) {
        fetch("http://ls-phone_new/HTMLMessage", {
            method: "post",
            body: JSON.stringify({
    ...
    var TwtMessage = TwitterMessage.replace(/<[^>]*>?/gm, '');
...
$(document).on('click', '#send-tweet', function(e){
    ...
    if (TweetMessage != "") {
        ...
        $.post('http://ls-phone_new/PostNewTweet', JSON.stringify({
            Message: TweetMessage,
            Date: CurrentDate,
            Picture: LS.Phone.Data.MetaData.profilepicture
        }), function(Tweets){
            LS.Phone.Notifications.LoadTweets(Tweets);
        });
        ...
        LS.Phone.Animations.TopSlideUp(".twitter-new-tweet-tab", 450, -120);
    } else {
        LS.Phone.Notifications.Add("fab fa-twitter", "Twitter", "Vul een bericht!", "#1DA1F2");
    }
});
...
$(document).on('click', '.hashtag-tag-text', function(e){
    ...
    if (!HashtagOpen) {
        ...
        $.post('http://ls-phone_new/GetHashtagMessages', JSON.stringify({hashtag: Hashtag}), function(HashtagData){
            LS.Phone.Notifications.LoadHashtagMessages(HashtagData.messages);
        });
...
function CopyMentionTag(elem) {
    ...
    LS.Phone.Notifications.Add("fab fa-twitter", "Twitter", $(elem).data('mentiontag')+ " gekopieerd!", "rgb(27, 149, 224)", 1250);
    document.execCommand("copy");
    $temp.remove();
}

LS.Phone.Notifications.LoadHashtags = function(hashtags) {
    if (hashtags !== null) {
        ...
        $.each(hashtags, function(i, hashtag){
            ...
            if (hashtag.messages.length >= MinimumTrending) {
                Elem = '<div class="twitter-hashtag" id="tag-'+hashtag.hashtag+'"><div class="twitter-hashtag-status">Trending in Lotus</div> <div class="twitter-hashtag-tag">#'+hashtag.hashtag+'</div> <div class="twitter-hashtag-messages">'+hashtag.messages.length+' '+TweetHandle+'</div> </div>';
            } else {
                Elem = '<div class="twitter-hashtag" id="tag-'+hashtag.hashtag+'"><div class="twitter-hashtag-status">Niet trending in Lotus</div> <div class="twitter-hashtag-tag">#'+hashtag.hashtag+'</div> <div class="twitter-hashtag-messages">'+hashtag.messages.length+' '+TweetHandle+'</div> </div>';
            }
...
LS.Phone.Notifications.LoadHashtagMessages = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
...
$(document).on('click', '.twitter-hashtag', function(event){
    ...
    LS.Phone.Notifications.LoadHashtagMessages(TweetData.messages);
