...
AddEventHandler('LSCore:Client:OnPlayerLoaded', function()
    Citizen.SetTimeout(1250, function()
        LSCore.Functions.TriggerCallback("ls-vehicleshop:server:get:config", function(config)
            Config = config
        end)
        LoggedIn = true
    end)
        ...
        if LoggedIn then
            ...
            if Distance < 5.0 and IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
                ...
                if not ShowingInteraction then
                    ...
                    exports['ls-ui']:ShowInteraction('[E] Voertuig Verkopen (50%)', 'primary')
                end
                if IsControlJustReleased(0, 38) then
                    local Vehicle = GetVehiclePedIsIn(GetPlayerPed(-1))
                    local Plate = GetVehicleNumberPlateText(Vehicle)
                    LSCore.Functions.TriggerCallback('ls-garage:server:is:vehicle:owner', function(IsOwner)
                        if IsOwner then
                            local VehicleData = LSCore.Shared.HashVehicles[GetEntityModel(Vehicle)]
                            if VehicleData ~= nil then
                                TriggerServerEvent('ls-vehicleshop:server:quick:sell', Plate, VehicleData)
...
RegisterNetEvent('ls-vehicleshop:client:buy:vehicle')
AddEventHandler('ls-vehicleshop:client:buy:vehicle', function(Nothing, Entity)
    if GetEntityModel(Entity['Entity']) == GetHashKey(Config.StoreVehicles[CurrentVehicle]['Vehicle']) then
        LSCore.Functions.TriggerCallback("ls-vehicleshop:server:can:purchase", function(CanBuy)
            if CanBuy then
                LSCore.Functions.Progressbar("lockpick-door", "Voertuig Kopen..", 5500, false, false, {
                    ...
                    TriggerServerEvent('ls-vehicleshop:server:buy:vehicle', GetEntityModel(Entity['Entity']))
        ...
        end, LSCore.Shared.HashVehicles[GetEntityModel(Entity['Entity'])]['Price'])
    else
        LSCore.Functions.Notify('Dit is niet het goeie model om te kopen..', 'error')
    end
end)

RegisterNetEvent('ls-vehicleshop:client:spawn:bought:vehicle')
AddEventHandler('ls-vehicleshop:client:spawn:bought:vehicle', function(VehicleName, Plate)
    ...
    LSCore.Functions.TriggerCallback('LSCore:server:spawn:vehicle', function(Veh)
        ...
        exports['ls-fuel']:SetFuelLevel(Vehicle, Plate, 100.0, false)
...
RegisterNetEvent('ls-vehicleshop:server:sync:config')
AddEventHandler('ls-vehicleshop:server:sync:config', function(ConfigData)
