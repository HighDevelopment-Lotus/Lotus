...
RegisterNetEvent('ls-hospital:client:use:bandage')
AddEventHandler('ls-hospital:client:use:bandage', function()
    if not exports['ls-progressbar']:GetTaskBarStatus() then
        Citizen.SetTimeout(1000, function()
            exports['ls-assets']:AddProp('HealthPack')
...
RegisterNetEvent('ls-hospital:client:use:health-pack')
AddEventHandler('ls-hospital:client:use:health-pack', function()
    if not exports['ls-progressbar']:GetTaskBarStatus() then
        Citizen.SetTimeout(1000, function()
            ...
            if Player ~= -1 and Distance < 1.5 then
              if IsTargetDead(GetPlayerServerId(Player)) then
                    TriggerEvent('ls-inventory-new:client:set:inventory:state', false)
                    exports['ls-assets']:RequestAnimationDict("mini@cpr@char_a@cpr_str")
...
RegisterNetEvent('ls-hospital:client:use:painkillers')
AddEventHandler('ls-hospital:client:use:painkillers', function()
    if not exports['ls-progressbar']:GetTaskBarStatus() then
        Citizen.SetTimeout(1000, function()
