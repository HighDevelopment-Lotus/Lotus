            ...
            for k, v in pairs(Config.Banks) do
                ...
                if Distance < 2.5 then
                    ...
                    if not ShowingInteraction then
                        ...
                        exports['ls-ui']:ShowInteraction('[F1] Bank', 'primary')
                    end
                end
            end
...
RegisterNetEvent('ls-banking:client:open:bank')
AddEventHandler('ls-banking:client:open:bank', function()
...
RegisterNetEvent('ls-banking:client:open:atm')
AddEventHandler('ls-banking:client:open:atm', function()
    Citizen.SetTimeout(450, function()
        exports['ls-assets']:RequestAnimationDict('amb@prop_human_atm@male@idle_a')
        exports['ls-assets']:RequestAnimationDict('amb@prop_human_atm@male@exit')
...
RegisterNetEvent('ls-banking:client:check:players:near')
AddEventHandler('ls-banking:client:check:players:near', function(TargetPlayer, Amount)
    local Player, Distance = LSCore.Functions.GetClosestPlayer()
    if Player ~= -1 and Distance < 3.0 then
        if GetPlayerServerId(Player) == TargetPlayer then
            exports['ls-assets']:RequestAnimationDict('friends@laf@ig_5')
            TaskPlayAnim(PlayerPedId(), 'friends@laf@ig_5', 'nephew', 5.0, 1.0, 5.0,48, 0.0, 0, 0, 0)
            TriggerServerEvent('ls-banking:server:give:cash', TargetPlayer, Amount) 
        else
...
RegisterNUICallback('GetAccounts', function(data, cb)
    LSCore.Functions.TriggerCallback("ls-bank:server:get:accounts", function(Accounts)
        cb(Accounts)
    end)  
end)

RegisterNUICallback('GetSharedAccounts', function(data, cb)
    LSCore.Functions.TriggerCallback("ls-bank:server:get:shared:accounts", function(Accounts)
        cb(Accounts)
    end)  
end)

RegisterNUICallback('GetBusinessdAccounts', function(data, cb)
    LSCore.Functions.TriggerCallback("ls-bank:server:get:business:accounts", function(Accounts)
        cb(Accounts)
    end)  
end)
...
RegisterNUICallback('WithdrawMoney', function(data, cb)
    ...
    LSCore.Functions.TriggerCallback("ls-bank:server:withdraw:money", function(IsDone)
        cb(IsDone)
    end, Data) 
end)
...
RegisterNUICallback('DepositMoney', function(data, cb)
    ...
    LSCore.Functions.TriggerCallback("ls-bank:server:deposit:money", function(IsDone)
        cb(IsDone)
    end, Data) 
end)
...
RegisterNUICallback('TransferMoney', function(data, cb)
    ...
    LSCore.Functions.TriggerCallback("ls-bank:server:transfer:money", function(IsDone)
        cb(IsDone)
    end, Data) 
end)

RegisterNUICallback('DeleteAccount', function(data, cb)
    LSCore.Functions.TriggerCallback("ls-bank:server:delete:account", function(IsDone)
        cb(IsDone)
    end, data.BankId) 
end)
...
RegisterNUICallback('CreateAccount', function(data, cb)
    ...
    LSCore.Functions.TriggerCallback("ls-bank:server:create:account", function(IsDone)
        cb(IsDone)
    end, Data) 
end)
...
RegisterNUICallback('DeletePlayer', function(data, cb)
    ...
    LSCore.Functions.TriggerCallback("ls-bank:server:delete:player", function(IsDone)
        cb(IsDone)
    end, Data) 
end)
...
RegisterNUICallback('AddPlayer', function(data, cb)
    local Data = {['BankId'] = data.BankId, ['CitizenId'] = data.CitizenId}
    LSCore.Functions.TriggerCallback("ls-bank:server:add:player", function(IsDone)
        cb(IsDone)
    end, Data) 
end)
