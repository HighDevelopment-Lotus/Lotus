        ...
        if TargetPlayer.PlayerData.source ~= SelfPlayer.PlayerData.source then
            if Amount ~= nil and Amount > 0 then
                if SelfPlayer.PlayerData.money['cash'] >= Amount then
                    TriggerClientEvent('ls-banking:client:check:players:near', SelfPlayer.PlayerData.source, TargetPlayer.PlayerData.source, math.ceil(Amount))
                else
...
RegisterNetEvent('ls-banking:server:give:cash')
AddEventHandler('ls-banking:server:give:cash', function(TargetPlayer, Amount)
    local SelfPlayer = LSCore.Functions.GetPlayer(source)
...
RegisterServerEvent('ls-banking:server:set:business:accounts')
AddEventHandler('ls-banking:server:set:business:accounts', function(Type, Amount, BankId, Reason, Source)
...
LSCore.Functions.CreateCallback("ls-bank:server:get:accounts", function(source, cb)
    local AccountTable = {}
...
LSCore.Functions.CreateCallback("ls-bank:server:get:shared:accounts", function(source, cb)
    local AccountTable = {}
    local Player = LSCore.Functions.GetPlayer(source)
...
LSCore.Functions.CreateCallback("ls-bank:server:get:business:accounts", function(source, cb)
    local AccountTable = {}
    local Player = LSCore.Functions.GetPlayer(source)
...
LSCore.Functions.CreateCallback("ls-bank:server:withdraw:money", function(source, cb, data)
    local Player = LSCore.Functions.GetPlayer(source)
...
LSCore.Functions.CreateCallback("ls-bank:server:deposit:money", function(source, cb, data)
    local Player = LSCore.Functions.GetPlayer(source)
    if data['Type'] == 'standard' then
...
LSCore.Functions.CreateCallback("ls-bank:server:delete:account", function(source, cb, BankId)
...
LSCore.Functions.CreateCallback("ls-bank:server:create:account", function(source, cb, data)
    local Player = LSCore.Functions.GetPlayer(source)
    local RandomIban = CreateRandomIban()
    LSCore.Functions.ExecuteSql(false, "INSERT INTO `player_accounts` (`citizenid`, `type`, `name`, `bankid`, `authorized`) VALUES ('"..Player.PlayerData.citizenid.."', '"..data['Type'].."', '"..data['Name'].."', '"..RandomIban.."', '[\""..Player.PlayerData.citizenid.."\"]')")
    cb(true)
end)

LSCore.Functions.CreateCallback("ls-bank:server:transfer:money", function(source, cb, data)
    local Player = LSCore.Functions.GetPlayer(source)
    if data['Type'] == 'standard' then
...
LSCore.Functions.CreateCallback("ls-bank:server:delete:player", function(source, cb, data)
...
LSCore.Functions.CreateCallback("ls-bank:server:add:player", function(source, cb, data)
    LSCore.Functions.ExecuteSql(false, "SELECT * FROM player_accounts WHERE `bankid` = '"..data['BankId'].."'", function(result)
        if result[1] ~= nil then
            local UserData = json.decode(result[1].authorized)
