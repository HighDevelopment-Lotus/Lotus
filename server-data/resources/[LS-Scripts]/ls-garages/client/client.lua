local LSCore, LoggedIn, CurrentParkingSlot, CurrentParking, TempVehicles = exports['ls-core']:GetCoreObject(), false, nil, nil, {}

RegisterNetEvent('LSCore:Client:OnPlayerLoaded')
AddEventHandler('LSCore:Client:OnPlayerLoaded', function()
    Citizen.SetTimeout(950, function()
        LSCore.Functions.TriggerCallback('ls-garages:server:get:config', function(ConfigData)
            Config = ConfigData
        end)    
       	LoggedIn = true 
    end)
...
RegisterNetEvent('ls-garages:client:try:park:vehicle')
AddEventHandler('ls-garages:client:try:park:vehicle', function(Nothing, Entity)
    local Vehicle = Entity['Entity']
    local Plate = GetVehicleNumberPlateText(Vehicle)
    LSCore.Functions.TriggerCallback("ls-garage:server:is:vehicle:owner", function(IsVehicleOwner)
        if IsVehicleOwner then
            local GarageData = {['Garage'] = Config.GarageSpots[CurrentParking]['Garage'], ['MetaData'] = {Fuel = exports['ls-fuel']:GetFuelLevel(Plate), Body = GetVehicleBodyHealth(Vehicle), Engine = GetVehicleEngineHealth(Vehicle)}}
            NetworkRequestControlOfEntity(Vehicle)
            exports['ls-autocare']:RemoveVehicleData(Plate)
            --TriggerServerEvent("ls-garage:server:save:vehicle:mods", Plate, LSCore.Functions.GetVehicleProperties(Vehicle))
            TriggerServerEvent('ls-garages:server:set:vehicle:state', Plate, 'in', GarageData)
            LSCore.Functions.DeleteVehicle(Vehicle)
        end
    end, Plate)
end)

RegisterNetEvent('ls-garages:client:try:park:house:vehicle')
AddEventHandler('ls-garages:client:try:park:house:vehicle', function(HouseName)
    local Vehicle = GetVehiclePedIsIn(GetPlayerPed(-1))
    local Plate = GetVehicleNumberPlateText(Vehicle)
    LSCore.Functions.TriggerCallback("ls-garage:server:is:vehicle:owner", function(IsVehicleOwner)
        if IsVehicleOwner then
            local GarageData = {['Garage'] = HouseName, ['MetaData'] = {Fuel = exports['ls-fuel']:GetFuelLevel(Plate), Body = GetVehicleBodyHealth(Vehicle), Engine = GetVehicleEngineHealth(Vehicle)}}
            NetworkRequestControlOfEntity(Vehicle)
...
RegisterNetEvent('ls-garage:client:open:garage:menu')
AddEventHandler('ls-garage:client:open:garage:menu', function()
    LSCore.Functions.TriggerCallback("ls-garage:server:get:my:vehicles", function(MyVehicles)
        if #MyVehicles > 0 then
            ...
            for k, v in pairs(MyVehicles) do
                if Config.GarageSpots[CurrentParking] ~= nil and Config.GarageSpots[CurrentParking]['Garage'] == v['VehicleGarage'] and v['VehicleGarageState'] == 'in' then
                    ...
                    MenuData['Data'] = {['Event'] = 'ls-garages:client:take:out', ['Type'] = 'Client', ['GType'] = 'Garage', ['Plate'] = v['VehiclePlate'], ['Model'] = v['VehicleName'], ['Motor'] = v['VehicleStatus'].Engine, ['Body'] = v['VehicleStatus'].Body, ['Fuel'] = v['VehicleStatus'].Fuel}
                    table.insert(MenuItems, MenuData)
                end
            end
            if #MenuItems > 0 then
                local Data = {['Title'] = Config.GarageSpots[CurrentParking]['Garage'], ['MainMenuItems'] = MenuItems, ['ReturnEvent'] = {['Event'] = 'ls-garages:client:remove:temp', ['Type'] = 'Client'}, ['CloseEvent'] = {['Event'] = 'ls-garages:client:remove:temp', ['Type'] = 'Client'}}
...
RegisterNetEvent('ls-garage:client:open:depot:menu')
AddEventHandler('ls-garage:client:open:depot:menu', function()
    LSCore.Functions.TriggerCallback("ls-garage:server:get:my:vehicles", function(MyVehicles)
        if #MyVehicles > 0 then
            local MenuItems = {}
            for k, v in pairs(MyVehicles) do
                if v['VehicleGarageState'] == 'out' then
                    ...
                    MenuData['Data'] = {['Event'] = 'ls-garages:client:take:out', ['Type'] = 'Client', ['GType'] = 'Depot', ['Plate'] = v['VehiclePlate'], ['Model'] = v['VehicleName'], ['Motor'] = v['VehicleStatus'].Engine, ['Body'] = v['VehicleStatus'].Body, ['Fuel'] = v['VehicleStatus'].Fuel, ['DepotPrice'] = v['VehiclePrice']}
                    table.insert(MenuItems, MenuData)
                end
            end
            if #MenuItems > 0 then
...
RegisterNetEvent('ls-garages:client:open:house:menu')
AddEventHandler('ls-garages:client:open:house:menu', function(HouseName)
    LSCore.Functions.TriggerCallback("ls-garage:server:get:house:vehicles", function(MyVehicles)
        if #MyVehicles > 0 then
            local MenuItems = {}
            for k, v in pairs(MyVehicles) do
                if v['VehicleGarageState'] == 'in' then
                    ...
                    MenuData['Image'] = v['VehicleImage']
                    MenuData['Desc'] = 'Motor: '..math.ceil(v['VehicleStatus'].Engine / 1000 * 100)..'% Body: '..math.ceil(v['VehicleStatus'].Body / 1000 * 100)..'% Fuel: '..math.ceil(v['VehicleStatus'].Fuel)..'%'
                    MenuData['Data'] = {['Event'] = 'ls-garages:client:take:out', ['Type'] = 'Client', ['GType'] = 'HouseGarage', ['Plate'] = v['VehiclePlate'], ['Model'] = v['VehicleName'], ['Motor'] = v['VehicleStatus'].Engine, ['Body'] = v['VehicleStatus'].Body, ['Fuel'] = v['VehicleStatus'].Fuel, ['HouseName'] = HouseName}
                    table.insert(MenuItems, MenuData)
                end
            end
            if #MenuItems > 0 then
...
RegisterNetEvent('ls-garages:client:open:impound:menu')
AddEventHandler('ls-garages:client:open:impound:menu', function()
    LSCore.Functions.TriggerCallback("ls-garage:server:all:vehicles", function(MyVehicles)
        if #MyVehicles > 0 then
            local MenuItems = {}
            for k, v in pairs(MyVehicles) do
                if v['VehicleGarageState'] == 'impounded' then
                    ...
                    MenuData['Data'] = {['Event'] = 'ls-garages:client:take:out', ['Type'] = 'Client', ['GType'] = 'Impound', ['Plate'] = v['VehiclePlate'], ['Model'] = v['VehicleName'], ['Motor'] = v['VehicleStatus'].Engine, ['Body'] = v['VehicleStatus'].Body, ['Fuel'] = v['VehicleStatus'].Fuel}
                    table.insert(MenuItems, MenuData)
                end
            end
            if #MenuItems > 0 then
...
RegisterNetEvent('ls-garages:client:take:out')
AddEventHandler('ls-garages:client:take:out', function(Data)
    if Data['GType'] == 'Impound' then
        ...
        if CanSpawn then
            LSCore.Functions.TriggerCallback('LSCore:server:spawn:vehicle', function(Veh)
                ...
                exports['ls-autocare']:SetupVehicleData(Data['Plate'])
                exports['ls-vehiclekeys']:SetVehicleKeys(Data['Plate'], true, false)
                exports['ls-fuel']:SetFuelLevel(Vehicle, Data['Plate'], Data['Fuel'], false)
                Citizen.SetTimeout(1000, function()
                    LSCore.Functions.TriggerCallback('ls-garages:server:get:vehicle:mods', function(Mods)
                        ...
                        TriggerServerEvent('ls-garages:server:set:vehicle:state', Data['Plate'], 'out')
                        ...
                        LSCore.Functions.TriggerCallback('LSCore:server:spawn:vehicle', function(Veh)
                            ...
                            Citizen.SetTimeout(1000, function()
                                LSCore.Functions.TriggerCallback('ls-garages:server:get:vehicle:mods', function(Mods)
                                    ...
                                    NetworkRegisterEntityAsNetworked(Vehicle)
                                    LSCore.Functions.SetVehiclePlate(Vehicle, Data['Plate'])
                                    TriggerServerEvent('ls-garages:server:set:vehicle:state', Data['Plate'], 'out', nil, VehToNet(Vehicle))
    ...
    elseif Data['GType'] == 'HouseGarage' then
            local VehicleSpawn = exports['ls-housing']:GetGarageCoords()
                ...
                LSCore.Functions.TriggerCallback('LSCore:server:spawn:vehicle', function(Veh)
                    ...
                    Citizen.SetTimeout(1000, function()
                        LSCore.Functions.TriggerCallback('ls-garages:server:get:vehicle:mods', function(Mods)
                            ...
                            exports['ls-autocare']:SetupVehicleData(Data['Plate'])
                            exports['ls-vehiclekeys']:SetVehicleKeys(Data['Plate'], true, false)
                            TriggerServerEvent('ls-garages:server:set:vehicle:state', Data['Plate'], 'out', nil, NetToVeh(Vehicle))
                        end, Data['Plate'])
                    end)
                end, Data['Model'], CoordTable, false, Data['Plate'])
            else
...
RegisterNetEvent('ls-garages:client:sync:outside:vehicles')
AddEventHandler('ls-garages:client:sync:outside:vehicles', function(Plate, ConfigData)
