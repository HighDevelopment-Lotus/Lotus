local MechanicProps = {}
local LSCore, LoggedIn, PlayerJob = exports['fw-base']:GetCoreObject(), false, {}
...
AddEventHandler('LSCore:Client:OnPlayerLoaded', function()
    Citizen.SetTimeout(1250, function()	 
        LSCore.Functions.TriggerCallback('framework-autocare:server:get:config', function(ConfigData)
           Config = ConfigData
        end)
                    ...
                    if GetPedInVehicleSeat(Vehicle, -1) == PlayerPedId() and VehicleClass ~= 13 then
                        if CurrentBody < CurrentBodyDamage or CurrentBody > CurrentBodyDamage and CurrentBody ~= CurrentBodyDamage then
                            if math.ceil(CurrentBodyDamage - CurrentBody) > 35 then
                                TriggerServerEvent('framework-autocare:server:do:damage', Plate)
                            end
                ...
                if GetPedInVehicleSeat(Vehicle, -1) == PlayerPedId() and VehicleClass ~= 13 then
                    if Config.VehiclePartData[Plate] ~= nil then
                        ...
                        if Config.VehiclePartData[Plate]['Brandstof Injectoren']['Procent'] < 10 then
                            local CurrentFuel = exports['fw-fuel']:GetFuelLevel(Plate)
                            exports['fw-fuel']:SetFuelLevel(Vehicle, Plate, math.random(5,10), false)
                            Citizen.Wait(1500)
                            exports['fw-fuel']:SetFuelLevel(Vehicle, Plate, CurrentFuel, false)
                        end
...
RegisterNetEvent('framework-autocare:client:check:vehicle')
AddEventHandler('framework-autocare:client:check:vehicle', function()
...
RegisterNetEvent('framework-autocare:client:repair:part')
AddEventHandler('framework-autocare:client:repair:part', function(data)
    if not exports['fw-progressbar']:GetTaskBarStatus() and not Config.BusyVehicles[Plate] then
        if Config.VehiclePartData[data['Plate']] ~= nil then
            if Config.VehiclePartData[data['Plate']][data['Part']]['Procent'] < 100 then
                LSCore.Functions.TriggerCallback('framework-autocare:server:has:repair:parts', function(HasParts)
                    if HasParts then
                        TriggerEvent('fw-inventory:client:set:inventory:state', false)
                        LSCore.Functions.Progressbar("repait", data['Part'].." Repareren..", 10000, false, true, {
                            ...
                            TriggerServerEvent('framework-autocare:server:set:part:data', data['Plate'], data['Part'], 100)
                            TriggerEvent('fw-inventory:client:set:inventory:state', true)
...
RegisterNetEvent('framework-autocare:client:use:part')
AddEventHandler('framework-autocare:client:use:part', function(Part, Class, ItemName)
    local Vehicle, Distance = LSCore.Functions.GetClosestVehicle()
    local Plate = GetVehicleNumberPlateText(Vehicle)
    if not exports['fw-progressbar']:GetTaskBarStatus() and not Config.BusyVehicles[Plate] then
        if Distance < 3.5 then
            if Config.VehiclePartData[Plate] ~= nil then
                if Config.VehiclePartData[Plate][Part]['Class'] ~= Class then
                    ...
                    LSCore.Functions.Progressbar("apply", Part.." Monteren..", 10000, false, true, {
                        ...
                        StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_player", 1.0)
                        TriggerServerEvent('framework-autocare:server:set:new:part', Plate, Part, Class)
                    end, function()
                       TriggerEvent('fw-inventory:client:set:inventory:state', true)
                       LSCore.Functions.Notify("Geannuleerd..", "error")
                       StopAnimTask(GetPlayerPed(-1), "mini@repair", "fixing_a_player", 1.0)
...
RegisterNetEvent('framework-autocare:client:use:repair')
AddEventHandler('framework-autocare:client:use:repair', function()
    local Vehicle, Distance = LSCore.Functions.GetClosestVehicle()
    local Plate = GetVehicleNumberPlateText(Vehicle)
    if not exports['fw-progressbar']:GetTaskBarStatus() and not Config.BusyVehicles[Plate] then
        if Distance < 4.0 then
...
RegisterNetEvent('framework-autocare:client:open:storage')
AddEventHandler('framework-autocare:client:open:storage', function(StashType)
    if exports['fw-inv']:CanOpenInventory() then
        TriggerServerEvent('framework-inv:server:open:inventory:other', StashType, 'Stash', 20, 20000000)
    end
end)

RegisterNetEvent('framework-autocare:client:open:craft')
AddEventHandler('framework-autocare:client:open:craft', function()
    if exports['fw-inv']:CanOpenInventory() then
...
RegisterNetEvent('framework-autocare:client:open:hood')
AddEventHandler('framework-autocare:client:open:hood', function()
    local Vehicle, Distance = LSCore.Functions.GetClosestVehicle()
    NetworkRequestControlOfEntity(Vehicle)
    while not NetworkHasControlOfEntity(Vehicle) do
        Wait(10)
...
RegisterNetEvent('framework-autocare:server:sync:parts:config')
AddEventHandler('framework-autocare:server:sync:parts:config', function(ConfigData)
...
function OpenVehicleServiceMenu(Plate)
    ...
    for k, v in pairs(Config.VehiclePartData[Plate]) do
      ...
      NewData['Data'] = {['Event'] = 'framework-autocare:client:repair:part', ['Type'] = 'Client', ['Plate'] = Plate, ['Class'] = v['Class'], ['Procent'] = v['Procent'], ['Part'] = k}
...
function SetupVehicleData(Plate)
    LSCore.Functions.TriggerCallback('framework-autocare:server:get:vehicle:part', function(PartResult)
        if PartResult ~= nil and PartResult ~= false then
            TriggerServerEvent('framework-autocare:server:register:vehicle', Plate, PartResult)
        end
    end, Plate)
end
...
function RemoveVehicleData(Plate)
    if Config.VehiclePartData[Plate] ~= nil then
        TriggerServerEvent('framework-autocare:server:delete:vehicle:data', Plate)
    end
end
