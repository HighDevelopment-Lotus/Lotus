...
AddEventHandler('playerDropped', function()
	...
	if carrying[oSource] then
		TriggerClientEvent("CarryPeople:cl_stop", carrying[oSource])
		carried[carrying[oSource]] = nil
		carrying[oSource] = nil
	end

	if carried[oSource] then
		TriggerClientEvent("CarryPeople:cl_stop", carried[oSource])
		carrying[carried[oSource]] = nil
		carried[oSource] = nil
	end
end)
...
RegisterServerEvent("fw-assets:carry:sync")
AddEventHandler("fw-assets:carry:sync", function(targetSrc)
	local source = source
	...
	if #(sourceCoords - targetCoords) <= 3.0 then 
		TriggerClientEvent("fw-assets:carry:syncTarget", targetSrc, source)
