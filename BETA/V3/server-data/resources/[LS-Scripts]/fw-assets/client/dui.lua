function GenerateNewDui(URL, Width, Height, DuiId)
    if Config.SavedDuiData[DuiId] == nil then
        ...
        TriggerServerEvent('framework-assets:serrver:set:dui:data', DuiId, ReturnData)
        return ReturnData
    else
        return false
    end
...
RegisterNetEvent('framework-assets:client:set:dui:url')
AddEventHandler('framework-assets:client:set:dui:url', function(DuiId, URL)
    if Config.SavedDuiData[DuiId] ~= nil then
        local Settings = Config.SavedDuiData[DuiId]
        Config.DuiLinks[DuiId] = URL
        Settings['DuiUrl'] = URL
...
RegisterNetEvent('framework-assets:client:change:dui:menu')
AddEventHandler('framework-assets:client:change:dui:menu', function(DuiId)
    ...
    LSCore.Functions.OpenInput(TxData, function(NoteData)
        if NoteData ~= false then
            TriggerServerEvent('framework-assets:server:set:dui:url', DuiId, NoteData)
        end
    end)
end)

RegisterNetEvent('framework-assets:client:set:dui:data')
AddEventHandler('framework-assets:client:set:dui:data', function(DuiId, DuiData)
