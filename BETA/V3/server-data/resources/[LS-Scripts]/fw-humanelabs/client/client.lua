...
AddEventHandler('LSCore:Client:OnPlayerLoaded', function()
    Citizen.SetTimeout(1250, function()
       LSCore.Functions.TriggerCallback("fw-humanelabs:server:get:config", function(config)
            Config = config
       end)
       LoggedIn = true
    end)
        ...
        if LoggedIn then 
            ...
            if not Config.HumanLabs['BeingRobbed'] then
                if Distance < 1.25 then 
                    ...
                    if not ShowingItems then
                        ShowingItems = true
                        NearExplosive = true
                        TriggerEvent('framwork-inv:client:set:required', RequirdItems, true)
                    end
                end
            end
            if Config.HumanLabs['BeingRobbed'] and not Config.HumanLabs['Hacked'] then
                ...
                if HDistance < 1.5 then
                    ...
                    if not ShowingItems then
                        ShowingItems = true
                        NearHack = true
                        TriggerEvent('framwork-inv:client:set:required', RequirdHackItems, true)
                    end
                end
            end
            ...
            if not NearAnything then 
                ...
                if ShowingItems then
                    ShowingItems = false
                    TriggerEvent('framwork-inv:client:set:required', RequirdItems, false)
                    TriggerEvent('framwork-inv:client:set:required', RequirdHackItems, false)
                end
...
RegisterNetEvent('fw-humanelabs:client:set:lab:robbed')
AddEventHandler('fw-humanelabs:client:set:lab:robbed', function()
    Config.HumanLabs['BeingRobbed'] = true
    Citizen.SetTimeout(15000, function()
        TriggerServerEvent('fw-doors:server:set:door:locks', 51, 1)
    end)
end)

RegisterNetEvent('fw-items:client:use:explosive')
AddEventHandler('fw-items:client:use:explosive', function()
    if NearExplosive then
        if not Config.HumanLabs['BeingRobbed'] then
            if CurrentCops >= Config.CopsNeeded then
                local Time = 0
                local PlayerCoords = GetEntityCoords(GetPlayerPed(-1))
                TriggerEvent('framwork-inv:client:set:inventory:state', false)
                ...
                TriggerEvent('framwork-inv:client:set:inventory:state', true)
                repeat
                    Time = Time + 1
                    Citizen.Wait(1000)
                until Time == 7
                ...
                TriggerServerEvent('fw-humanelabs:server:set:lab:robbed')
                StartLockdownEvent()
            else
...
RegisterNetEvent('fw-items:client:use:black-card')
AddEventHandler('fw-items:client:use:black-card', function()
    if NearHack then
        if Config.HumanLabs['BeingRobbed'] then
                ...
                if HasItem then
                    ...
                    exports['minigame-shape']:StartShapeGame(function(Outcome)
                        if Outcome then
                            ...
                            TriggerEvent('framwork-inv:client:set:inventory:state', true)
                            TriggerServerEvent('fw-humanelabs:server:end:lockdown')
                            TriggerServerEvent("fw-sound:server:play:distance", 200.0, "Lock-Down-End", 1.0)
                        else
...
AddEventHandler('fw-items:client:use:lockpick', function(IsAdvanced)
    if CurrentLocker ~= nil then
        if IsAdvanced then
            TriggerServerEvent('fw-humanelabs:server:set:locker:state', CurrentLocker, 'Busy', true)
            exports['fw-lockpick']:OpenLockpickGame(function(Success)
                if Success then
                    ...
                    TriggerServerEvent('fw-humanelabs:server:set:locker:state', CurrentLocker, 'Busy', false)
                end
            end)
        else
            LSCore.Functions.TriggerCallback('LSCore:HasItem', function(HasItem)
...
RegisterNetEvent('fw-humanelabs:server:sync:lockers')
AddEventHandler('fw-humanelabs:server:sync:lockers', function(SyncData)
    Config.HumanLabs['Lockers'] = SyncData
end)

RegisterNetEvent('fw-humanelabs:server:end:lockdown')
AddEventHandler('fw-humanelabs:server:end:lockdown', function()
...
function StartLockdownEvent()
    ...
    TriggerServerEvent("fw-sound:server:play:distance", 200.0, "Lock-Down", 1.0)
    TriggerServerEvent('framework-police:server:alert:humanelabs', GetEntityCoords(GetPlayerPed(-1)), 'Chianski Passage')
    Citizen.Wait(2500)
    TriggerServerEvent('framework-police:server:alert:humanelabs', GetEntityCoords(GetPlayerPed(-1)), 'Chianski Passage')
end
...
function LockpickLocker()
    if not IsWearingHandshoes() then
        TriggerServerEvent("framework-police:server:create:evidence", 'Finger', GetEntityCoords(GetPlayerPed(-1)))
    end
    LockPicking = true
    LockPickAnimation()
    LSCore.Functions.Progressbar("open_locker", "Lockpicken..", math.random(34000, 58000), false, true, {
        ...
        LSCore.Functions.TriggerCallback('framework-humanelabs:server:locker:reward:new:2021', function() end)
        TriggerServerEvent('fw-humanelabs:server:set:locker:state', CurrentLocker, 'IsOpen', true)
        TriggerServerEvent('fw-humanelabs:server:set:locker:state', CurrentLocker, 'Busy', false)
