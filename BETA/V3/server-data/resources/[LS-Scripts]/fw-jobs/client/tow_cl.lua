...
AddEventHandler('framework-jobmanager:client:hook:car:tow', function()
    ...
    if IsVehicleValid(TowVehicle, 'flatbed') then
        ...
        if CurrentTowVehicle == nil then
            if Distance <= 4.0 then
                if Vehicle ~= TowVehicle then
                    LSCore.Functions.Progressbar("towing-vehicle", "Voertuig optakelen", 5000, false, true, {
                        ...
                        if DoingJob then
                            RemoveBlip(JobBlip)
                            LSCore.Functions.Notify("Voertuig opgetakeld. Breng deze naar Depot op de gele parkeervak", "success")
                        end
                    end, function() -- Cancel
            ...
            LSCore.Functions.Progressbar("untowing_vehicle", "Voertuig aftakelen", 5000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
        ...
        LSCore.Functions.Notify("This is not a flatbed", "error")
    end
end)
...
function CreateTowBlip()
	...
	SetBlipRoute(TowBlip, true)
	SetBlipRouteColour(TowBlip, 5)
    SetBlipColour(TowBlip, 1)
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentSubstringPlayerName('Opdracht')
    ...
    -- ShowTempBlip(Config.TowLocations[CurrentTow]['Coords']['X'], Config.TowLocations[CurrentTow]['Coords']['Y'], Config.TowLocations[CurrentTow]['Coords']['Z'], 'Afsleep Klant')
end
