local CurrentTaxiVehicle, MeterOpen, MeterActive, DistanceCoords, SpawnedNpc, GettingIn, GettingOut, ShowingInteraction = nil, false, false, nil, false, false, false, false
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(4)
        if LoggedIn then
            if PlayerJob.name == 'taxi' and PlayerJob.onduty then
                local NearAnything = false
                local PlayerCoords = GetEntityCoords(PlayerPedId())

                for k, v in pairs(Config.Deleters['Delete']) do
                    ...
                    if Distance < 2.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                         ...
                         HasJobVehicle, DoingJob = false, false
            ...
            if NpcData['DoingNpc'] then
                ...
                if not MeterActive then
                    ...
                    if Distance < 25.0 then
                        ...
                        if Distance < 5.0 and GetEntitySpeed(GetVehiclePedIsIn(PlayerPedId())) < 5.0 and not GettingIn then
                            local Vehicle = GetVehiclePedIsIn(PlayerPedId())
                            GettingIn = true
                    ...
                    if Distance < 20.0 and GetEntitySpeed(GetVehiclePedIsIn(PlayerPedId())) < 5.0 and not GettingOut then
                        GettingOut = true
                        TaskLeaveAnyVehicle(NpcData['CurrentNpc'])
                        TriggerServerEvent('framework-jobmanager:server:add:payment', math.random(37, 150)) 
...
AddEventHandler('framework-taxi:client:openMenu', function()
        local MenuItems = {
            [1] = {['Title'] = 'Werkvoertuig 1', ['Desc'] = 'Standaard Werkvoertuig', ['Data'] = { ['Event'] = 'framework-jobmanager:client:request:vehicle:taxi', ['Type'] = 'Client', ['args'] = 'lstax'} }
        }
...
AddEventHandler('framework-jobmanager:client:reset:taxi:meter', function()
    local Vehicle = GetVehiclePedIsIn(PlayerPedId())
    if IsVehicleValid(Vehicle, 'lstax') then
        if MeterOpen then
...
AddEventHandler('framework-jobmanager:client:taxi:npc', function()
    ...
    if not NpcData['DoingNpc'] then
        if HasJobVehicle and IsVehicleValid(Vehicle, 'lstax') then
