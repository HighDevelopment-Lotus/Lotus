...
Padlock.Close = function() {
    ...
    $.post('https://fw-lockers/PadLockClose');
}
...
Keypad.Close = function(data) {
    ...
    $.post('https://fw-lockers/PadLockClose');
    if (data.error != null) {
        $.post('https://fw-lockers/CombinationFail');
    }
}
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post("https://fw-lockers/UseCombination", JSON.stringify({
        type: type,
        combination: e.value,
    }));
};
...
findCombo = function(comboArr){
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        ...
        if (angle > (comboArr[i] - numOffset) * tickAngle &&
            ...
            // on unlock
            $.post('https://fw-lockers/callcops');
            if (i == comboArr.length - 1) {
                // unlock :)
                $.post('https://fw-lockers/PadLockSuccess');
                Padlock.Close();
            }
        }
        break;
