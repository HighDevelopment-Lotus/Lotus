...
AddEventHandler('framework-lockpick:client:openLockpick', function(callback)
...
RegisterNUICallback('callback', function(data, cb)
    ...
    if data.success then
        TriggerServerEvent('framework-lockpick:server:add:skill:lockpick', math.random(1,2))
    else
        if math.random(1, 20) <= 9 then
            TriggerServerEvent('framework-lockpick:server:remove:skill:lockpick', 1)
        end
    end
    cb('ok')
end)
