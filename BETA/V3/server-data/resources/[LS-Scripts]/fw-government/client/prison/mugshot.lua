...
RegisterNetEvent('framework-prison:client:set:in:jail')
AddEventHandler('framework-prison:client:set:in:jail', function(Name, Months, CitizenId, Date)
...
function JailIntro(Name, Months, CitizenId, Date)
    ...
    TriggerEvent('framework-sound:client:play', 'handcuff', 0.4)
    TriggerEvent('framework-police:client:check:cuffed')
    RequestModel(GetHashKey("s_m_y_hwaycop_01"))
    while not HasModelLoaded(GetHashKey("s_m_y_hwaycop_01"))  do 
      Wait(1) 
    end
    Cop = CreatePed(5, GetHashKey('s_m_y_hwaycop_01'), 403.75, -1001.25, -100.00, 4.34, false)
    ...
    TriggerEvent('framework-sound:client:play', 'jail-photo', 0.4)
    Citizen.Wait(3000) 
    TriggerEvent('framework-sound:client:play', 'jail-photo', 0.4)
    Citizen.Wait(3000) 
    ...
    TriggerServerEvent("framework-ui:server:add:news:jail", Months)
    TriggerServerEvent("framework-prison:server:set:jail:items")
    TriggerEvent('framework-prison:client:enter:prison', Months, true)
...
function ResetMugshot()
 DeleteEntity(Cop)
 SetModelAsNoLongerNeeded(GetHashKey('s_m_y_hwaycop_01'))
