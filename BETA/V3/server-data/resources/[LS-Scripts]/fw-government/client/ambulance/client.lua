local BedData, BedCam, CanCheckin, CanPersonRespawn, ShowingInteraction, CurrentAmbu = nil, nil, false, false, false, 0
...
RegisterNetEvent('framework-ambulance:SetAmbuCount')
AddEventHandler('framework-ambulance:SetAmbuCount', function(Amount)
    CurrentAmbu = Amount
end)
...
AddEventHandler('framework-hospital:client:check:in', function()
    if CurrentAmbu <= 2 then
        ...
        LSCore.Functions.Notify("Roep een Ambulancier", 'error')
    end
end)
