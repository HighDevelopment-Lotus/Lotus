...
--                         if PlayerData.job.name == 'police' and PlayerData.job.onduty or PlayerData.job.name == 'ambulance' and PlayerData.job.onduty or PlayerData.job.name == 'vanilla' and PlayerData.job.onduty then
...
AddEventHandler('framework-cityhall:client:open:menu', function(MenuType)
    ...
    if (#MenuItems > 0) then
        local Data = {['Title'] = 'Lotus Stadhuis', ['MainMenuItems'] = MenuItems}
