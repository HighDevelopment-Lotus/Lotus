...
RegisterNetEvent('framework-police:client:try:toggle:disco')
AddEventHandler('framework-police:client:try:toggle:disco', function()
    if DiscoMode then
        TriggerServerEvent('framework-police:server:toggle:disco:mode', false)
    else
        TriggerServerEvent('framework-police:server:toggle:disco:mode', true)
    end
end)

RegisterNetEvent('framework-police:client:toggle:disco:mode')
AddEventHandler('framework-police:client:toggle:disco:mode', function(Bool)
    DiscoMode = Bool
    if not Bool then
        DarkModeOn = false
        SetArtificialLightsState(false)
