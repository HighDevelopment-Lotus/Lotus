...
Citizen.CreateThread(function()
    while true do 
        Citizen.Wait((1000 * 100))
        TriggerEvent('framework-police:server:update:current:cops')
        TriggerEvent('framework-ambulance:server:update:current:ambulance')
    end
end)
...
RegisterServerEvent('framework-ambulance:server:update:current:ambulance')
AddEventHandler('framework-ambulance:server:update:current:ambulance', function()
    local AmbulanceAmount = 0
    for k, v in pairs(LSCore.Functions.GetPlayers()) do
        local Player = LSCore.Functions.GetPlayer(v)
        if Player ~= nil then 
            if (Player.PlayerData.job.name == "ambulance" and Player.PlayerData.job.onduty) then
                AmbulanceAmount = AmbulanceAmount + 1
            end
        end
    end
    TriggerClientEvent("framework-ambulance:SetAmbuCount", -1, AmbulanceAmount)
end)
...
LSCore.Commands.Add("setpolice", "Neem een agent aan", {{name="id", help="Speler ID"}, {name="rang", help="Rang? 1-11"}}, true, function(source, args)
    local Player = LSCore.Functions.GetPlayer(source)
    local TargetPlayer = LSCore.Functions.GetPlayer(tonumber(args[1]))
    ...
    if Player.PlayerData.metadata['ishighcommand'] then
        if TargetPlayer ~= nil then
            ...
            TargetPlayer.Functions.SetJob('police', Rang)
        end
    end
end)
...
LSCore.Commands.Add("112r", "Beantwoord een melding", {}, false, function(source, args)
    local Player = LSCore.Functions.GetPlayer(source)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        local PlayerId = tonumber(args[1])
        table.remove(args, 1)
        ...
        local OtherPlayer = LSCore.Functions.GetPlayer(PlayerId)
        if OtherPlayer ~= nil then
            TriggerClientEvent('chatMessage', PlayerId, "Politie - "..GetPlayerName(source), "112r", Message)
            TriggerEvent("framework-logs:server:SendLog", "report", "Melding Reply", "red", "**"..GetPlayerName(source).."** beantwoord: **"..OtherPlayer.PlayerData.name.. " **(ID: "..OtherPlayer.PlayerData.source..") **Bericht:** " ..Message, false)
            for k, v in pairs(LSCore.Functions.GetPlayers()) do
                if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
                        TriggerClientEvent('chatMessage', v, "Melding Aantwoord ("..source..") - "..GetPlayerName(source), "112r", Message)
                end
            end
        else
            TriggerClientEvent('LSCore:Notify', source, "Speler niet aanwezig", "error")
        end
    else
        TriggerClientEvent('chatMessage', source, "SYSTEM", "error", "Dit command is voor hulpdiensten")
    end
...
AddEventHandler('framework-police:server:send:call:alert', function(Coords, Message)
    local Player = LSCore.Functions.GetPlayer(source)
    local Name = ''..Player.PlayerData.charinfo.firstname..' '..Player.PlayerData.charinfo.lastname ..' (ID: '.. source..')'
...
LSCore.Commands.Add("setambulance", "Neem een ambulancier aan", {{name="id", help="Speler ID"}, {name="rang", help="Rang? 1-11"}}, true, function(source, args)
    local Player = LSCore.Functions.GetPlayer(source)
    local TargetPlayer = LSCore.Functions.GetPlayer(tonumber(args[1]))
    local Rang = tonumber(args[2])
    if Player.PlayerData.metadata['ishighcommand'] and Player.PlayerData.job.name == 'ambulance' then
      	if TargetPlayer ~= nil then
      	    ...
      	    TargetPlayer.Functions.SetJob('ambulance', Rang)
      	end
    end
end)
