    ...
    setTimeout(function() {
         ...
         setTimeout(function() {
            ...
            if (ActiveAlerts == 0 || ActiveAlerts < 0) {
                ActiveAlerts = 0
                if (!HistoryOpen) {
                    $.post('http://fw-alerts/NoActiveAlerts', JSON.stringify({}))
                }
            }
        },  485);
    },  7500);
...
$(document).on('click', '.alert-marker', function(e) {
    ...
    $.post('http://fw-alerts/SetGps', JSON.stringify({Coords: AlertData['Coords']}))
});
...
document.onkeyup = function (data) {
    if (data.which == 27) { // Escape
        $.post('http://fw-alerts/CloseAlerts', JSON.stringify({}))
