...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(5)
        if LoggedIn then
            LSCore.Functions.TriggerCallback('framework-radialmenu:server:HasItem', function(HasItem)
                if HasItem then
...
RegisterNetEvent('framework-radialmenu:client:force:close')
AddEventHandler('framework-radialmenu:client:force:close', function()
    ShowMenu = false
    SetNuiFocus(false, false)
    SendNUIMessage({
        state = 'destroy'
