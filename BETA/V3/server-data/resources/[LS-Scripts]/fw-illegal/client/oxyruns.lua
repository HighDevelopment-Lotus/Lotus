...
RegisterNetEvent('framework-illegal:client:start:oxy')
AddEventHandler('framework-illegal:client:start:oxy', function()
...
RegisterNetEvent('framework-illegal:client:open:shop')
AddEventHandler('framework-illegal:client:open:shop', function()
    buyDealerStuff()
end)
...
function buyDealerStuff()
    ...
    local repItems = {}
    local runslevel = PlayerData.metadata["drugs"]
        for k, v in pairs(Config.OxyItems) do
            if runslevel >= Config.OxyItems[k].minrep then
                repItems[k] = Config.OxyItems[k]
            end
        end
        if exports['fw-inv']:CanOpenInventory() then
            local Shop = {['Type'] = 'Store', ['SubType'] = 'Dealer', ['InvName'] = 'OxyShop', ['Items'] = repItems}
...
RegisterNetEvent('framework-illegal:client:try:deliver')
AddEventHandler('framework-illegal:client:try:deliver', function()
    if not exports['fw-progressbar']:GetTaskBarStatus() then
        ...
        LSCore.Functions.Progressbar("oxy-box", "Afgeven..", 5000, false, false, {
            ...
            Delivering = false
            LSCore.Functions.TriggerCallback('framework-illegal:server:deliver:oxy', function(DidDeliver)
                if DidDeliver then
                    TriggerEvent('framework-inv:client:set:inventory:state', true)
                    if math.random(1,100) <= 13 then
                        TriggerServerEvent('framework-police:server:send:oxy:alert', GetEntityCoords(PlayerPedId()), LSCore.Functions.GetStreetLabel())
                    end
...
function StartJobAtLocation()
    ...
    Citizen.SetTimeout(math.random(10000, 17500), function()
        if OxyData['InsideZone'] then
            ...
            exports['fw-assets']:RequestModelHash(RandomPed)
            exports['fw-assets']:RequestModelHash(RandomVehicle)
...
function StartOxyRun()
    ...
    OxyData['CurrentOxyLocation'] = math.random(1, #Config.OxyLocations)
    TriggerServerEvent('framework-illegal:server:recieve:boxes', (OxyData['TimesNeeded'] * Config.DefaultOxyData['BoxAmount']))
    SetNewWaypoint(Config.OxyLocations[OxyData['CurrentOxyLocation']]['Coords'].x, Config.OxyLocations[OxyData['CurrentOxyLocation']]['Coords'].y)
end

-- function StartOxyRun()
--     DoingOxy = true
--     OxyData['DeliverBoxes'] = 2
--     OxyData['TimesNeeded'] = math.random(1,2)
--     OxyData['CurrentOxyLocation'] = math.random(1, #Config.OxyLocations)
--     TriggerServerEvent('framework-illegal:server:recieve:boxes', (OxyData['TimesNeeded'] * Config.DefaultOxyData['BoxAmount']))
--     SetNewWaypoint(Config.OxyLocations[OxyData['CurrentOxyLocation']]['Coords'].x, Config.OxyLocations[OxyData['CurrentOxyLocation']]['Coords'].y)
-- end
...
function CanDeliverOxyBox(Entity)
    local HasPackage = exports['fw-inv']:HasEnoughOfItem('oxy-box', 1)
    if HasPackage and OxyData['InsideZone'] and not Delivering then
        if LSCore.Functions.GetClosestVehicle() == OxyData['CurrentOxyVehicle'] then
            return true
        else
            return false
        end
