                ...
                if GetPedInVehicleSeat(Vehicle, -1) == PlayerPedId() then
                    if Config.NosActive[Plate] ~= nil then
                        if Config.NosActive[Plate] > 9 then
                            ...
                            TriggerServerEvent('framework-vehicles:server:remove:amount:nitrous', Plate, 10)
                            Citizen.Wait(1400)
                            if Config.NosActive[Plate] == 0 or Config.NosActive[Plate] < 0 then
                                SetVehicleBoostActive(Vehicle, false)
                                SetVehicleEnginePowerMultiplier(Vehicle, 1.0)
                                SetVehicleEngineTorqueMultiplier(Vehicle, 1.0)
                                TriggerServerEvent('framework-vehicles:server:remove:nitrous', Plate)
                                TriggerServerEvent('framework-vehicles:server:remove:flames', VehToNet(Vehicle))
                            end
                        end
                    end
                end
    ...
    while true do
        ...
        if LoggedIn then
            if IsControlJustPressed(0, 19) then
                ...
                if GetPedInVehicleSeat(Vehicle, -1) == PlayerPedId() then
                    if Config.NosActive[Plate] ~= nil then
                        TriggerServerEvent('framework-vehicles:server:set:flames', VehToNet(Vehicle))
                    end
                end
            end
...
RegisterNetEvent('framework-vehicles:client:set:flames')
AddEventHandler('framework-vehicles:client:set:flames', function(Veh)
...
RegisterNetEvent('framework-vehicles:client:remove:flames')
AddEventHandler('framework-vehicles:client:remove:flames', function(Veh)
    local Vehicle = NetToVeh(Veh)
    local Plate = GetVehicleNumberPlateText(Vehicle)
    if VehicleParticles[Plate] ~= nil then
...
RegisterNetEvent('framework-vehicles:client:use:nitrous')
AddEventHandler('framework-vehicles:client:use:nitrous', function()
    local Vehicle = GetVehiclePedIsIn(PlayerPedId())
    local Plate = GetVehicleNumberPlateText(Vehicle)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        if Config.NosActive[Plate] == nil then
            LSCore.Functions.Progressbar("use_nos", "Nitrous Aansluiten..", 1000, false, true, {
                ...
                TriggerServerEvent('framework-vehicles:server:register:nitrous', Plate)
            end)
        else
...
RegisterNetEvent('framework-vehicles:client:sync:nitrous')
AddEventHandler('framework-vehicles:client:sync:nitrous', function(NosData)
