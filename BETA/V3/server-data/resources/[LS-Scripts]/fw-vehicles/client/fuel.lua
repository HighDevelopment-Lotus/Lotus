local LoggedIn, NearPump, CurrentPump = false, false, nil

RegisterNetEvent("LSCore:Client:OnPlayerLoaded")
AddEventHandler("LSCore:Client:OnPlayerLoaded", function()
    Citizen.SetTimeout(1250, function()
        LSCore.Functions.TriggerCallback("framework-fuel:server:get:fuel:config", function(config)
            Config = config
        end)
        LoggedIn = true
    end)
                        ...
                        if Config.VehicleFuel[Plate] ~= 0 then
                            if GetPedInVehicleSeat(Vehicle, -1) == PlayerPedId() then
                                if GetEntitySpeed(Vehicle) > 3 then
                                    SetFuelLevel(Vehicle, Plate, Config.VehicleFuel[Plate] - 0.1)
                                    Citizen.Wait(12500)
                                end
                            end
                        end
...
RegisterNetEvent('framework-fuel:client:open:menu')
AddEventHandler('framework-fuel:client:open:menu', function()
    local MenuItems = {[1] = {['Title'] = 'Voertuig Tanken', ['Desc'] = 'Voertuig volledig tanken.<br>Kosten: €'..Config.TankLocations[CurrentPump]['Tank-Price']..',-', ['Data'] = {['Event'] = 'framework-fuel:client:refuel:vehicle', ['Type'] = 'Client', ['Price'] = Config.TankLocations[CurrentPump]['Tank-Price']}}}
    local Data = {['Title'] = 'Benzine Pomp', ['MainMenuItems'] = MenuItems}
    LSCore.Functions.OpenMenu(Data)
end)

RegisterNetEvent('framework-fuel:client:refuel:vehicle')
AddEventHandler('framework-fuel:client:refuel:vehicle', function(data)
    ...
    if Vehicle ~= 0 and VehDistance < 4.5 then
        if exports['fw-misc']:GetBlackoutStatus() then
            ...
            LSCore.Functions.TriggerCallback('LSCore:RemoveBank', function(DidPay)
                if DidPay then
...
RegisterNetEvent('framework-items:client:jerry_can')
AddEventHandler('framework-items:client:jerry_can', function()
	local Vehicle, Distance = LSCore.Functions.GetClosestVehicle()
	local Plate = GetVehicleNumberPlateText(Vehicle)
	local CurrentFuel = exports['fw-vehicles']:GetFuelLevel(Plate)
	if Vehicle ~= -1 and Distance < 4.5 and not IsPedInAnyVehicle(PlayerPedId()) then
		if CurrentFuel < 100 then
			...
			LSCore.Functions.Notify('Er zit al genoeg benzine in', 'error', 6500)
		end
	end
end)

RegisterNetEvent('framework-fuel:client:sync:vehicle:fuel')
AddEventHandler('framework-fuel:client:sync:vehicle:fuel', function(Plate, ConfigData)
...
function SetFuelLevel(Vehicle, Plate, Amount, Spawned)
    ...
    TriggerServerEvent('framework-fuel:server:set:fuel', Plate, Vehicle, Amount)
end
