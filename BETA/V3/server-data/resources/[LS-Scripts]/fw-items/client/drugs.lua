...
RegisterNetEvent('framework-items:client:use:joint')
AddEventHandler('framework-items:client:use:joint', function()
            ...
            Citizen.SetTimeout(1000, function()
                LSCore.Functions.Progressbar("smoke-joint", "Joint opsteken..", 2500, false, true, {
                  ...
                  disableCombat = true,
                    ...
                    TriggerEvent('framework-items:server:add:addiction:weed')
                    ...
                    TriggerEvent('framework-items:client:joint:effect')
...
RegisterNetEvent("framework-items:client:use:coke")
AddEventHandler("framework-items:client:use:coke", function()
    if not exports['fw-progressbar']:GetTaskBarStatus() then
	  	  if not DoingSomething then
	  	      DoingSomething = true
            TriggerEvent('framework-inv:client:set:inventory:state', false)
...
RegisterNetEvent("framework-items:client:use:lsd")
AddEventHandler("framework-items:client:use:lsd", function()
    if not exports['fw-progressbar']:GetTaskBarStatus() then
	  	  if not DoingSomething then
	  	      DoingSomething = true
            TriggerEvent('framework-inv:client:set:inventory:state', false)
...
RegisterNetEvent('framework-items:client:joint:effect')
AddEventHandler('framework-items:client:joint:effect', function()
  OnWeed = true
  while OnWeed do
    Citizen.Wait(1)
    if WeedTime > 0 then
      WeedTime = WeedTime - 1
      TriggerServerEvent('framework-ui:server:remove:stress', math.random(9, 15))
      Citizen.Wait(1000)
    end
    if WeedTime == 0 then
       OnWeed = false
