...
LSCore.Functions.CreateUseableItem("id_card", function(source, item)
	...
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
		for k, v in pairs(LSCore.Functions.GetPlayers()) do
			local distance = #(GetEntityCoords(GetPlayerPed(Player.PlayerData.source)) - GetEntityCoords(GetPlayerPed(v)))
			if distance < 2.0 then		
				TriggerClientEvent("framework-items:client:show:id", v, source, Player.PlayerData.citizenid, item.info)
			end
		end
...
LSCore.Functions.CreateUseableItem("driver_license", function(source, item)
	...
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        for k, v in pairs(LSCore.Functions.GetPlayers()) do
			local distance = #(GetEntityCoords(GetPlayerPed(Player.PlayerData.source)) - GetEntityCoords(GetPlayerPed(v)))
			if distance < 2.0 then		
				TriggerClientEvent("framework-items:client:show:driver", v, source, Player.PlayerData.citizenid, item.info)
			end
		end
    end
end)
