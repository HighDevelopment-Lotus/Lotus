...
LSCore, LoggedIn, Currenthouse = exports['fw-base']:GetCoreObject(), false, nil
HasKey, IsInHouse = false, false

RegisterNetEvent('LSCore:Client:OnPlayerLoaded')
AddEventHandler('LSCore:Client:OnPlayerLoaded', function()
    Citizen.SetTimeout(1250, function()
        LSCore.Functions.TriggerCallback("framework-housing:server:get:config", function(config)
           Config = config
        end)
        Citizen.Wait(145)
        AddBlipForHouse()
                ...
                if not IsInHouse then
                    if not Config.Houses[Currenthouse]['Owned'] then
                        ...
                        if Distance < 1.3 then
                            ...
                            if not ShowingInteraction then
                                ...
                                exports['fw-ui']:ShowInteraction('[E] Huis Bezichtigen', 'primary')
                            end
                            if IsControlJustReleased(0, 38) then
                                TriggerServerEvent('framework-housing:server:view:house', Currenthouse)
                            end
                        end
                    end
                    if Config.Houses[Currenthouse]['Owned'] and not Config.Houses[Currenthouse]['Door-Lock'] or HasKey then
                        ...
                        if Distance < 1.3 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                TriggerEvent('framework-housing:client:enter:house')
                            end
                        end
                    end
                            ...
                            if Distance < 3.0 then
                                ...
                                if IsControlJustReleased(0, 38) then
                                    ...
                                    if not IsPedInAnyVehicle(PlayerPedId()) then
                                        TriggerEvent('framework-garages:client:open:house:menu', Currenthouse)
                                    else
                                        LSCore.Functions.TriggerCallback('framework-garage:server:is:vehicle:owner', function(IsOwned)
                                            if IsOwned then
                                                if GetPedInVehicleSeat(Vehicle, -1) == PlayerPedId() then
                                                    print('test')
                                                    TriggerEvent('framework-garages:client:try:park:house:vehicle', Currenthouse)
                                                    LSCore.Functions.Notify("Trtrt..", "error")
                                                end
                                            else
                        ...
                        if CurrentBell ~= nil then
                            if (GetDistanceBetweenCoords(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z, Config.Houses[Currenthouse]['Coords']['Enter']['X'] - OffSets.exit.x, Config.Houses[Currenthouse]['Coords']['Enter']['Y'] - OffSets.exit.y, Config.Houses[Currenthouse]['Coords']['Enter']['Z'] - OffSets.exit.z, true) < 2.0) then
                                ...
                                if IsControlJustReleased(0, 47) then
                                    TriggerServerEvent("framework-housing:server:open:door", CurrentBell, Currenthouse)
                                    CurrentBell = nil
                                end
                            end
                        end
                            ...
                            if Distance < 1.45 then
                                ...
                                if IsControlJustReleased(0, 38) then
                                    if exports['fw-inv']:CanOpenInventory() then
                                        TriggerServerEvent('framework-inv:server:open:inventory:other', Currenthouse, 'Stash', Other.slots, Other.maxweight)
...
RegisterNetEvent('framework-housing:client:enter:house')
AddEventHandler('framework-housing:client:enter:house', function()
    local Housing = {}
    local PlayerCoords = GetEntityCoords(PlayerPedId())
    ...
    if Config.Houses[Currenthouse]['Tier'] == 15 then
        TriggerEvent("framework-sound:client:play", "hospital-elevator", 0.20)
    else
    ...
    elseif Config.Houses[Currenthouse]['Tier'] == 9 then
        Other = {maxweight = 1800000, slots = 55} -- 1200000
    ...
    elseif Config.Houses[Currenthouse]['Tier'] == 11 then
        Other = {maxweight = 1700000, slots = 55} -- 1550000
        Housing = exports['fw-interiors']:GarageTierOne(CoordsTable)
    elseif Config.Houses[Currenthouse]['Tier'] == 12 then
        Other = {maxweight = 6800000, slots = 55} -- 1950000
        Housing = exports['fw-interiors']:GarageTierTwo(CoordsTable)
    elseif Config.Houses[Currenthouse]['Tier'] == 13 then
        Other = {maxweight = 1100000, slots = 55} -- 1250000
        Housing = exports['fw-interiors']:GarageTierThree(CoordsTable)
    elseif Config.Houses[Currenthouse]['Tier'] == 14 then
        Other = {maxweight = 1250000, slots = 55} -- 1150000
        Housing = exports['fw-interiors']:SpawnWinkel(CoordsTable)
    elseif Config.Houses[Currenthouse]['Tier'] == 15 then
        Other = {maxweight = 1900000, slots = 55} -- 650000
        Housing = exports['fw-interiors']:SpawnKantoor(CoordsTable)
    elseif Config.Houses[Currenthouse]['Tier'] == 16 then
        Other = {maxweight = 1700000, slots = 55} -- 1150000
        Housing = exports['fw-interiors']:SpawnDrugs(CoordsTable)
    end
...
RegisterNetEvent('framework-housing:client:sync:config')
AddEventHandler('framework-housing:client:sync:config', function(HouseId, ConfigData)
    Config.Houses[HouseId] = ConfigData
    Currenthouse = nil
    SetClosestHouse()
end)

RegisterNetEvent('framework-housing:client:set:owned')
AddEventHandler('framework-housing:client:set:owned', function(HouseId, Owned, CitizenId)
...
RegisterNetEvent('framework-housing:client:create:house')
AddEventHandler('framework-housing:client:create:house', function(Price, Tier)
    local PlayerCoords = GetEntityCoords(PlayerPedId())
    ...
    TriggerServerEvent('framework-housing:server:add:new:house', StreetName:gsub("%-", " "), CoordsTable, Price, Tier)
end)

RegisterNetEvent('framework-housing:client:delete:house')
AddEventHandler('framework-housing:client:delete:house', function()
    if Currenthouse ~= nil then 
        TriggerServerEvent('framework-housing:server:detlete:house', Currenthouse)
    else
        LSCore.Functions.Notify("Geen huis in de buurt..", "error")
    end
end)

RegisterNetEvent('framework-housing:client:add:garage')
AddEventHandler('framework-housing:client:add:garage', function()
    if Currenthouse ~= nil then 
        local PlayerCoords = GetEntityCoords(PlayerPedId())
        local PlayerHeading = GetEntityHeading(PlayerPedId())
        local CoordsTable = {['X'] = PlayerCoords.x, ['Y'] = PlayerCoords.y, ['Z'] = PlayerCoords.z, ['H'] = PlayerHeading}
        TriggerServerEvent('framework-housing:server:add:garage', Currenthouse, Config.Houses[Currenthouse]['Adres'], CoordsTable)
    else
        LSCore.Functions.Notify("Geen huis in de buurt..", "error")
    end
end)

RegisterNetEvent('framework-housing:client:set:location')
AddEventHandler('framework-housing:client:set:location', function(Type)
    local PlayerCoords = GetEntityCoords(PlayerPedId())
    local CoordsTable = {['X'] = PlayerCoords.x, ['Y'] = PlayerCoords.y, ['Z'] = PlayerCoords.z}
    if IsInHouse then
        if HasKey then
            if Type == 'stash' then
                TriggerServerEvent('framework-housing:server:set:location', Currenthouse, CoordsTable, 'stash')
            elseif Type == 'clothes' then
                TriggerServerEvent('framework-housing:server:set:location', Currenthouse, CoordsTable, 'clothes')
            elseif Type == 'logout' then
                TriggerServerEvent('framework-housing:server:set:location', Currenthouse, CoordsTable, 'logout')
           end
        end
    end
end)

RegisterNetEvent('framework-housing:client:refresh:location')
AddEventHandler('framework-housing:client:refresh:location', function(HouseId, CoordsTable, Type)
...
RegisterNetEvent('framework-housing:client:give:keys')
AddEventHandler('framework-housing:client:give:keys', function()
    ...
    if Player ~= -1 and Distance < 1.5 then  
        LSCore.Functions.GetPlayerData(function(PlayerData)
            if Config.Houses[Currenthouse]['Owner'] == PlayerData.citizenid then
               TriggerServerEvent('framework-housing:server:give:keys', Currenthouse, GetPlayerServerId(Player))
            else
...
RegisterNetEvent('framework-housing:client:ring:door')
AddEventHandler('framework-housing:client:ring:door', function()                  
    if Currenthouse ~= nil then
        local PlayerCoords = GetEntityCoords(PlayerPedId())
        local Distance = #(PlayerCoords - vector3(Config.Houses[Currenthouse]['Coords']['Enter']['X'], Config.Houses[Currenthouse]['Coords']['Enter']['Y'], Config.Houses[Currenthouse]['Coords']['Enter']['Z']))
        if Distance < 1.2 then
            LSCore.Functions.Notify("Je hebt aangebeld!", "success")
            TriggerServerEvent('framework-housing:server:ring:door', Currenthouse)
        end
    end
end)

RegisterNetEvent('framework-housing:client:ringdoor')
AddEventHandler('framework-housing:client:ringdoor', function(Player, HouseId)
    if Currenthouse == HouseId and IsInHouse then
        CurrentBell = Player
        TriggerEvent("framework-sound:client:play", "house-doorbell", 0.1)
        LSCore.Functions.Notify("Er staat iemand aan de deur..")
    end
end)

RegisterNetEvent('framework-housing:client:set:in:house')
AddEventHandler('framework-housing:client:set:in:house', function(House)
    local PlayerCoords = GetEntityCoords(PlayerPedId())
...
RegisterNetEvent('framework-housing:client:set:new:key:holders')
AddEventHandler('framework-housing:client:set:new:key:holders', function(HouseId, HouseKeys)
    Config.Houses[HouseId]['Key-Holders'] = HouseKeys
end)

RegisterNetEvent('framework-housing:client:set:house:door')
AddEventHandler('framework-housing:client:set:house:door', function(HouseId, bool)
    Config.Houses[HouseId]['Door-Lock'] = bool
end)

RegisterNetEvent('framework-housing:client:view:house')
AddEventHandler('framework-housing:client:view:house', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
...
RegisterNetEvent('framework-housing:client:reset:house:door')
AddEventHandler('framework-housing:client:reset:house:door', function()
    if Currenthouse ~= nil then
        local PlayerCoords = GetEntityCoords(PlayerPedId())
        local Distance = #(PlayerCoords - vector3(Config.Houses[Currenthouse]['Coords']['Enter']['X'], Config.Houses[Currenthouse]['Coords']['Enter']['Y'], Config.Houses[Currenthouse]['Coords']['Enter']['Z']))
        if Distance < 2.3 then
            if not Config.Houses[Currenthouse]['Door-Lock'] then
                OpenDoorAnim()
                TriggerServerEvent('framework-sound:server:play:source', 'doorlock-keys', 0.4)
                TriggerServerEvent('framework-housing:server:set:house:door', Currenthouse, true)
            else
...
RegisterNetEvent('framework-housing:client:toggle:blips')
AddEventHandler('framework-housing:client:toggle:blips', function()
    ToggleRealesteBlips()
end)

RegisterNetEvent('framework-housing:client:breaking:door')
AddEventHandler('framework-housing:client:breaking:door', function()
    if Currenthouse ~= nil then
        local PlayerCoords = GetEntityCoords(PlayerPedId())
        local Distance = #(PlayerCoords - vector3(Config.Houses[Currenthouse]['Coords']['Enter']['X'], Config.Houses[Currenthouse]['Coords']['Enter']['Y'], Config.Houses[Currenthouse]['Coords']['Enter']['Z']))
        if Distance < 2.3 then
            if Config.Houses[Currenthouse]['Door-Lock'] then
                ...
                LSCore.Functions.Progressbar("bonk-door", "Deur Bonken..", 15000, false, true, {
                    ...
                    RamAnimation(false)
                    TriggerServerEvent('framework-housing:server:set:house:door', Currenthouse, false)
                end, function() -- Cancel
                    RamAnimation(false)
                end)
            else
...
function LogoutPlayer()
    ...
    exports['fw-interiors']:DespawnInterior(HouseData, function()
        ...
        TriggerEvent("framework-sound:client:play", "house-door-close", 0.1)
        Citizen.Wait(450)
        TriggerServerEvent('framework-housing:server:logout')
    end)
end
...
function SetClosestHouse()
    ...
    if not IsInHouse then
        ...
        if Currenthouse ~= nil then
            LSCore.Functions.TriggerCallback('framework-housing:server:has:house:key', function(HasHouseKey)
                HasKey = HasHouseKey
            end, Currenthouse)
        end
    end
...
function SetHouseLocations()
    if Currenthouse ~= nil then
        LSCore.Functions.TriggerCallback('framework-housing:server:get:locations', function(result)
            if result ~= nil then
...
RegisterNUICallback('buy', function()
    ...
    TriggerServerEvent('framework-housing:server:buy:house', Currenthouse)
end)
