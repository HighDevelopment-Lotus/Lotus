...
LSCore.Functions.CreateCallback('framework-phone/getSharedAccounts', function(source, cb)
    local bacc = {}
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
...
LSCore.Functions.CreateCallback('framework-phone:server:PayInvoice', function(source, cb, sender, amount, invoiceId)
    ...
    if Trgt ~= nil then
        if Ply.PlayerData.money.bank >= amount then
            ...
            -- TriggerEvent('framework-banking/addTransaction', src, Ply.PlayerData.charinfo.account, 'Remove', 'Betaling van een factuur', amount)
            Trgt.Functions.AddMoney('bank', amount, "paid-invoice")
            -- TriggerEvent('framework-banking/addTransaction', src, Trgt.PlayerData.charinfo.account, 'Add', 'Ontvangen betaling van een factuur', amount)
...
RegisterServerEvent('framework-phone:server:UpdateTweets')
AddEventHandler('framework-phone:server:UpdateTweets', function(TweetData)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
    table.insert(Tweets, TweetData)
    Citizen.Wait(100)
    ...
    TriggerClientEvent('framework-phone:client:UpdateTweets', -1, src, Tweets, TweetData)
end)
    ...
    LSCore.Functions.ExecuteSql(false, "SELECT * FROM `players` WHERE `charinfo` LIKE '%"..iban.."%'", function(result)
        if result[1] ~= nil then
            ...
            if recieverSteam ~= nil then
                ...
                -- TriggerEvent('framework-banking/addTransaction', src, recieverSteam.PlayerData.charinfo.account, 'Add', 'Ontvangen overschrijving', amount)
                sender.Functions.RemoveMoney('bank', amount, "phone-transfered-to-"..recieverSteam.PlayerData.citizenid)
                -- TriggerEvent('framework-banking/addTransaction', src, sender.PlayerData.charinfo.account, 'Remove', 'Geld overgemaakt naar ' .. recieverSteam.PlayerData.charinfo.account, amount)
                ...
                -- TriggerEvent('framework-banking/addTransaction', src, sender.PlayerData.charinfo.account, 'Remove', 'Mobiele overschrijving', amount)
            end
        else
...
LSCore.Functions.CreateCallback("framework-phone:server:get:my:houseskeys", function(source, cb)
    local src = source
    local TempTable = {}
    local Player = LSCore.Functions.GetPlayer(src)
