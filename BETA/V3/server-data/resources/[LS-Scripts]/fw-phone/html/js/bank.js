...
$(document).on('click', '.bank-app-account', function(e){
    ...
    LS.Phone.Notifications.Add("fas fa-university", "Bank", "Bankrekening nr. gekopieerd!", "#badc58", 1750);
});
			...
			if (amount > 0) {
                $.post('http://fw-phone/TransferMoney', JSON.stringify({
                    ...
                    if (data.CanTransfer) {
                        ...
                        LS.Phone.Notifications.Add("fas fa-university", "Bank", "Je hebt &euro; "+amount+",- overgemaakt!", "#badc58", 1500);
                    } else {
                        LS.Phone.Notifications.Add("fas fa-university", "Bank", "Je hebt niet genoeg saldo!", "#badc58", 1500);
                    }
                })
                LS.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
            } else {
                LS.Phone.Notifications.Add("fas fa-university", "Bank", "Hoeveelheid mag niet kleiner zijn dan 0!", "#badc58", 1500);
            }
        } else {
            LS.Phone.Notifications.Add("fas fa-university", "Bank", "Je hebt niet genoeg saldo!", "#badc58", 1500);
        }
    } else {
        LS.Phone.Notifications.Add("fas fa-university", "Bank", "Vul alle velden in!", "#badc58", 1750);
    }
});
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('http://fw-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                LS.Phone.Notifications.Add("fas fa-university", "Bank", "Je hebt &euro;"+InvoiceData.amount+" betaald!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
RefreshBankData = function() {
    $.post('http://fw-phone/RefreshSharedAccounts', JSON.stringify({}), function(accounts){
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        LS.Phone.Notifications.Add("fas fa-university", "Bank", "Er is geen IBAN gebonden aan dit contact!", "#badc58", 2500);
    }
    LS.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});