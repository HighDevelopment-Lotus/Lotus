...
function CleanVehicle(vehicle)
	...
	Framework.Functions.Progressbar("repair_vehicle", "Bezig met poetsen...", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	}, {}, {}, {}, function() -- Done
		Framework.Functions.Notify("Voertuig schoon!")
		SetVehicleDirtLevel(vehicle, 0.1)
        SetVehicleUndriveable(vehicle, false)
		WashDecalsFromVehicle(vehicle, 1.0)
		TriggerServerEvent('vehiclefailure:server:removewashingkit')
		TriggerEvent('inventory:client:ItemBox', Framework.Shared.Items["cleaningkit"], "remove")
...
function RepairVehicleFull(vehicle)
	...
	Framework.Functions.Progressbar("repair_vehicle", "Bezig met sleutelen..", math.random(20000, 30000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		TriggerServerEvent('kwk-vehiclefailure:removeItem', "advancedrepairkit")
	end, function() -- Cancel
		StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
		Framework.Functions.Notify("Mislukt!", "error")
		if (IsBackEngine(GetEntityModel(vehicle))) then
...
function RepairVehicle(vehicle)
	-- if GetVehicleEngineHealth(vehicle) + 150 <= 1000.0 then
		...
		Framework.Functions.Progressbar("repair_vehicle", "Bezig met sleutelen..", math.random(10000, 20000), false, true, {
			...
			TriggerServerEvent('kwk-vehiclefailure:removeItem', "repairkit")
		end, function() -- Cancel
			StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
			Framework.Functions.Notify("Mislukt!", "error")
			if (IsBackEngine(GetEntityModel(vehicle))) then
...
-- 	else
-- 	Framework.Functions.Notify("Voertuig kan niet nog meer worden gerepareerd..")
--   end 
end
...
Citizen.CreateThread(function()
    while true do
        ...
        if showPro then
            Framework.Functions.DrawText3D(pos.x, pos.y, pos.z, TimeLeft .. '~g~%')
        end
    end
end)
					...
					if cfg.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
						...
						if speed >= 1.0 then
							...
							if brake > 127 then
								...
								--exports['kwk-vehicletuning']:SetVehicleStatus(GetVehicleNumberPlateText(vehicle), "brakes", exports['kwk-vehicletuning']:GetVehicleStatus(GetVehicleNumberPlateText(vehicle), "brakes") - 0.01)
							end
...
function DamageRandomComponent()
	...
	--exports['kwk-vehicletuning']:SetVehicleStatus(GetVehicleNumberPlateText(vehicle), randomComponent, exports['kwk-vehicletuning']:GetVehicleStatus(GetVehicleNumberPlateText(vehicle), randomComponent) - randomDamage)
end
