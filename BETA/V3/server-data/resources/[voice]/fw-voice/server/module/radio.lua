...
function removePlayerFromRadio(source, radioChannel)
	...
	for player, _ in pairs(radioData[radioChannel]) do
		TriggerClientEvent('framework-voice:removePlayerFromRadio', player, source)
	end
...
function addPlayerToRadio(source, radioChannel)
	if not canJoinChannel(source, radioChannel) then
		...
		return TriggerClientEvent('framework-voice:removePlayerFromRadio', source, source)
	end
	...
	radioData[radioChannel] = radioData[radioChannel] or {}
	for player, _ in pairs(radioData[radioChannel]) do
		TriggerClientEvent('framework-voice:addPlayerToRadio', player, source)
	end
	voiceData[source] = voiceData[source] or defaultTable(source)

	voiceData[source].radio = radioChannel
	radioData[radioChannel][source] = false
	TriggerClientEvent('framework-voice:syncRadioData', source, radioData[radioChannel])
end
...
function setPlayerRadio(source, _radioChannel)
	...
	if isResource then
		...
		-- changed
		TriggerClientEvent('framework-voice:clSetPlayerRadio', source, radioChannel)
	end
...
RegisterNetEvent('framework-voice:setPlayerRadio', function(radioChannel)
	...
	if radioTbl then
		...
		for player, _ in pairs(radioTbl) do
			if player ~= source then
				TriggerClientEvent('framework-voice:setTalkingOnRadio', player, source, talking)
