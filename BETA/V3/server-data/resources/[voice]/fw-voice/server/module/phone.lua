...
function removePlayerFromCall(source, callChannel)
    ...
    for player, _ in pairs(callData[callChannel]) do
        TriggerClientEvent('framework-voice:removePlayerFromCall', player, source)
    end
...
function addPlayerToCall(source, callChannel)
    ...
    for player, _ in pairs(callData[callChannel]) do
        ...
        if player ~= source then
            TriggerClientEvent('framework-voice:addPlayerToCall', player, source)
        end
    end
    callData[callChannel][source] = false
    voiceData[source] = voiceData[source] or defaultTable(source)
    voiceData[source].call = callChannel
    TriggerClientEvent('framework-voice:syncCallData', source, callData[callChannel])
end
...
function setPlayerCall(source, _callChannel)
	...
	if isResource then
		...
		-- changed
		TriggerClientEvent('framework-voice:clSetPlayerCall', source, callChannel)
	end
...
RegisterNetEvent('framework-voice:setPlayerCall', function(callChannel)
    ...
    if callTbl then
        ...
        for player, _ in pairs(callTbl) do
            if player ~= source then
                ...
                TriggerClientEvent('framework-voice:setTalkingOnCall', player, source, talking)
            end
        end
    else
...
RegisterNetEvent('framework-voice:setTalkingOnCall', setTalkingOnCall)