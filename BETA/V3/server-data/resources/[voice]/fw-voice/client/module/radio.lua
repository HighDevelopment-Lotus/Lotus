...
RegisterNetEvent('framework-voice:syncRadioData', syncRadioData)
...
RegisterNetEvent('framework-voice:setTalkingOnRadio', setTalkingOnRadio)
...
RegisterNetEvent('framework-voice:addPlayerToRadio', addPlayerToRadio)
...
RegisterNetEvent('framework-voice:removePlayerFromRadio', removePlayerFromRadio)
...
function setRadioChannel(channel)
	...
	TriggerServerEvent('framework-voice:setPlayerRadio', channel)
...
--- check if the player is dead or has handcuffs on
...
function IsDeadOrHandCuffed()
	local PlayerData = LSCore.Functions.GetPlayerData()
	if PlayerData.metadata["ishandcuffed"] or PlayerData.metadata["isdead"] then 
		return true
	end
end
...
RegisterCommand('+radiotalk', function()
	if GetConvarInt('voice_enableRadios', 1) ~= 1 then return end
	if IsDeadOrHandCuffed() then return end

	if not radioPressed and radioEnabled then
		if radioChannel > 0 then
			logger.info('[radio] Start broadcasting, update targets and notify server.')
			playerTargets(radioData, MumbleIsPlayerTalking(PlayerId()) and callData or {})
			TriggerServerEvent('framework-voice:setTalkingOnRadio', true)
			radioPressed = true
			playMicClicks(true)
			...
			Citizen.CreateThread(function()
				TriggerEvent("framework-voice:radioActive", true)
...
RegisterCommand('-radiotalk', function()
	if radioChannel > 0 or radioEnabled and radioPressed then
		...
		playerTargets(MumbleIsPlayerTalking(PlayerId()) and callData or {})
		TriggerEvent("framework-voice:radioActive", false)
		playMicClicks(false)
		if GetConvarInt('voice_enableRadioAnim', 0) == 1 then
			StopAnimTask(PlayerPedId(), "random@arrests", "generic_radio_enter", -4.0)
		end
		TriggerServerEvent('framework-voice:setTalkingOnRadio', false)
	end
end, false)
...
RegisterNetEvent('framework-voice:clSetPlayerRadio', syncRadio)
