	...
	Citizen.CreateThread(function()
		...
		while callChannel ~= 0 do
			...
			if MumbleIsPlayerTalking(PlayerId()) and not changed then
				...
				TriggerServerEvent('framework-voice:setTalkingOnCall', true)
			elseif changed and MumbleIsPlayerTalking(PlayerId()) ~= 1 then
				changed = false
				MumbleClearVoiceTargetPlayers(voiceTarget)
				TriggerServerEvent('framework-voice:setTalkingOnCall', false)
			end
			Wait(0)
		end
	end)
...
RegisterNetEvent('framework-voice:syncCallData', function(callTable, channel)
	callData = callTable
...
RegisterNetEvent('framework-voice:setTalkingOnCall', function(tgt, enabled)
	if tgt ~= playerServerId then
...
RegisterNetEvent('framework-voice:addPlayerToCall', function(plySource)
	callData[plySource] = false
end)

RegisterNetEvent('framework-voice:removePlayerFromCall', function(plySource)
...
function setCallChannel(channel)
	...
	TriggerServerEvent('framework-voice:setPlayerCall', channel)
	callChannel = channel
...
RegisterNetEvent('framework-voice:clSetPlayerCall', function(_callChannel)
	if GetConvarInt('voice_enablePhones', 1) ~= 1 then return end
