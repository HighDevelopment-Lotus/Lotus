        ...
        if (data.active) {
            if (!RaceActive) {
                ...
                if (data.data.TotalLaps == 0) {
                    $("#race-lap").html('Ronde: Sprint');
                } else {
                    $("#race-lap").html('Ronde: ' + data.data.CurrentLap + ' / ' + data.data.TotalLaps);
                }
                $("#race-time").html('Ronde Tijd: ' + secondsTimeSpanToHMS(data.data.Time));
                if (data.data.BestLap !== 0) {
                    $("#race-besttime").html('Beste Ronde: ' + secondsTimeSpanToHMS(data.data.BestLap));
                } else {
                    $("#race-besttime").html('Beste Ronde: N/A');
                }
                $("#race-totaltime").html('Totale Tijd: ' + secondsTimeSpanToHMS(data.data.TotalTime));
            } else {
                $("#race-racename").html('Race: ' + data.data.RaceName);
                $("#race-checkpoints").html('Checkpoint: ' + data.data.CurrentCheckpoint + ' / ' + data.data.TotalCheckpoints);
                if (data.data.TotalLaps == 0) {
