...
LSCore.Functions.CreateCallback('framework-lapraces:server:GetRacingLeaderboards', function(source, cb)
    cb(Races)
end)
...
RegisterServerEvent('framework-lapraces:server:FinishPlayer')
AddEventHandler('framework-lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    local src = source
    ...
    if Races[RaceData.RaceId].Records ~= nil and next(Races[RaceData.RaceId].Records) ~= nil then
        if BLap < Races[RaceData.RaceId].Records.Time then
            ...
            TriggerClientEvent('framework-phone:client:RaceNotify', src, 'Je hebt het WR van '..RaceData.RaceName..' verbroken met een tijd van: '..SecondsToClock(BLap)..'!')
        end
    else
        Races[RaceData.RaceId].Records = {
            Time = BLap,
        ...
        LSCore.Functions.ExecuteSql(false, "UPDATE `server_lapraces` SET `records` = '"..json.encode(Races[RaceData.RaceId].Records).."' WHERE `raceid` = '"..RaceData.RaceId.."'")
        TriggerClientEvent('framework-phone:client:RaceNotify', src, 'Je hebt het WR van '..RaceData.RaceName..' neergezet met een tijd van: '..SecondsToClock(BLap)..'!')
    end
    AvailableRaces[AvailableKey].RaceData = Races[RaceData.RaceId]
    TriggerClientEvent('framework-lapraces:client:PlayerFinishs', -1, RaceData.RaceId, PlayersFinished, Player)
    ...
    TriggerClientEvent('framework-phone:client:UpdateLapraces', -1)
end)
...
RegisterServerEvent('framework-lapraces:server:CreateLapRace')
AddEventHandler('framework-lapraces:server:CreateLapRace', function(RaceName)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
    
    if IsWhitelisted(Player.PlayerData.citizenid) then
        if IsNameAvailable(RaceName) then
            TriggerClientEvent('framework-lapraces:client:StartRaceEditor', source, RaceName)
        else
...
LSCore.Functions.CreateCallback('framework-lapraces:server:GetRaces', function(source, cb)
    cb(AvailableRaces)
end)

LSCore.Functions.CreateCallback('framework-lapraces:server:GetListedRaces', function(source, cb)
    cb(Races)
end)

LSCore.Functions.CreateCallback('framework-lapraces:server:GetRacingData', function(source, cb, RaceId)
    cb(Races[RaceId])
end)

LSCore.Functions.CreateCallback('framework-lapraces:server:HasCreatedRace', function(source, cb)
    cb(HasOpenedRace(LSCore.Functions.GetPlayer(source).PlayerData.citizenid))
end)

LSCore.Functions.CreateCallback('framework-lapraces:server:IsAuthorizedToCreateRaces', function(source, cb, TrackName)
...
LSCore.Functions.CreateCallback('framework-lapraces:server:GetTrackData', function(source, cb, RaceId)
...
RegisterServerEvent('framework-lapraces:server:JoinRace')
AddEventHandler('framework-lapraces:server:JoinRace', function(RaceData)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
    ...
    if CurrentRace ~= nil then
        ...
        if (AmountOfRacers - 1) == 0 then
            ...
            TriggerClientEvent('framework-lapraces:client:LeaveRace', src, Races[CurrentRace])
        else
    ...
    TriggerClientEvent('framework-lapraces:client:JoinRace', src, Races[RaceId], RaceData.Laps)
    TriggerClientEvent('framework-phone:client:UpdateLapraces', -1)
    ...
    if creatorsource ~= Player.PlayerData.source then
        TriggerClientEvent('framework-phone:client:RaceNotify', creatorsource, string.sub(Player.PlayerData.charinfo.firstname, 1, 1)..'. '..Player.PlayerData.charinfo.lastname..' is de race gejoined!')
    end
end)

RegisterServerEvent('framework-lapraces:server:LeaveRace')
AddEventHandler('framework-lapraces:server:LeaveRace', function(RaceData)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
    local RaceName
    ...
    local creatorsource = LSCore.Functions.GetPlayerByCitizenId(AvailableRaces[AvailableKey].SetupCitizenId).PlayerData.source
    if creatorsource ~= Player.PlayerData.source then
        TriggerClientEvent('framework-phone:client:RaceNotify', creatorsource, string.sub(Player.PlayerData.charinfo.firstname, 1, 1)..'. '..Player.PlayerData.charinfo.lastname..' is de race geleaved!')
    end
    local AmountOfRacers = 0
    for k, v in pairs(Races[RaceData.RaceId].Racers) do
        AmountOfRacers = AmountOfRacers + 1
    ...
    if (AmountOfRacers - 1) == 0 then
        ...
        table.remove(AvailableRaces, AvailableKey)
        TriggerClientEvent('LSCore:Notify', src, 'Je was de enige in de race.. De race is beëindigd.', 'error')
        TriggerClientEvent('framework-lapraces:client:LeaveRace', src, Races[RaceId])
...
RegisterServerEvent('framework-lapraces:server:SetupRace')
AddEventHandler('framework-lapraces:server:SetupRace', function(RaceId, Laps)
                    ...
                    if Races[RaceId].Waiting then
                        ...
                        for cid,_ in pairs(Races[RaceId].Racers) do
                            ...
                            if RacerData ~= nil then
                                TriggerClientEvent('framework-lapraces:client:LeaveRace', RacerData.PlayerData.source, Races[RaceId])
                            end
                        end
                        table.remove(AvailableRaces, AvailableKey)
...
RegisterServerEvent('framework-lapraces:server:UpdateRaceState')
AddEventHandler('framework-lapraces:server:UpdateRaceState', function(RaceId, Started, Waiting)
    Races[RaceId].Waiting = Waiting
    Races[RaceId].Started = Started
end)

RegisterServerEvent('framework-lapraces:server:UpdateRacerData')
AddEventHandler('framework-lapraces:server:UpdateRacerData', function(RaceId, Checkpoint, Lap, Finished)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('framework-lapraces:client:UpdateRaceRacerData', -1, RaceId, Races[RaceId])
end)

RegisterServerEvent('framework-lapraces:server:StartRace')
AddEventHandler('framework-lapraces:server:StartRace', function(RaceId)
    local src = source
    ...
    if RaceId ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == MyPlayer.PlayerData.citizenid then
            ...
            for CitizenId,_ in pairs(Races[RaceId].Racers) do
                local Player = LSCore.Functions.GetPlayerByCitizenId(CitizenId)
                if Player ~= nil then
                    TriggerClientEvent('framework-lapraces:client:RaceCountdown', Player.PlayerData.source)
                end
            end
            TriggerClientEvent('framework-phone:client:UpdateLapraces', -1)
        else
...
RegisterServerEvent('framework-lapraces:server:SaveRace')
AddEventHandler('framework-lapraces:server:SaveRace', function(RaceData)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
    ...
    LSCore.Functions.InsertSql(false, "INSERT INTO `server_lapraces` (`name`, `checkpoints`, `creator`, `distance`, `raceid`) VALUES ('"..RaceData.RaceName.."', '"..json.encode(Checkpoints).."', '"..Player.PlayerData.citizenid.."', '"..RaceData.RaceDistance.."', '"..GenerateRaceId().."')")
end)
...
LSCore.Functions.CreateCallback('framework-lapraces:server:CanRaceSetup', function(source, cb)
