...
RegisterNetEvent('framework-lapraces:client:StartRaceEditor')
AddEventHandler('framework-lapraces:client:StartRaceEditor', function(RaceName)
    if not RaceData.InCreator then
        ...
        LSCore.Functions.Notify('Je bent al een race aan het maken.', 'error')
    end 
end)
        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    if CreatorData.Checkpoints ~= nil and next(CreatorData.Checkpoints) ~= nil then
                        ...
                        LSCore.Functions.Notify('Je hebt nog geen checkpoints geplaatst..', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["K"]) or IsDisabledControlJustPressed(0, Keys["K"]) then
                    if CreatorData.Checkpoints ~= nil and #CreatorData.Checkpoints >= 2 then
                        ...
                        LSCore.Functions.Notify('Je moet minimaal 10 checkpoints hebben', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["]"]) or IsDisabledControlJustPressed(0, Keys["]"]) then
                    if CreatorData.TireDistance + 1.0 ~= 16.0 then
                        ...
                        LSCore.Functions.Notify('Je kunt niet hoger dan 15')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["["]) or IsDisabledControlJustPressed(0, Keys["["]) then
                    if CreatorData.TireDistance - 1.0 ~= 1.0 then
                        ...
                        LSCore.Functions.Notify('Je kunt niet lager dan 2')
                    end
                end
            else
            ...
            if IsControlJustPressed(0, Keys["9"]) or IsDisabledControlJustPressed(0, Keys["9"]) then
                if not CreatorData.ConfirmDelete then
                    ...
                    LSCore.Functions.Notify('Druk nogmaals op [9] om te bevestigen', 'error', 5000)
                else
                    ...
                    LSCore.Functions.Notify('Race-editor geannuleerd!', 'error')
...
function SaveRace()
    ...
    TriggerServerEvent('framework-lapraces:server:SaveRace', CreatorData)

    LSCore.Functions.Notify('Race: '..CreatorData.RaceName..' is opgeslagen!', 'success')
...
function DeleteCheckpoint()
    ...
    if RaceData.ClosestCheckpoint ~= 0 then
        if CreatorData.Checkpoints[RaceData.ClosestCheckpoint] ~= nil then
            ...
            LSCore.Functions.Notify('Je kunt niet te snel gaan..', 'error')
        end
    else
        LSCore.Functions.Notify('Je kunt niet te snel gaan..', 'error')
    end
...
RegisterNetEvent('framework-lapraces:client:UpdateRaceRacerData')
AddEventHandler('framework-lapraces:client:UpdateRaceRacerData', function(RaceId, RaceData)
...
RegisterNetEvent('framework-lapraces:client:JoinRace')
AddEventHandler('framework-lapraces:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        RaceData.InRace = true
        SetupRace(Data, Laps)
        TriggerServerEvent('framework-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, false, true)
    else
        LSCore.Functions.Notify('Je zit al in een race..', 'error')
    end
end)

RegisterNetEvent('framework-lapraces:client:LeaveRace')
AddEventHandler('framework-lapraces:client:LeaveRace', function(data)
    LSCore.Functions.Notify('Je bent de race geleaved!')
    for k, v in pairs(CurrentRaceData.Checkpoints) do
...
RegisterNetEvent('framework-lapraces:client:RaceCountdown')
AddEventHandler('framework-lapraces:client:RaceCountdown', function()
    TriggerServerEvent('framework-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, true, false)
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    LSCore.Functions.Notify('De race zal van start gaan over 10 seconden', 'error', 2500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                elseif Countdown <= 5 then
                    LSCore.Functions.Notify(Countdown, 'error', 500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                end
        ...
        if CurrentRaceData.RaceName ~= nil then
            ...
            LSCore.Functions.Notify('GO!', 'success', 1000)
        ...
        LSCore.Functions.Notify('Je zit momenteel niet in een race..', 'error')
    end
end)
                ...
                if CheckpointDistance < MaxDistance then
                    if CurrentRaceData.TotalLaps == 0 then
                        if CurrentRaceData.CurrentCheckpoint + 1 < #CurrentRaceData.Checkpoints then
                            ...
                            SetNewWaypoint(CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.x, CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.y)
                            TriggerServerEvent('framework-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, false)
                            DoPilePfx()
                            ...
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            CurrentRaceData.CurrentCheckpoint = CurrentRaceData.CurrentCheckpoint + 1
                            TriggerServerEvent('framework-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, true)
                            FinishRace()
                        end
                    else
                        if CurrentRaceData.CurrentCheckpoint + 1 > #CurrentRaceData.Checkpoints then
...
function FinishRace()
    TriggerServerEvent('framework-lapraces:server:FinishPlayer', CurrentRaceData, CurrentRaceData.TotalTime, CurrentRaceData.TotalLaps, CurrentRaceData.BestLap)
    if CurrentRaceData.BestLap ~= 0 then
        LSCore.Functions.Notify('Race gefinished in '..SecondsToClock(CurrentRaceData.TotalTime)..', met als best lap: '..SecondsToClock(CurrentRaceData.BestLap))
    else
        LSCore.Functions.Notify('Race gefinished in '..SecondsToClock(CurrentRaceData.TotalTime))
    end
    for k, v in pairs(CurrentRaceData.Checkpoints) do
        if CurrentRaceData.Checkpoints[k].blip ~= nil then
            RemoveBlip(CurrentRaceData.Checkpoints[k].blip)
...
RegisterNetEvent('framework-lapraces:client:PlayerFinishs')
AddEventHandler('framework-lapraces:client:PlayerFinishs', function(RaceId, Place, FinisherData)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            LSCore.Functions.Notify(FinisherData.PlayerData.charinfo.firstname..' is gefinished op plek: '..Place, 'error', 3500)
        end
    end
end)
...
RegisterNetEvent('framework-lapraces:client:WaitingDistanceCheck')
AddEventHandler('framework-lapraces:client:WaitingDistanceCheck', function()
    Wait(1000)
    Citizen.CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            LSCore.Functions.Notify('Ga terug naar de start of je wordt gekicked over: '..ToFarCountdown..'s', 'error', 500)
                        else
                            TriggerServerEvent('framework-lapraces:server:LeaveRace', CurrentRaceData)
                            ToFarCountdown = 10
                            break
                        end
                        Citizen.Wait(1000)
