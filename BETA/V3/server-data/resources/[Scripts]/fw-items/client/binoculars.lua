        ...
        if camera then
            ...
            if not ( IsPedSittingInAnyVehicle( lPed ) ) then
                Citizen.CreateThread(function()
                    ...
                    PlayAmbientSpeech1(PlayerPedId(), "GENERIC_CURSE_MED", "SPEECH_PARAMS_FORCE")
                    TriggerEvent("framework-police:client:set:evidence", true)
                end)
            end
            ...
            SetNightvision(false)
            SetSeethrough(false)
            TriggerEvent("framework-police:client:set:evidence", false)
        end
    end
end)
...
-- Activate binoculars
RegisterNetEvent('framework-items:binoculars:toggle')
AddEventHandler('framework-items:binoculars:toggle', function()
...
RegisterNetEvent('framework-items:camera:toggle')
AddEventHandler('framework-items:camera:toggle', function()
    camera = not camera
    if not camera then
        ClearPedTasks(PlayerPedId())
    end
