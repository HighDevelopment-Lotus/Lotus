...
--     exports['fw-ui']:SetSeatbelt(false)
-- end

RegisterKeyMapping('seatbelt', 'Gordel', 'keyboard', 'X')
    ...
    if IsPedInAnyVehicle(PlayerPedId()) then
        if GetVehicleClass(GetVehiclePedIsIn(PlayerPedId())) ~= 8 and GetVehicleClass(GetVehiclePedIsIn(PlayerPedId())) ~= 13 and GetVehicleClass(GetVehiclePedIsIn(PlayerPedId())) ~= 14 then
            if SeatbeltStatus then
                TriggerEvent("framework-sound:client:play", "car-unbuckle", 0.25)
                exports['fw-ui']:SetSeatbelt(false)
                SeatbeltStatus = false
            else
                TriggerEvent("framework-sound:client:play", "car-buckle", 0.25)
                exports['fw-ui']:SetSeatbelt(true)
                SeatbeltStatus = true
            end
        end
    end
