        ...
        if LoggedIn then
            if InTrunk then
                if IsEntityDead(PlayerPedId()) then
                    ...
                    SetEntityCoords(PlayerPedId(), VehicleCoords.x, VehicleCoords.y, VehicleCoords.z + 5.0)
                    SetEntityCollision(PlayerPedId(), true, true)
                    TriggerServerEvent('framework-assets:server:set:trunk:data', Plate, false)
                    InTrunk = false
                end
                Citizen.Wait(150)
            else
...
RegisterNetEvent('framework-assets:client:sync:trunk:data')
AddEventHandler('framework-assets:client:sync:trunk:data', function(Plate, ConfigData)
    Config.TrunkData[Plate] = ConfigData
end)

RegisterNetEvent('framework-assets:client:getin:trunk')
AddEventHandler('framework-assets:client:getin:trunk', function()
    ...
    if Vehicle ~= 0 and Distance < 5.0 then
        ...
        if Config.TrunkData[Plate] == nil then
            TriggerServerEvent('framework-assets:server:setup:trunk:data', Plate)
            Citizen.Wait(150)
            if not Config.TrunkData[Plate]['Busy'] then
                if GetVehicleDoorAngleRatio(Vehicle, 5) > 0 then
                    ...
                    if ClassData['CanEnter'] then
                        ...
                        TriggerServerEvent('framework-assets:server:set:trunk:data', Plate, true)
...
RegisterNetEvent('framework-assets:client:getout:trunk')
AddEventHandler('framework-assets:client:getout:trunk', function()
    local Vehicle = GetEntityAttachedTo(PlayerPedId())
    local VehicleCoords = GetOffsetFromEntityInWorldCoords(Vehicle, 0, -5.0, 0)
    local Plate = GetVehicleNumberPlateText(Vehicle)
    if GetVehicleDoorAngleRatio(Vehicle, 5) > 0 then
