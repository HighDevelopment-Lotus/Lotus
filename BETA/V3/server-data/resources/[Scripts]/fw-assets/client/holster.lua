...
RegisterNetEvent('framework-assets:client:do:holster:anim')
AddEventHandler('framework-assets:client:do:holster:anim', function(WeaponName)
	...
	if exports['fw-weapons']:GetWeaponList(GetHashKey(WeaponName)) ~= nil then
		local PlayerRotation = GetEntityHeading(PlayerPedId())
		if WeaponHolstered then
			if IsPlayerPolice() then
				CanFire = false
				local CurrentHolster = GetPedDrawableVariation(PlayerPedId(), 7)
				...
				if GetPlayerGender() == 'Man' then
					if CurrentHolster == 153 then
						SetPedComponentVariation(PlayerPedId(), 7, 155, 0, 2)
					elseif CurrentHolster == 156 then
						SetPedComponentVariation(PlayerPedId(), 7, 154, 0, 2)
					end
				else
					if CurrentHolster == 122 then
						SetPedComponentVariation(PlayerPedId(), 7, 124, 0, 2)
					elseif CurrentHolster == 125 then
						SetPedComponentVariation(PlayerPedId(), 7, 123, 0, 2)
					end
				end
			...
			if IsPlayerPolice() then
				...
				TaskPlayAnimAdvanced(PlayerPedId(), "rcmjosh4", "josh_leadout_cop2", GetEntityCoords(PlayerPedId(), true), 0, 0, PlayerRotation, 3.0, 3.0, -1, 50, 0, 0, 0)
				Citizen.Wait(300)
				SetPedComponentVariation(PlayerPedId(), 7, PoliceHolsterData['HolsterProp'], 0, 2)
...
RegisterNetEvent('framework-assets:client:reset:holster')
AddEventHandler('framework-assets:client:reset:holster', function()
