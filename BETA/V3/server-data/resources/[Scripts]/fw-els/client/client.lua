...
AddEventHandler('LSCore:Client:OnPlayerLoaded', function()
    Citizen.SetTimeout(1250, function()
        LSCore.Functions.TriggerCallback("framework-emergencylights:server:get:config", function(ConfigData)
            Config = ConfigData
        end)
        LoggedIn = true
    end)
                    ...
                    if IsControlJustPressed(0, 36) then
                        if not Config.UiOpend then
                            ...
                            if Config.ButtonData[Plate] ~= nil then
                                ...
                                TriggerServerEvent('framework-emergencylights:server:set:sounds:disabled')
...
RegisterNetEvent('framework-emergencylights:client:set:lights')
AddEventHandler('framework-emergencylights:client:set:lights', function(Vehicle, Plate, Data)
    local Count = 0
...
RegisterNetEvent('framework-emergencylights:client:sync')
AddEventHandler('framework-emergencylights:client:sync', function(ConfigData)
    Config.ButtonData = ConfigData
end)

RegisterNetEvent('framework-emergencylights:client:update:button')
AddEventHandler('framework-emergencylights:client:update:button', function(Data, Plate)
    Config.ButtonData[Plate][Data.Type] = Data.State
end)

RegisterNetEvent('framework-emergencylights:client:toggle:sounds')
AddEventHandler('framework-emergencylights:client:toggle:sounds', function(Sender, State)
...
RegisterNetEvent('framework-emergencylights:client:set:sounds:disabled')
AddEventHandler('framework-emergencylights:client:set:sounds:disabled', function(Sender)
    local SelfPed = GetPlayerPed(GetPlayerFromServerId(Sender))
    local Vehicle = GetVehiclePedIsIn(SelfPed)
    ToggleMuteDefaultSiren(Vehicle, true)
end)
...
RegisterNUICallback('RegisterButton', function(data)
    ...
    TriggerServerEvent('framework-emergencylights:server:update:button', data, Plate)
end)

RegisterNUICallback('SetLights', function(data)
    local Vehicle = GetVehiclePedIsIn(PlayerPedId())
    TriggerServerEvent('framework-emergencylights:server:set:lights', VehToNet(Vehicle), GetVehicleNumberPlateText(Vehicle),data )
end)

RegisterNUICallback('SetSirens', function(data)
   TriggerServerEvent('framework-emergencylights:server:toggle:sounds', data.Bool)
end)

function SetupEmergencyVehicle(Vehicle, Plate)
    Config.EmergencyData[Plate] = {['SirenState'] = false, ['SoundId'] = nil}
    TriggerServerEvent('framework-emergencylights:server:setup:first:time', Plate)
    TriggerServerEvent('framework-emergencylights:server:set:sounds:disabled')
    for i = 1, 15 do
