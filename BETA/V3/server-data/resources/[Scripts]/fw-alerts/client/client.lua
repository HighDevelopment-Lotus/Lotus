        ...
        if ForBoth then
            if (PlayerData.job.name == "police" or PlayerData.job.name == "ambulance") and PlayerData.job.onduty then
                ...
                if Data['Type'] == 'Danger' then
                    TriggerServerEvent('framework-sound:server:play:distance', 5.0, "alert-panic-button", 0.5)
                elseif Data['Type'] == 'Almost' then
                    TriggerServerEvent("framework-sound:server:play:source", "alert-high-prio", 0.2)
...
RegisterNetEvent('framework-alerts:client:open:previous:alert')
AddEventHandler('framework-alerts:client:open:previous:alert', function(JobName)
