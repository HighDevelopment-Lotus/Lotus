    ...
    if CanSync then
        Citizen.SetTimeout(25, function()
            ...
            if ConfigData.SyncData['Blackout'] then
                TriggerServerEvent('pepe-blackout:server:blackout:set:config:value', 'Options', 'BlackoutActive', true)
                SetArtificialLightsState(true)
                SetArtificialLightsStateAffectsVehicles(true)
            else
                TriggerServerEvent('pepe-blackout:server:blackout:set:config:value', 'Options', 'BlackoutActive', false)
