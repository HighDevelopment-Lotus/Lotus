...
RegisterNetEvent('framework-vehiclefailure:client:fix:veh')
AddEventHandler('framework-vehiclefailure:client:fix:veh', function()
	if IsPedSittingInAnyVehicle(PlayerPedId()) then
		...
		SetVehicleFuelLevel(Vehicle, Amount + 0.0)
 		TriggerServerEvent('framework-fuel:server:update:fuel', vehiclePlate, Vehicle, math.floor(Amount))
		exports['fw-autocare']:SetItemsRepaired(vehiclePlate)
	else
	...
	LSCore.Functions.Notify("Voertuig gefixt nerd", 'success')
end)