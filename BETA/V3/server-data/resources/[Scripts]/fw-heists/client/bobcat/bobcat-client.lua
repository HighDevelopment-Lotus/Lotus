...
AddEventHandler('framework-heists:client:bobcat:try:grab:trolly', function(Nothing, Entity)
    if CurrentCops >= Config.CopsNeeded then
        ...
        if Player ~= -1 and Distance < 4.15 then
            ...
            TriggerEvent('framework-inv:client:set:inventory:state', false)
            TriggerEvent('framework-heists:client:trolly:grab', 'Money', Entity['Entity'], 'Bobcat')
...
AddEventHandler('framework-heists:client:bobcat:try:grab:crate', function(Nothing, Entity)
    if CurrentCops >= Config.CopsNeeded then
        local Player, Distance = LSCore.Functions.GetClosestPlayer()
        if Player ~= -1 and Distance < 4.15 then
            LSCore.Functions.Notify("Uhm volgensmij is er iemand anders ook in de buurt..", "error")
            ...
            TriggerEvent('framework-inv:client:set:inventory:state', false)
            TriggerEvent('framework-heists:client:trolly:grab:crate', 12000, Entity['Entity'], 'Bobcat')
