local LSCore, LoggedIn, ShowingInteraction = exports['fw-base']:GetCoreObject(), false, false
...
local ExplosiveData = exports['fw-inv']:GetItemData('explosive')
local KitData = exports['fw-inv']:GetItemData('electronickit')
local CardData = exports['fw-inv']:GetItemData('black-card')
...
--        LSCore.Functions.TriggerCallback("framework-humanelabs:server:get:config", function(config)
--             Config = config
--        end)
--        LoggedIn = true
--     end)
        ...
        if LoggedIn then 
            ...
            if not Config.HumanLabs['BeingRobbed'] then
                if Distance < 1.25 then 
                    ...
                    if not ShowingItems then
                        ShowingItems = true
                        NearExplosive = true
                        TriggerEvent('framework-inv:client:set:required', RequirdItems, true)
                    end
                end
            end
            if Config.HumanLabs['BeingRobbed'] and not Config.HumanLabs['Hacked'] then
                ...
                if HDistance < 1.5 then
                    ...
                    if not ShowingItems then
                        ShowingItems = true
                        NearHack = true
                        TriggerEvent('framework-inv:client:set:required', RequirdHackItems, true)
                    end
                end
            end
                ...
                for k, v in pairs(Config.HumanLabs['Lockers']) do 
                    ...
                    if LDistance < 1.7 and not v['Busy'] and not v['IsOpen'] then
                        ...
                        if not ShowingInteraction then
                            ...
                            exports['fw-ui']:ShowInteraction('[Lockpick] Kastje', 'primary')
                        end
                    end
                end
            end
            if not NearAnything then 
                ...
                if ShowingItems then
                    ShowingItems = false
                    TriggerEvent('framework-inv:client:set:required', RequirdItems, false)
                    TriggerEvent('framework-inv:client:set:required', RequirdHackItems, false)
                end
...
RegisterNetEvent('framework-humanelabs:client:set:lab:robbed')
AddEventHandler('framework-humanelabs:client:set:lab:robbed', function()
    Config.HumanLabs['BeingRobbed'] = true
    Citizen.SetTimeout(15000, function()
        TriggerServerEvent('framework-doors:server:set:door:locks', 51, 1)
    end)
end)
    ...
    if NearExplosive then
        if not Config.HumanLabs['BeingRobbed'] then
            if CurrentCops >= Config.CopsNeeded then
                ...
                Citizen.Wait(1000)
                TaskPlantBomb(PlayerPedId(), PlayerCoords.x, PlayerCoords.y, PlayerCoords.z, 218.5)
                ...
                TriggerServerEvent('framework-humanelabs:server:set:lab:robbed')
                StartLockdownEvent()
            else
                ...
                if HasItem then
                    ...
                    exports['minigame-shape']:StartShapeGame(function(Outcome)
                        if Outcome then
                            ...
                            TriggerEvent('framework-inv:client:set:inventory:state', true)
                            TriggerServerEvent('framework-humanelabs:server:end:lockdown')
                            TriggerServerEvent("framework-sound:server:play:distance", 200.0, "Lock-Down-End", 1.0)
                        else
...
AddEventHandler('framework-items:client:use:lockpick', function(IsAdvanced)
    if CurrentLocker ~= nil then
        print(CurrentLocker)
        if IsAdvanced then
            TriggerServerEvent('framework-humanelabs:server:set:locker:state', CurrentLocker, 'Busy', true)
            exports['fw-lockpick']:OpenLockpickGame(function(Success)
                if Success then
                    ...
                    TriggerServerEvent('framework-humanelabs:server:set:locker:state', CurrentLocker, 'IsOpen', false)
                    TriggerServerEvent('framework-humanelabs:server:set:locker:state', CurrentLocker, 'Busy', false)
                end
            end)
        else
            LSCore.Functions.TriggerCallback('LSCore:HasItem', function(HasItem)
...
RegisterNetEvent('framework-humanelabs:server:sync:lockers')
AddEventHandler('framework-humanelabs:server:sync:lockers', function(SyncData)
    Config.HumanLabs['Lockers'] = SyncData
end)

RegisterNetEvent('framework-humanelabs:server:end:lockdown')
...
function StartLockdownEvent()
    ...
    TriggerServerEvent("framework-sound:server:play:distance", 200.0, "Lock-Down", 1.0)
    TriggerServerEvent('framework-police:server:alert:humanelabs', GetEntityCoords(PlayerPedId()), 'Chianski Passage')
    Citizen.Wait(2500)
    TriggerServerEvent('framework-police:server:alert:humanelabs', GetEntityCoords(PlayerPedId()), 'Chianski Passage')
end
...
function SpawnHumaneLabsSecurity()
    ...
    exports['fw-assets']:RequestModelHash('cs_orleans')
...
function LockpickLocker()
    ...
    LSCore.Functions.Progressbar("open_locker", "Lockpicken..", math.random(34000, 58000), false, true, {
        ...
        -- TriggerServerEvent('framework-humanelabs:server:locker:reward:new:2022:new')
        LSCore.Functions.TriggerCallback('framework-humanelabs:server:locker:reward:new:2022:new', function() end)
        TriggerServerEvent('framework-humanelabs:server:set:locker:state', CurrentLocker, 'IsOpen', true)
        TriggerServerEvent('framework-humanelabs:server:set:locker:state', CurrentLocker, 'Busy', false)
...
function ElevatorToWater()
    ...
    SetEntityCoords(PlayerPedId(), Config.LabElevator['ToWater']['X'], Config.LabElevator['ToWater']['Y'], Config.LabElevator['ToWater']['Z'])
    Citizen.Wait(250)
    DoScreenFadeIn(450)
end
...
function ElevatorToLab()
    ...
    Citizen.Wait(450)
    -- TriggerEvent("framework-sound:client:play", "hospital-elevator", 0.25)
    SetEntityCoords(PlayerPedId(), Config.LabElevator['ToLab']['X'], Config.LabElevator['ToLab']['Y'], Config.LabElevator['ToLab']['Z'])
    Citizen.Wait(250)
    DoScreenFadeIn(450)
end
