...
RegisterNetEvent("framework-housing:client:decorate")
AddEventHandler("framework-housing:client:decorate", function()
	Citizen.Wait(500)
	if IsInHouse then 
		if HasKey then 
...
RegisterNetEvent("framework-housing:server:sethousedecorations")
AddEventHandler("framework-housing:server:sethousedecorations", function(house, decorations)
...
function LoadDecorations(house)
	if Config.Houses[house] ~= nil then
		if Config.Houses[house]['Decorations'] == nil or next(Config.Houses[house]['Decorations']) == nil then
			LSCore.Functions.TriggerCallback('framework-housing:server:get:decorations', function(result)
...
function SaveDecorations()
	if Currenthouse ~= nil then
		...
		TriggerServerEvent("framework-housing:server:save:decorations", Currenthouse, ObjectList)
	end
end
