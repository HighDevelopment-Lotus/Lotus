        ...
        if (CanOpenApp) {
            if (QB.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "settings") {
                    ...
                    $.post('https://fw-phone/GetMentionedTweets', JSON.stringify({}), function(MentionedTweets){
                        QB.Phone.Notifications.LoadMentionedTweets(MentionedTweets)
                    })
                    $.post('https://fw-phone/GetHashtags', JSON.stringify({}), function(Hashtags){
                        QB.Phone.Notifications.LoadHashtags(Hashtags)
                    })
                    if (QB.Phone.Data.IsOpen) {
                        $.post('https://fw-phone/GetTweets', JSON.stringify({}), function(Tweets){
                    ...
                    $.post('https://fw-phone/GetBankContacts', JSON.stringify({}), function(contacts){
                        QB.Phone.Functions.LoadContactsWithNumber(contacts);
                    });
                    $.post('https://fw-phone/GetInvoices', JSON.stringify({}), function(invoices){
                        QB.Phone.Functions.LoadBankInvoices(invoices);
                    });
                } else if (PressedApplication == "whatsapp") {
                    $.post('https://fw-phone/GetWhatsappChats', JSON.stringify({}), function(chats){
                        QB.Phone.Functions.LoadWhatsappChats(chats);
                    });
                } else if (PressedApplication == "phone") {
                    $.post('https://fw-phone/GetMissedCalls', JSON.stringify({}), function(recent){
                        QB.Phone.Functions.SetupRecentCalls(recent);
                    });
                    $.post('https://fw-phone/GetSuggestedContacts', JSON.stringify({}), function(suggested){
                        QB.Phone.Functions.SetupSuggestedContacts(suggested);
                    });
                    $.post('https://fw-phone/ClearGeneralAlerts', JSON.stringify({
                        app: "phone"
                    }));
                } else if (PressedApplication == "mail") {
                    $.post('https://fw-phone/GetMails', JSON.stringify({}), function(mails){
                    ...
                    $.post('https://fw-phone/LoadAdverts', JSON.stringify({}), function(Adverts){
                        QB.Phone.Functions.RefreshAdverts(Adverts);
                    })
                } else if (PressedApplication == "garage") {
                    $.post('https://fw-phone/SetupGarageVehicles', JSON.stringify({}), function(Vehicles){
                        SetupGarageVehicles(Vehicles);
                    })
                } else if (PressedApplication == "crypto") {
                    $.post('https://fw-phone/GetCryptoData', JSON.stringify({
                        crypto: "qbit",
                    }), function(CryptoData){
                        SetupCryptoData(CryptoData);
                    })

                    $.post('https://fw-phone/GetCryptoTransactions', JSON.stringify({}), function(data){
                        RefreshCryptoTransactions(data);
                    })
                } else if (PressedApplication == "racing") {
                    $.post('https://fw-phone/GetAvailableRaces', JSON.stringify({}), function(Races){
                        SetupRaces(Races);
                    });
                } else if (PressedApplication == "houses") {
                    $.post('https://fw-phone/GetPlayerHouses', JSON.stringify({}), function(Houses){
                        SetupPlayerHouses(Houses);
                    });
                    $.post('https://fw-phone/GetPlayerKeys', JSON.stringify({}), function(Keys){
                    ...
                    $.post('https://fw-phone/GetCurrentLawyers', JSON.stringify({}), function(data){
                        SetupLawyers(data);
                    });
                } else if (PressedApplication == "store") {
                    $.post('https://fw-phone/SetupStoreApps', JSON.stringify({}), function(data){
                        SetupAppstore(data);
                    });
                } else if (PressedApplication == "trucker") {
                    $.post('https://fw-phone/GetTruckerData', JSON.stringify({}), function(data){
                        SetupTruckerInfo(data);
                    });
                }
                else if (PressedApplication == "gallery") {
                    $.post('https://fw-phone/GetGalleryData', JSON.stringify({}), function(data){
...
$(document).on('click', '.mykeys-key', function(e){
    ...
    $.post('https://fw-phone/SetHouseLocation', JSON.stringify({
        HouseData: KeyData
    }))
});
...
$(document).on('click', '.phone-take-camera-button', function(event){
    ...
    $.post('https://fw-phone/TakePhoto', JSON.stringify({}),function(url){
    })
...
$(document).on('click', '.phone-silent-button', function(event){
    event.preventDefault();
    $.post('https://fw-phone/phone-silent-button', JSON.stringify({}),function(Data){
        if(Data){
...
QB.Phone.Notifications.Add = function(icon, title, text, color, timeout) {
    $.post('https://fw-phone/HasPhone', JSON.stringify({}), function(HasPhone){
        if (HasPhone) {
