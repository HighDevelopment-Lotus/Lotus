...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('https://fw-phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('https://fw-phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
QB.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(QB.Phone.Data.PlayerData.charinfo.firstname+"."+QB.Phone.Data.PlayerData.charinfo.lastname+"@quackcity.nl");
    $("#mail-header-lastsync").html("Laatst gesychroniseerd "+MessageTime);
    if (Mails !== null && Mails !== undefined) {
        if (Mails.length > 0) {
            ...
            $(".mail-list").html('<p class="nomails">Het is nog vrij leeg</p>');
        }
...
$(document).on('click', '#advert-sendmessage-chat', function(e){
    ...
    if (Advert !== "") {
        ...
        if (!picture){
            $.post('https://fw-phone/PostAdvert', JSON.stringify({
                message: Advert,
                url: null
            }));
            ClearInputNew()
        ...
        QB.Phone.Notifications.Add("fas fa-ad", "Advertisement", "Je kunt geen lege advertentie plaatsen", "#ff8f1a", 2000);
    }
});
...
$(document).on('click','#new-advert-photo',function(e){
    e.preventDefault();
    $.post('https://fw-phone/TakePhoto',function(url){
        if(url){
...
QB.Phone.Functions.RefreshAdverts = function(Adverts) {
    if (Adverts.length > 0 || Adverts.length == undefined) {
        ...
        $(".advert-list").html("");
        var element = '<div class="advert"><span class="advert-sender">Nog niets te vinden hier</span></div>';
        $(".advert-list").append(element);
    }
}
...
$(document).on('click','#adv-delete',function(e){
    e.preventDefault();
    $.post('https://fw-phone/DeleteAdvert', function(){
        setTimeout(function(){
            QB.Phone.Notifications.Add("fas fa-ad", "Advertisement", "Ad is verwijderd", "#ff8f1a", 2000);
        },400)
    });
})