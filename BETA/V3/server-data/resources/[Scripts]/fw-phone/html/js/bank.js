...
$(document).on('click', '#accept-transfer', function(e){
    ...
    if (iban != "" && amount != "") {
            $.post('https://fw-phone/CanTransferMoney', JSON.stringify({
                sendTo: iban,
                amountOf: amount,
            }), function(data){
...
$(document).on('click', '.pay-invoice', function(event){
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('https://fw-phone/PayInvoice', JSON.stringify({
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    $.post('https://fw-phone/DeclineInvoice', JSON.stringify({
        sender: InvoiceData.sender,
        amount: InvoiceData.amount,
        society: InvoiceData.society,
        invoiceId: InvoiceData.id,
...
QB.Phone.Functions.SetupSharedAccounts = function(accounts) {
    $("#sharedAccounts").html("");
...
RefreshBankData = function() {
    $.post('https://fw-phone/RefreshSharedAccounts', JSON.stringify({}), function(accounts){
