SetupLawyers = function(data) {
    ...
    if (data.length > 0) {
        ...
        if (lawyers.length > 0) {
            ...
            var element = '<div class="lawyer-list"><div class="no-lawyers">Er zijn geen advocaten in dienst momenteel</div></div>'
            $(".lawyers-list").append(element);
        }
        ...
        if (realestate.length > 0) {
            ...
            var element = '<div class="lawyer-list"><div class="no-lawyers">Er zijn geen makelaren in dienst momenteel</div></div>'
            $(".lawyers-list").append(element);
        }

        $(".lawyers-list").append('<br><h1 style="font-size:1.641025641025641vh; padding:1.0256410256410255vh; color:#fff; margin-top:0; width:100%; display:block; background-color: rgb(0, 204, 102);">Low Santos (' + mechanic.length + ')</h1>');
        ...
        if (mechanic.length > 0) {
            ...
            var element = '<div class="lawyer-list"><div class="no-lawyers">Er zijn geen medewerkers in dienst momenteel</div></div>'
            $(".lawyers-list").append(element);
        }
        ...
        if (taxi.length > 0) {
            ...
            var element = '<div class="lawyer-list"><div class="no-lawyers">Er is geen taxi in dienst momenteel</div></div>'
            $(".lawyers-list").append(element);
        }
        ...
        if (ambulance.length > 0) {
            ...
            var element = '<div class="lawyer-list"><div class="no-lawyers">Er zijn geen ambulanciers in dienst momenteel.</div></div>'
            $(".lawyers-list").append(element);
        }
    } else {
        $(".lawyers-list").append('<h1 style="font-size:1.641025641025641vh; padding:1.0256410256410255vh; color:#fff; margin-top:0; border-top-left-radius: .5vh; border-top-right-radius: .5vh; width:100%; display:block; background-color: rgb(42, 137, 214);">Advocaten (' + lawyers.length + ')</h1>');
...
$(document).on('click', '.lawyer-list-call', function(e){
    ...
    $.post('https://fw-phone/CallContact', JSON.stringify({
        ContactData: cData,
            ...
            if (status.IsOnline) {
                if (status.CanCall) {
                    if (!status.InCall) {
                        ...
                        QB.Phone.Notifications.Add("fas fa-phone", "Telefoon", "Je zit nog in een oproep");
                    }
                } else {
                    QB.Phone.Notifications.Add("fas fa-phone", "Telefoon", "Deze persoon is in gesprek");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-phone", "Telefoon", "Persoon is niet beschikbaar");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-phone", "Telefoon", "Je kan jezelf niet bellen gekkie");
        }
    });
});
