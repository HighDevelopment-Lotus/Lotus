...
$(document).on('click', '#documents-save-note-for-doc', function(e){
    ...
    if ((Title &&Text ) != ""){
        $.post('https://fw-phone/documents_Save_Note_As', JSON.stringify({
            Title: Title,
            Text: Text,
            Time: Times,
            Type: "New",
...
function LoadGetNotes(){
    ...
    $(".documents-select span").html("<span>Selecteer</span>");

    var AddOption = '<div class="casino-text-clear">Nog geen notities</div>'+
                    '<div class="casino-text-clear" style="font-size: 500%;color: #0d1218c0;"><i class="fas fa-frown"></i></div>'
    $('.documents-list').append(AddOption);

    $.post('https://fw-phone/GetNote_for_Documents_app', JSON.stringify({}), function(HasNote){
        if(HasNote){
