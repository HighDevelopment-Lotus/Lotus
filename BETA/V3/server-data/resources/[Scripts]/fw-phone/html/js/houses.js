...
$(document).on('click', '.mykeys', function(e){
    e.preventDefault();
    var HouseData = $(this).data('HouseData');
    $.post('http://fw-phone/setwaypoint', JSON.stringify({CoordsX: HouseData['Coords']['X'], CoordsY: HouseData['Coords']['Y']}));
});
    ...
    if (CurrentHouseData['Keyholders'] !== undefined && CurrentHouseData['Keyholders'] !== null) {
        ...
        $.each(CurrentHouseData['Keyholders'], function(i, keyholder){
            if (keyholder !== null && keyholder !== undefined) {
                var elem;
                if (keyholder['Name'] != QB.Phone.Data.PlayerData.charinfo.firstname+' '+QB.Phone.Data.PlayerData.charinfo.lastname) {
...
$(document).on('click', '.house-key-delete', function(e){
    ...
    $.post('http://fw-phone/GetPlayerKeys', JSON.stringify({}), function(Houses){
...
$(document).on('click', '#myhouse-option-transfer-confirm', function(e){
    ...
    $.post('http://fw-phone/ChangeOwner', JSON.stringify({newCid: newCitizenId, HouseData: CurrentHouseData['House']}), function(Transferd){
        if (Transferd) {
            $(".myhouses-options").animate({
                left: 0
            }, AnimationDuration);
