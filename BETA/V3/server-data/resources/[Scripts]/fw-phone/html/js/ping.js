$(document).on('click', '#ping-send', function(e){
    ...
    if (IDPlayer >= 1){
        $.post('https://fw-phone/SendPingPlayer', JSON.stringify({
            id: IDPlayer
        }));
        $("#channel").val("");
    }
...
$(document).on('click', '#ping-accept', function(e){
    e.preventDefault();
    $.post('https://fw-phone/AcceptPingPlayer', JSON.stringify({}));
});

$(document).on('click', '#ping-reject', function(e){
    e.preventDefault();
    $.post('https://fw-phone/rejectPingPlayer', JSON.stringify({}));
});
