...
function LoadEmploymentApp(){
    $.post('https://fw-phone/GetGroupsApp', JSON.stringify({}), function(Data){
        AddDIV(Data)
    });
}
...
$(document).on('click', '#employment-sbmit-for-create-group', function(e){
    ...
    if (Name != "" && pass != "" && pass2 != ""){
        if(pass == pass2){
            $.post('https://fw-phone/employment_CreateJobGroup', JSON.stringify({
                name: Name,
                pass: pass,
            }));
            ...
            QB.Phone.Notifications.Add("fas fa-exclamation-circle", "System", "Het wachtwoord is niet correct")
        }
    }else{
        QB.Phone.Notifications.Add("fas fa-exclamation-circle", "System", "Velden zijn niet correct")
    }
});
...
$(document).on('click', '#employment-delete-group', function(e){
    ...
    $.post('https://fw-phone/employment_DeleteGroup', JSON.stringify({
        delete: Delete,
    }));
});
...
$(document).on('click', '#employment-sbmit-for-join-group', function(e){
    ...
    if(EnterPass == JoinPass){
        var CSN = QB.Phone.Data.PlayerData.citizenid;
        $.post('https://fw-phone/employment_JoinTheGroup', JSON.stringify({
            PCSN: CSN,
            id: JoinID,
        }));
        ClearInputNew()
...
$(document).on('click', '#employment-list-group', function(e){
    e.preventDefault();
    var id = $(this).data('id')
    $.post('https://fw-phone/employment_CheckPlayerNames', JSON.stringify({
        id: id,
        }), function(Data){
           ClearInputNew()
...
$(document).on('click', '#employment-leave-grouped', function(e){
    ...
    var CSN = QB.Phone.Data.PlayerData.citizenid;
    var id = $(this).data('id')
    $.post('https://fw-phone/employment_leave_grouped', JSON.stringify({
        id: id,
        csn: CSN,
    }));
});