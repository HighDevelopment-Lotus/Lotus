...
local function findVehFromPlateAndLocate(plate)
    local gameVehicles = LSCore.Functions.GetVehicles()
    for i = 1, #gameVehicles do
        local vehicle = gameVehicles[i]
        if DoesEntityExist(vehicle) then
            if LSCore.Functions.GetPlate(vehicle) == plate then
...
--         -1559354806
--     }
--         exports["framework-target"]:AddTargetModel(PublicPhoneobject, {
--             options = {
--                 {
--                     type = "client",
...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen then
        if not PlayerData.metadata['ishandcuffed'] and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] and not IsPauseMenuActive() then
            ...
            LSCore.Functions.Notify("Action not available at the moment..", "error")
        end
    end
end)
...
RegisterNUICallback('PostAdvert', function(data)
    TriggerServerEvent('framework-phone:server:AddAdvert', data.message, data.url)
end)

RegisterNUICallback("DeleteAdvert", function()
    TriggerServerEvent("framework-phone:server:DeleteAdvert")
end)
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    TriggerServerEvent('framework-phone:server:BillingEmail', data, true)
end)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    TriggerServerEvent('framework-phone:server:BillingEmail', data, false)
end)
...
RegisterNUICallback('PostNewTweet', function(data, cb)
    ...
    TriggerServerEvent('framework-phone:server:UpdateTweets', PhoneData.Tweets, TweetMessage)
end)

RegisterNUICallback('DeleteTweet',function(data)
    TriggerServerEvent('framework-phone:server:DeleteTweet', data.id)
end)
...
RegisterNUICallback('InstallApplication', function(data, cb)
    ...
    if NewSlot <= Config.MaxSlots then
        TriggerServerEvent('framework-phone:server:InstallApplication', {
            app = data.app,
        })
        cb({
            app = data.app,
...
RegisterNUICallback('RemoveApplication', function(data, cb)
    TriggerServerEvent('framework-phone:server:RemoveInstallation', data.app)
end)

-- RegisterNUICallback('GetTruckerData', function(data, cb)
--     local TruckerMeta = LSCore.Functions.GetPlayerData().metadata["jobrep"]["trucker"]
--     local TierData = exports['fw-trucker']:GetTier(TruckerMeta)
--     cb(TierData)
-- end)
...
RegisterNUICallback('DeleteImage', function(image,cb)
    TriggerServerEvent('framework-phone:server:RemoveImageFromGallery',image)
    Wait(400)
    TriggerServerEvent('framework-phone:server:getImageFromGallery')
    cb(true)
end)
...
if findVehFromPlateAndLocate(veh.plate) then
    LSCore.Functions.Notify("Je voertuig staat aangegeven op de kaart", "success")
else
    LSCore.Functions.Notify("Dit voertuig kon niet worden gevonden kijk of hij in Depot staat", "error")
end
end)
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    LSCore.Functions.Notify('GPS gezet: '..data.alert.title)
    ...
    LSCore.Functions.TriggerCallback('framework-phone:server:GetVehicleSearchResults', function(result)
        if result ~= nil then
            for k, v in pairs(result) do
                LSCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
...
RegisterNUICallback('FetchVehicleScan', function(data, cb)
    ...
    local plate = LSCore.Functions.GetPlate(vehicle)
    local vehname = GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)):lower()
    LSCore.Functions.TriggerCallback('framework-phone:server:ScanPlate', function(result)
        -- LSCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
            result.isFlagged = false
	    if LSCore.Shared.Vehicles[vehname] ~= nil then
                result.label = LSCore.Shared.Vehicles[vehname]['Name']
            else
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    LSCore.Functions.TriggerCallback('framework-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            LSCore.Functions.Notify('Je bent te ver van de race vandaag. GPS Locatie is gezet', 'error', 5000)
...
RegisterNUICallback('RefreshSharedAccounts', function(data, cb)
    LSCore.Functions.TriggerCallback('framework-phone:server:getSharedAccounts', function(accounts)
        cb(accounts)
    end)
end)
...
RegisterNUICallback('GetPlayerKeys', function(data, cb)
    LSCore.Functions.TriggerCallback('framework-phone:server:get:my:houseskeys', function(Keys)
        cb(Keys)
    end)
end)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    LSCore.Functions.Notify("GPS gezet naar " .. data.HouseData.HouseData.adress .. "!", "success")
end)

RegisterNUICallback('RemoveKeyholder', function(data)
    TriggerServerEvent('framework-houses:server:removeHouseKey', data.HouseData.name, {
...
RegisterNUICallback('TransferCid', function(data, cb)
    ...
    LSCore.Functions.TriggerCallback('framework-phone:server:TransferCid', function(CanTransfer)
        cb(CanTransfer)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    LSCore.Functions.Notify('GPS gezet!', 'success')
end)
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
        LSCore.Functions.TriggerCallback('framework-phone:server:CanTransferMoney', function(Transferd)
            if Transferd then
...
RegisterNUICallback('CallContact', function(data, cb)
    LSCore.Functions.TriggerCallback('framework-phone:server:GetCallState', function(CanCall, IsOnline, contactData)
        local status = {
            CanCall = CanCall,
            IsOnline = IsOnline,
...
RegisterNUICallback("TakePhoto", function(data,cb)
    ...
    while takePhoto do
        ...
        elseif IsControlJustPressed(1, 176) then
            LSCore.Functions.TriggerCallback("framework-phone:server:GetWebhook",function(hook)
                LSCore.Functions.Notify('Foto gemaakt', 'primary')
                exports['screenshot-basic']:requestScreenshotUpload(tostring(hook), "files[]", function(data)
                    local image = json.decode(data)
                    DestroyMobilePhone()
                    CellCamActivate(false, false)
                    TriggerServerEvent('framework-phone:server:addImageToGallery', image.attachments[1].proxy_url)
                    Wait(400)
                    TriggerServerEvent('framework-phone:server:getImageFromGallery')
                    cb(json.encode(image.attachments[1].proxy_url))
                    LSCore.Functions.Notify('Foto is opgeslagen!', 'success')
                    OpenPhone()
                end)
            end)
...
RegisterNetEvent('framework-phone:client:TransferMoney', function(amount, newmoney)
    PhoneData.PlayerData.money.bank = newmoney
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Bank", text = "&#36;"..amount.." bijgeschreven!", icon = "fas fa-university", color = "#8c7ae6", }, })
    SendNUIMessage({ action = "UpdateBank", NewBalance = PhoneData.PlayerData.money.bank })
end)

RegisterNetEvent('framework-phone:client:UpdateTweets', function(src, Tweets, NewTweetData, delete)
        ...
        if src ~= MyPlayerId then
            SendNUIMessage({
                ...
                PhoneNotify = {
                    ...
                    text = "Heeft een nieuwe Tweet gepost",
                    icon = "fab fa-twitter",
                    color = "#1DA1F2",
                },
            })
...
RegisterNetEvent('framework-phone:client:RaceNotify', function(message)
    SendNUIMessage({
        action = "PhoneNotification",
        PhoneNotify = {
            title = "Racing",
...
RegisterNetEvent('framework-phone:client:AddRecentCall', function(data, time, type)
...
RegisterNetEvent("framework-phone-new:client:BankNotify", function(text)
    SendNUIMessage({
        action = "PhoneNotification",
        NotifyData = {
            title = "Bank",
...
RegisterNetEvent('framework-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        action = "PhoneNotification",
        PhoneNotify = {
            title = "Nieuwe mail",
            text = "van: "..MailData.sender,
...
RegisterNetEvent('framework-phone:client:UpdateMails', function(NewMails)
    SendNUIMessage({
        action = "UpdateMails",
        Mails = NewMails
    })
...
RegisterNetEvent('framework-phone:client:UpdateAdvertsDel', function(Adverts)
...
RegisterNetEvent('framework-phone:client:UpdateAdverts', function(Adverts, LastAd)
    PhoneData.Adverts = Adverts
    SendNUIMessage({
        action = "PhoneNotification",
        PhoneNotify = {
...
-- RegisterNetEvent('framework-phone:client:BillingEmail', function(data, paid, name)
--     if paid then
...
RegisterNetEvent('framework-phone:client:CancelCall', function()
    if PhoneData.CallData.CallType == "ongoing" then
        SendNUIMessage({
...
RegisterNUICallback('phone-silent-button', function(data,cb)
    if CallVolume == tonumber("0.2") then
        CallVolume = 0
        LSCore.Functions.Notify("Stille modus aan", "success")
        cb(true)
    else
        CallVolume = 0.2
        LSCore.Functions.Notify("Stille modus uit", "error")
        cb(false)
    end
end)

RegisterNetEvent('framework-phone:client:GetCalled', function(CallerNumber, CallId, AnonymousCall)
    local RepeatCount = 0
    local CallData = {
        number = CallerNumber,
...
RegisterNetEvent('framework-phone:client:UpdateMessages', function(ChatMessages, SenderNumber, New)
...
RegisterNetEvent('framework-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
...
RegisterNetEvent('framework-phone:RefreshPhone', function()
    LoadPhone()
    SetTimeout(250, function()
        SendNUIMessage({
...
RegisterNetEvent('framework-phone:client:AddTransaction', function(SenderData, TransactionData, Message, Title)
    local Data = {
...
RegisterNetEvent('framework-phone:client:AddNewSuggestion', function(SuggestionData)
...
RegisterNetEvent('framework-phone:client:UpdateHashtags', function(Handle, msgData)
...
RegisterNetEvent('framework-phone:client:AnswerCall', function()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        PhoneData.CallData.CallType = "ongoing"
        PhoneData.CallData.AnsweredCall = true
        PhoneData.CallData.CallTime = 0
...
RegisterNetEvent('framework-phone:client:addPoliceAlert', function(alertData)
    PlayerJob = LSCore.Functions.GetPlayerData().job
...
RegisterNetEvent('framework-phone:client:GiveContactDetails', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local PlayerId = GetPlayerServerId(player)
        TriggerServerEvent('framework-phone:server:GiveContactDetails', PlayerId)
    else
        LSCore.Functions.Notify("No one nearby!", "error")
    end
end)

RegisterNetEvent('framework-phone:client:UpdateLapraces', function()
    SendNUIMessage({
        action = "UpdateRacingApp",
    })
end)

RegisterNetEvent('framework-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
...
RegisterNetEvent('framework-phone:refreshImages', function(images)
    PhoneData.Images = images
end)

RegisterNetEvent("framework-phone:client:CustomNotification", function(title, text, icon, color, timeout) -- Send a PhoneNotification to the phone from anywhere
    SendNUIMessage({
        action = "PhoneNotification",
        PhoneNotify = {
            title = title,
...
RegisterNUICallback('AcceptPingPlayer', function()
    TriggerServerEvent('framework-pings:server:acceptping')
    TriggerEvent("framework-phone:ping:client:UiUppers", false)
end)

RegisterNUICallback('rejectPingPlayer', function()
    TriggerServerEvent('framework-pings:server:denyping')
    TriggerEvent("framework-phone:ping:client:UiUppers", false)
end)

RegisterNUICallback('SendPingPlayer', function(data)
    TriggerServerEvent('framework-pings:server:SendPing2', data.id)
    
end)

local CurrentPings = {}

RegisterNetEvent('framework-pings:client:DoPing', function(id)
    TriggerServerEvent('framework-pings:server:SendPing', id)
end)

RegisterNetEvent('framework-pings:client:AcceptPing', function(PingData, SenderData)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)

        TriggerServerEvent('framework-pings:server:SendLocation', PingData, SenderData)
end)

RegisterNetEvent('framework-pings:client:SendLocation', function(PingData, SenderData)
    LSCore.Functions.Notify('Their location has been blipped on your map', 'success')
    ...
    SetTimeout(5 * (60 * 1000), function()
        LSCore.Functions.Notify('Ping '..PingData.sender..' has expired...', 'error')
...
RegisterNetEvent('framework-phone:ping:client:UiUppers', function(toggle)
    if toggle then
        SendNUIMessage({
            action = "acceptrejectBlock",
        })
        TriggerEvent("framework-hud:ping:client:ShowIcon", true)
    else
        SendNUIMessage({
            action = "acceptrejectNone",
        })
        TriggerEvent("framework-hud:ping:client:ShowIcon", false)
    end
end)

RegisterNUICallback('CasinoAddBet', function(data)
    TriggerServerEvent('framework-phone:server:CasinoAddBet', data)
end)

RegisterNetEvent('framework-phone:client:addbetForAll', function(data)
    SendNUIMessage({
        action = "BetAddToApp",
        datas = data,
    })
...
RegisterNUICallback('BettingAddToTable', function(data)
    TriggerServerEvent('framework-phone:server:BettingAddToTable', data)
end)

RegisterNUICallback('CasinoDeleteTable', function(data)
    TriggerServerEvent('framework-phone:server:DeleteAndClearTable')
end)

RegisterNUICallback('CheckHasBetTable', function(data, cb)
    LSCore.Functions.TriggerCallback('framework-phone:server:CheckHasBetTable', function(HasTable)
        cb(HasTable)
    end)
end)

RegisterNUICallback('casino_status', function(data)
    TriggerServerEvent('framework-phone:server:casino_status')
end)

RegisterNUICallback('CheckHasBetStatus', function(data, cb)
    LSCore.Functions.TriggerCallback('framework-phone:server:CheckHasBetStatus', function(HasStatus)
        cb(HasStatus)
    end)
end)

RegisterNUICallback('WineridCasino', function(data)
    TriggerServerEvent('framework-phone:server:WineridCasino', data)
end)
...
RegisterNUICallback('CasinoPhoneJobCenter', function(data)
    if data.action == 1 then
        TriggerServerEvent('framework-phone:server:SetJobJobCenter', data)
    elseif data.action == 2 then
        SetNewWaypoint(data.x, data.y)
        LSCore.Functions.Notify('GPS set', "success")
    end
end)

RegisterNUICallback('employment_CreateJobGroup', function(data)
    TriggerServerEvent('framework-phone:server:employment_CreateJobGroup', data)
end)

RegisterNetEvent('framework-phone:client:EveryoneGrupAddsForAll', function(data)
    SendNUIMessage({
        action = "GroupAddDIV",
        datas = data,
    })
...
RegisterNUICallback('employment_DeleteGroup', function(data)
    TriggerServerEvent('framework-phone:server:employment_DeleteGroup', data)
end)

RegisterNUICallback('GetGroupsApp', function(data, cb)
    LSCore.Functions.TriggerCallback('framework-phone:server:GetGroupsApp', function(HasGroups)
        cb(HasGroups)
    end)
end)

RegisterNUICallback('employment_JoinTheGroup', function(data)
    TriggerServerEvent('framework-phone:server:employment_JoinTheGroup', data)
end)

RegisterNUICallback('employment_leave_grouped', function(data)
    TriggerServerEvent('framework-phone:server:employment_leave_grouped', data)
end)

RegisterNUICallback('employment_CheckPlayerNames', function(data, cb)
    LSCore.Functions.TriggerCallback('framework-phone:server:employment_CheckPlayerNames', function(HasName)
        cb(HasName)
    end, data.id)
end)

RegisterNUICallback('SendBillForPlayer_debt', function(data)
    TriggerServerEvent('framework-phone:server:SendBillForPlayer_debt', data)
end)

RegisterNUICallback('GetHasBills_debt', function(data, cb)
    LSCore.Functions.TriggerCallback('framework-phone:server:GetHasBills_debt', function(Has)
        cb(Has)
    end)
end)

RegisterNUICallback('debit_AcceptBillForPay', function(data)
    TriggerServerEvent('framework-phone:server:debit_AcceptBillForPay', data)
end)

RegisterNetEvent('framework-phone:RefreshPhoneForDebt', function()
    SendNUIMessage({
        action = "DebtRefresh",
    })
end)
...
RegisterNUICallback('wenmo_givemoney_toID', function(data)
    TriggerServerEvent('framework-phone:server:wenmo_givemoney_toID', data)
end)
...
RegisterNUICallback('documents_Save_Note_As', function(data)
    TriggerServerEvent('framework-phone:server:documents_Save_Note_As', data)
end)

RegisterNUICallback('GetNote_for_Documents_app', function(data, cb)
    LSCore.Functions.TriggerCallback('framework-phone:server:GetNote_for_Documents_app', function(Has)
        cb(Has)
    end)
end)

RegisterNetEvent('framework-phone:RefReshNotes_Free_Documents', function()
    SendNUIMessage({
...
RegisterNUICallback('Send_lsbn_ToChat', function(data)
    TriggerServerEvent('framework-phone:server:Send_lsbn_ToChat', data)
end)

RegisterNetEvent('framework-phone:LSBN-reafy-for-add', function(data, toggle, text)
    if toggle then
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
...
RegisterNUICallback('GetLSBNchats', function(data)
    TriggerServerEvent('framework-phone:server:GetLSBNchats', data)
end)
...
local function GetGroupCSNs(Data)
    local gData = {}
    LSCore.Functions.TriggerCallback('framework-phone:server:GetGroupCSNs', function(HasGroup)
        gData = HasGroup
    end, Data)
    Wait(100)
    return gData
