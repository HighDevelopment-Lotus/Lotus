    ...
    if Player ~= nil then
        ...
        if garageresult[1] ~= nil then
            for k, v in pairs(garageresult) do
                ...
                if (LSCore.Shared.Vehicles[vehicleModel] ~= nil) and (Garages[v.garage] ~= nil) then
                    v.garage = Garages[v.garage].label
                    v.vehicle = LSCore.Shared.Vehicles[vehicleModel].name
                    v.brand = LSCore.Shared.Vehicles[vehicleModel].brand
                end
...
LSCore.Functions.CreateCallback('framework-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    local Invoices = {}
    local Ply = LSCore.Functions.GetPlayer(source)
    local SenderPly = LSCore.Functions.GetPlayerByCitizenId(sendercitizenid)
    ...
    TriggerEvent('framework-phone:server:sendNewMailToOffline', sendercitizenid, invoiceMailData)
    TriggerEvent("framework-bossmenu:server:addAccountMoney", society, amount)
...
LSCore.Functions.CreateCallback('framework-phone:server:ScanPlate', function(source, cb, plate)
    ...
    if plate ~= nil then
        ...
        TriggerClientEvent('LSCore:Notify', src, 'No Vehicle Nearby', 'error')
        cb(nil)
    end
end)
...
LSCore.Functions.CreateCallback('framework-phone:server:CanTransferMoney', function(source, cb, amount, iban)
    ...
    if (Player.PlayerData.money.bank - amount) >= 0 then
        ...
        local result = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', {query})
        if result[1] ~= nil then
            local Reciever = LSCore.Functions.GetPlayerByCitizenId(result[1].citizenid)
...
RegisterNetEvent('framework-phone:server:AddAdvert', function(msg, url)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
...
RegisterNetEvent('framework-phone:server:DeleteAdvert', function()
    local Player = LSCore.Functions.GetPlayer(source)
    local citizenid = Player.PlayerData.citizenid
    Adverts[citizenid] = nil
    TriggerClientEvent('framework-phone:client:UpdateAdvertsDel', -1, Adverts)
end)

RegisterNetEvent('framework-phone:server:SetCallState', function(bool)
    local src = source
...
RegisterNetEvent('framework-phone:server:RemoveMail', function(MailId)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
...
RegisterNetEvent('framework-phone:server:sendNewMail', function(mailData)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
...
RegisterNetEvent('framework-phone:server:sendNewMailToOffline', function(citizenid, mailData)
...
RegisterNetEvent('framework-phone:server:sendNewEventMail', function(citizenid, mailData)
    local Player = LSCore.Functions.GetPlayerByCitizenId(citizenid)
    if mailData.button == nil then
        MySQL.insert('INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)', {citizenid, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0})
    else
    ...
    SetTimeout(200, function()
        ...
        TriggerClientEvent('framework-phone:client:UpdateMails', Player.PlayerData.source, mails)
    end)
end)

RegisterNetEvent('framework-phone:server:ClearButtonData', function(mailId)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
...
RegisterNetEvent('framework-phone:server:MentionedPlayer', function(firstName, lastName, TweetMessage)
    for k, v in pairs(LSCore.Functions.GetPlayers()) do
        local Player = LSCore.Functions.GetPlayer(v)
        if Player ~= nil then
...
RegisterNetEvent('framework-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = LSCore.Functions.GetPlayer(src)
...
RegisterNetEvent('framework-phone:server:BillingEmail', function(data, paid)
    for k, v in pairs(LSCore.Functions.GetPlayers()) do
        local target = LSCore.Functions.GetPlayer(v)
        if target.PlayerData.job.name == data.society then
            if paid then
                local name = '' .. LSCore.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' .. LSCore.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
                TriggerClientEvent('framework-phone:client:BillingEmail', target.PlayerData.source, data, true, name)
            else
                local name = '' .. LSCore.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' .. LSCore.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
                TriggerClientEvent('framework-phone:client:BillingEmail', target.PlayerData.source, data, false, name)
            end
        end
    end
end)

RegisterNetEvent('framework-phone:server:UpdateHashtags', function(Handle, messageData)
...
RegisterNetEvent('framework-phone:server:SetPhoneAlerts', function(app, alerts)
    local src = source
...
RegisterNetEvent('framework-phone:server:TransferMoney', function(iban, amount)
    local src = source
    ...
    if result[1] ~= nil then
        ...
        if reciever ~= nil then
            ...
            if PhoneItem ~= nil then
                TriggerClientEvent('framework-phone:client:TransferMoney', reciever.PlayerData.source, amount,
        ...
        TriggerClientEvent('LSCore:Notify', src, "That account number doesn't exist!", "error")
    end
end)

RegisterNetEvent('framework-phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, oldIban)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
    MySQL.query(
...
RegisterNetEvent('framework-phone:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
...
RegisterNetEvent('framework-phone:server:AddNewContact', function(name, number, iban)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
...
RegisterNetEvent('framework-phone:server:UpdateMessages', function(ChatMessages, ChatNumber, New)
    local src = source
...
RegisterNetEvent('framework-phone:server:AddRecentCall', function(type, data)
    local src = source
    local Ply = LSCore.Functions.GetPlayer(src)
...
RegisterNetEvent('framework-phone:server:CancelCall', function(ContactData)
...
RegisterNetEvent('framework-phone:server:AnswerCall', function(CallData)
...
RegisterNetEvent('framework-phone:server:SaveMetaData', function(MData)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
...
RegisterNetEvent('framework-phone:server:GiveContactDetails', function(PlayerId)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
...
RegisterNetEvent('framework-phone:server:AddTransaction', function(data)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
...
RegisterNetEvent('framework-phone:server:InstallApplication', function(ApplicationData)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
...
RegisterNetEvent('framework-phone:server:RemoveInstallation', function(App)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
...
RegisterNetEvent('framework-phone:server:getImageFromGallery', function()
    ...
    local Player = LSCore.Functions.GetPlayer(src)
    local images = MySQL.query.await('SELECT * FROM phone_gallery WHERE citizenid = ? ORDER BY `date` DESC',{Player.PlayerData.citizenid})
    TriggerClientEvent('framework-phone:refreshImages', src, images)
end)
...
LSCore.Commands.Add("setmetadata", "Set Player Metadata (God Only)", {}, false, function(source, args)
    local Player = LSCore.Functions.GetPlayer(source)
    if args[1] then
...
LSCore.Commands.Add('bill', 'Bill A Player', {{name = 'id', help = 'Player ID'}, {name = 'amount', help = 'Fine Amount'}}, false, function(source, args)
    local biller = LSCore.Functions.GetPlayer(source)
    local billed = LSCore.Functions.GetPlayer(tonumber(args[1]))
        ...
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('framework-phone:RefreshPhone', billed.PlayerData.source)
                    TriggerClientEvent('LSCore:Notify', source, 'Invoice successfully sent', 'success')
                    TriggerClientEvent('LSCore:Notify', billed.PlayerData.source, 'New Invoice Received')
                else
                    TriggerClientEvent('LSCore:Notify', source, 'Must be a valid amount above 0', 'error')
                end
            else
                TriggerClientEvent('LSCore:Notify', source, 'You cannot bill yourself...', 'error')
            end
        else
            TriggerClientEvent('LSCore:Notify', source, 'Player not Online', 'error')
        end
    else
        TriggerClientEvent('LSCore:Notify', source, 'No Access', 'error')
    end
end)
...
RegisterNetEvent('framework-pings:server:SendPing2', function(id)
    local src = source

    TriggerClientEvent('framework-pings:client:DoPing', src, tonumber(id))
end)

RegisterNetEvent('framework-pings:server:acceptping', function()
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
    if Pings[src] ~= nil then
        TriggerClientEvent('framework-pings:client:AcceptPing', src, Pings[src], LSCore.Functions.GetPlayer(Pings[src].sender))
        TriggerClientEvent('LSCore:Notify', Pings[src].sender, Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname.." accepted your ping request!")
        Pings[src] = nil
    else
        TriggerClientEvent('LSCore:Notify', src, "You have no ping...", "error")
    end
end)

RegisterNetEvent('framework-pings:server:denyping', function()
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
    if Pings[src] ~= nil then
        TriggerClientEvent('LSCore:Notify', Pings[src].sender, "Your ping request has been rejected...", "error")
        TriggerClientEvent('LSCore:Notify', src, "You turned down the ping...", "success")
        Pings[src] = nil
    else
        TriggerClientEvent('LSCore:Notify', src, "You have no ping...", "error")
    end
...
RegisterNetEvent('framework-pings:server:SendPing', function(id)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
    local Target = LSCore.Functions.GetPlayer(id)
    ...
    if Target ~= nil then
        ...
        if OtherItem ~= nil then
            TriggerClientEvent('LSCore:Notify', src, "You have requested the location of "..Target.PlayerData.charinfo.firstname.." "..Target.PlayerData.charinfo.lastname)
            Pings[id] = {
                coords = coords,
                sender = src,
            }
            TriggerClientEvent('LSCore:Notify', id, "You have received a ping request from "..Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname..". Use the app to allow or reject!")
            TriggerClientEvent('framework-phone:ping:client:UiUppers', id, true)
        else
            TriggerClientEvent('LSCore:Notify', src, "Could not send ping...", "error")
        end
    else
        TriggerClientEvent('LSCore:Notify', src, "This person is not in the city...", "error")
    end
end)

RegisterNetEvent('framework-pings:server:SendLocation', function(PingData, SenderData)
    TriggerClientEvent('framework-pings:client:SendLocation', PingData.sender, PingData, SenderData)
end)


LSCore.Commands.Add("p#", "Provide Phone Number", {}, false, function(source, args)
    local Player = LSCore.Functions.GetPlayer(source)
...
local CasinoTable = {}
local BetNumber = 0
RegisterNetEvent('framework-phone:server:CasinoAddBet', function(data)
    BetNumber = BetNumber + 1
    CasinoTable[BetNumber] = {['Name'] = data.name, ['chanse'] = data.chanse, ['id'] = BetNumber}
    TriggerClientEvent('framework-phone:client:addbetForAll', -1, CasinoTable)
end)

local CasinoBetList = {}
local casino_status = true

RegisterNetEvent('framework-phone:server:BettingAddToTable', function(data)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
    ...
    if casino_status then
        if Player.PlayerData.money.bank >= amount then
            if not CasinoBetList[CSN] then
                ...
                TriggerClientEvent('LSCore:Notify', src, "You are already betting...", "error")
            end
        else
            TriggerClientEvent('LSCore:Notify', src, "You do not have enough money!", "error")
        end
    else
        TriggerClientEvent('LSCore:Notify', src, "Betting is not active...", "error")
    end
end)

RegisterNetEvent('framework-phone:server:DeleteAndClearTable', function()
    local src = source
    CasinoTable = {}
    CasinoBetList = {}
    BetNumber = 0
...
LSCore.Functions.CreateCallback('framework-phone:server:CheckHasBetTable', function(source, cb)
    cb(CasinoTable)
end)


RegisterNetEvent('framework-phone:server:casino_status', function()
    casino_status = not casino_status
end)

LSCore.Functions.CreateCallback('framework-phone:server:CheckHasBetStatus', function(source, cb)
    cb(casino_status)
end)

RegisterNetEvent('framework-phone:server:WineridCasino', function(data)
    local Winer = data.id
    for k, v in pairs(CasinoBetList) do
        if v.id == Winer then
            local OtherPly = LSCore.Functions.GetPlayerByCitizenId(v.csn)
            if OtherPly then
...
RegisterNetEvent('framework-phone:server:SetJobJobCenter', function(data)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
    if Player.Functions.SetJob(data.job) then
        TriggerClientEvent('LSCore:Notify', src, 'Changed your job to: '..data.label)
    else
        TriggerClientEvent('LSCore:Notify', src, 'Invalid Job...', 'error')
    end
end)
...
RegisterNetEvent('framework-phone:server:employment_CreateJobGroup', function(data)
    ...
    if Player then
        if not EmploymentGroup[CSN] then
            ...
            TriggerClientEvent('LSCore:Notify', src, "Group created!", "success")
        else
            TriggerClientEvent('LSCore:Notify', src, "You have already created a group...", "error")
        end
    end
end)

RegisterNetEvent('framework-phone:server:employment_DeleteGroup', function(data)
...
RegisterNetEvent('framework-phone:server:SendBillForPlayer_debt', function(data)
    local src = source
    local biller = LSCore.Functions.GetPlayer(src)
    local billed = LSCore.Functions.GetPlayer(tonumber(data.ID))
    local amount = tonumber(data.Amount)
    if billed then
            if amount and amount > 0 then
                ...
                TriggerClientEvent('LSCore:Notify', src, 'Debt successfully sent!', 'success')
                TriggerClientEvent('LSCore:Notify', billed.PlayerData.source, 'New Debt Received')
                Wait(1)
                TriggerClientEvent('framework-phone:RefreshPhoneForDebt', billed.PlayerData.source)
            else
                TriggerClientEvent('LSCore:Notify', src, 'Must be a valid amount above 0', 'error')
            end
    else
        TriggerClientEvent('LSCore:Notify', src, 'Player not Online', 'error')
    end
end)

LSCore.Functions.CreateCallback('framework-phone:server:GetHasBills_debt', function(source, cb)
    local src = source
    local Ply = LSCore.Functions.GetPlayer(src)
...
RegisterNetEvent('framework-phone:server:debit_AcceptBillForPay', function(data)
    local src = source
    local Ply = LSCore.Functions.GetPlayer(src)
    local OtherPly = LSCore.Functions.GetPlayerByCitizenId(data.CSN)
    ...
    if OtherPly then
        if Ply.PlayerData.money.bank then
            ...
            Wait(1)
            TriggerClientEvent('framework-phone:RefreshPhoneForDebt', OtherPly.PlayerData.source)
        else
            TriggerClientEvent('LSCore:Notify', src, 'You don\'t have enough money...', 'error')
        end
    else
        TriggerClientEvent('LSCore:Notify', src, 'Player not Online', 'error')
    end
...
RegisterNetEvent('framework-phone:server:wenmo_givemoney_toID', function(data)
    local src = source
    local Ply = LSCore.Functions.GetPlayer(src)
    local OtherPly = LSCore.Functions.GetPlayer(tonumber(data.ID))
...
RegisterNetEvent('framework-phone:server:documents_Save_Note_As', function(data)
    local src = source
    local Ply = LSCore.Functions.GetPlayer(src)
    if data.Type == "New" then
        ...
        TriggerClientEvent('LSCore:Notify', src, 'Note Saved')
    elseif data.Type == "Update" then
        ...
        if Note[1] ~= nil then
            ...
            MySQL.insert('INSERT INTO phone_note (citizenid, title,  text, lastupdate) VALUES (?, ?, ?, ?)',{Ply.PlayerData.citizenid, data.Title, data.Text, data.Time})
            TriggerClientEvent('LSCore:Notify', src, 'Note Updated', 'success')
        end
    elseif data.Type == "Delete" then
        local ID = tonumber(data.ID)
        MySQL.query('DELETE FROM phone_note WHERE id = ?', {ID})
        TriggerClientEvent('LSCore:Notify', src, 'Note Deleted', 'error')
    end
    Wait(100)
    TriggerClientEvent('framework-phone:RefReshNotes_Free_Documents', src)
end)

LSCore.Functions.CreateCallback('framework-phone:server:GetNote_for_Documents_app', function(source, cb)
    local src = source
    local Ply = LSCore.Functions.GetPlayer(src)
