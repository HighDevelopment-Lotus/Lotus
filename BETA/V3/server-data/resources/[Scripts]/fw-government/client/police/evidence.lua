		  			...
		  			if not IsWeaponSilent(Weapon) then
						if not GunShotAlert then
							...
							if math.random(1, 3) == 1 then
		  						TriggerServerEvent('framework-police:server:send:alert:gunshots', GetEntityCoords(PlayerPedId()), GunCategory, LSCore.Functions.GetStreetLabel(), IsPedInAnyVehicle(PlayerPedId()))
	...
	while true do
		...
		if LoggedIn then
			if PlayerJob.name == 'police' and PlayerJob.onduty then
				if IsPlayerFreeAiming(PlayerId()) and GetSelectedPedWeapon(PlayerPedId()) == GetHashKey("WEAPON_FLASHLIGHT") or CameraActive then
							...
							if Distance < 10.0 then
								...
								if Distance < 0.65 then
									...
									if not ShowingInteractions then
										...
										exports['fw-ui']:ShowInteraction('[E] Bewijs', 'primary')
									end
									if IsControlJustReleased(0, 38) then
										if v['Weapon'] ~= nil then
											v['Ammo'] = Config.AmmoLabels[exports['fw-weapons']:GetAmmoType(v['Weapon'])]
										end
										local StreetLabel = LSCore.Functions.GetStreetLabel()
										TriggerServerEvent("framework-police:server:add:evidence", k, v, StreetLabel:gsub("%'", ""))
									end
								end
							end
						end
...
RegisterNetEvent('framework-police:client:set:evidence')
AddEventHandler('framework-police:client:set:evidence', function(bool)
	CameraActive = bool
end)

RegisterNetEvent('framework-police:client:open:evidence')
AddEventHandler('framework-police:client:open:evidence', function(args)
		...
		local Distance = #(Coords - v['Coords'])
 	  	if Distance <= 45.0 then
			...
			if exports['fw-inv']:CanOpenInventory() then
				TriggerServerEvent('framework-inv:server:open:inventory:other', "evidencestash_"..args, 'Stash', 50, 2500000)
			end
 	  	end
 	end
...
RegisterNetEvent('framework-police:client:sync:evidence')
AddEventHandler('framework-police:client:sync:evidence', function(EvidenceID, ConfigData)
