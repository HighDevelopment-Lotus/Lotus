...
LSCore.Functions.CreateUseableItem("lotto-card", function(source, item)
	...
	if Player.Functions.RemoveItem(item.name, 1, false, true) then
		...
		TriggerClientEvent("framework-lottery:client:open:card", source, math.random(11,99), Prijs) 
    end
end)

RegisterServerEvent('framework-lottery:server:sell:card')
AddEventHandler('framework-lottery:server:sell:card', function()
    local Player = LSCore.Functions.GetPlayer(source)
