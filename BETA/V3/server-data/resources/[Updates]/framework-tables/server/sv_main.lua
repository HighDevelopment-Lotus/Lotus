...
LSCore.Functions.CreateCallback('framework-tables:server:finish', function(source, cb, type)
...
LSCore.Functions.CreateCallback('framework-tables:server:get:item:amount', function(source, cb, item, amount)
    local Item = item
    local Amount = amount
...
LSCore.Functions.CreateUseableItem("cracktable", function(source, item)
	...
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent('framework-tables:client:place:table', source, 'Meth')
    end
end)
