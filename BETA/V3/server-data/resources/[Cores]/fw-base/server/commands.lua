...
LSCore.Commands.Add("job", "Bekijk jouw huidige baan", {}, false, function(source, args)
    local src = source
...
LSCore.Commands.Add("gang", "Bekijk jouw huidige gang", {}, false, function(source, args)
    local src = source
    local Player = LSCore.Functions.GetPlayer(src)
    TriggerClientEvent('chatMessage', source, "SYSTEM", "warning", "Baan: "..Player.PlayerData.gang.label.." - "..Player.PlayerData.gang.gradelabel)
end)


LSCore.Commands.Add("setgang", "Voeg een speler toe aan een gang", {{name="id", help="Player ID"}, {name="job", help="Gang"}, {name="grade", help="Rang"}}, true, function(source, args)
    local Player = LSCore.Functions.GetPlayer(tonumber(args[1]))
    if Player ~= nil then
