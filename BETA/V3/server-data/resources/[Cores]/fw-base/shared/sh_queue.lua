...
function Queue:AddToConnecting(ids, ignorePos, autoRemove, done)
    local function remove()
        ...
        done(Config.Language.connectingerr .."\n🔸 Kijk op onze discord voor meer informatie: discord.gg/6CECJx4")
...
local function playerConnect(source, setKickReason, deferrals)
    ...
    if Queue:NotFull(true) and _Queue.JoinDelay <= GetGameTimer() then
        ...
        TriggerEvent("framework-logs:server:SendLog", "joinleave", "Queue", "orange", "**".. name .. "** joining..")
